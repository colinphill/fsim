...
SystemVerilogCoverageDeclarationPercentage declaration_percentage(
  ...
  if (item.kind == SystemVerilogCoverageDeclarationKind::Coverpoint) {
      ...
      if (bin.selection
        ...
        for (const auto& hit : instance.bin_hits) {
          ...
          result.eligible_weight =
              saturating_add(result.eligible_weight, bin.weight);
          if (hit.covered) {
            result.covered_weight =
                saturating_add(result.covered_weight, bin.weight);
          }
        }
      } else {
        result.eligible_weight =
    ...
    for (const auto& state : instance.cross_bin_state) {
      ...
      result.eligible_weight =
          saturating_add(result.eligible_weight, state.weight);
      if (state.covered) {
        result.covered_weight =
            saturating_add(result.covered_weight, state.weight);
      }
    }
  } else {
    for (const auto& bin : item.bins) {
...
void merge_instance_state(
  ...
  for (const auto& state : instance.cross_bin_state) {
    ...
    found->hit_count =
        saturating_add(found->hit_count, state.hit_count);
    found->exclusion_count =
        saturating_add(found->exclusion_count, state.exclusion_count);
    found->excluded = found->excluded || state.excluded;
    found->covered =
        !found->excluded && found->hit_count >= found->at_least;
  }
}
...
calculate_systemverilog_covergroup_instance_percentage(
  ...
  for (const auto& item : declaration.coverage_declarations) {
    auto item_result =
        declaration_percentage(declaration, instance, item);
