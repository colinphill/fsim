...
std::optional<Expression> VhdlParser::parse_vhdl_bit_string_literal() {
  ...
  if (at(TokenKind::Number) && at(TokenKind::Identifier, 1) &&
      at(TokenKind::StringLiteral, 2)) {
    specifier_offset = 1;
  ...
  if (specifier != "b" && specifier != "o" && specifier != "x" &&
      specifier != "d" && specifier != "ub" && specifier != "uo" &&
      specifier != "ux" && specifier != "sb" && specifier != "so" &&
      specifier != "sx" && specifier != "ud" && specifier != "sd") {
    return std::nullopt;
  }
  ...
  if (specifier_offset != 0 &&
      start.span.end.offset != specifier_token.span.begin.offset) {
    malformed = true;
  }
  ...
  try {
    ...
    const auto append_digit = [&](const char raw, const unsigned radix_width) {
      ...
      if (std::string_view{"UXZWLH-"}.find(character) !=
      ...
      const auto position =
          std::string_view{"0123456789ABCDEF"}.find(character);
      if (position == std::string_view::npos ||
          position >= (std::size_t{1} << radix_width)) {
        malformed = true;
        return;
      }
    ...
    if (base == 'b') {
      for (const char raw : digits) {
        ...
        const auto character =
            static_cast<char>(std::toupper(static_cast<unsigned char>(raw)));
        if (std::string_view{"01UXZWLH-"}.find(character) ==
            std::string_view::npos) {
          malformed = true;
        } else {
          append_bit(character);
      ...
      if (limbs.size() == 1U && limbs.front() == 0U) {
        ...
        const auto most_significant_bits =
            32U - static_cast<unsigned>(std::countl_zero(limbs.back()));
    ...
    if (discarded_character) {
      const char fill =
          specifier.front() == 's' && !bits.empty() ? bits.front() : '0';
...
VhdlParser::binary_operation() const {
  ...
  if (at(TokenKind::Question) &&
      (at(TokenKind::Assign, 1) ||
       (at(TokenKind::NotEqual, 1) && current(1).text == "/="))) {
  ...
  if (at(TokenKind::Star) || at(TokenKind::Slash) || keyword("mod", 0, true) ||
      keyword("rem", 0, true)) {
...
Expression VhdlParser::parse_primary() {
  ...
  if (at(TokenKind::Number)) {
    ...
    if (at(TokenKind::Identifier) && !binary_operation() &&
        std::ranges::find(physical_literal_terminators,
  ...
  if (at(TokenKind::Identifier)) {
    ...
    if (match(TokenKind::Apostrophe)) {
      ...
      static constexpr std::array<std::string_view, 25> supported_attributes{
          "left",          "right",       "low",     "high",
          ...
          "reverse_range"};
      if (std::ranges::find(supported_attributes, designator) ==
      ...
      const bool requires_argument =
          designator == "pos" || designator == "val" || designator == "succ" ||
      ...
      if (!at(TokenKind::RightParen)) {
        do {
          ...
          if (argument_name.empty() &&
              (keyword("downto", 0, true) || keyword("to", 0, true))) {
  ...
  if (match(TokenKind::LeftParen)) {
    ...
    auto first = parse_expression();
    if (!at(TokenKind::Arrow) && !at(TokenKind::Comma) &&
        !at(TokenKind::Pipe) && !keyword("to", 0, true) &&
    ...
    const auto append_association = [&](Expression head) {
      const auto choice_with_optional_range = [&]() {
        ...
        const auto direction = previous();
        Expression right;
        if (at(TokenKind::Arrow) || at(TokenKind::Pipe) ||
            at(TokenKind::Comma) || at(TokenKind::RightParen) || at_end()) {
      ...
      while (match(TokenKind::Pipe)) {
        if (at(TokenKind::Arrow) || at(TokenKind::Comma) ||
            at(TokenKind::RightParen) || at_end()) {
      ...
      if (match(TokenKind::Arrow)) {
        ...
        for (auto &choice_expression : choices) {
          if (choice_expression.kind == ExpressionKind::BooleanLiteral ||
              choice_expression.kind == ExpressionKind::LogicLiteral ||
              choice_expression.kind == ExpressionKind::StringLiteral ||
              choice_expression.kind == ExpressionKind::Aggregate) {
      ...
      if (choices.size() != 1 ||
          (choices.front().kind == ExpressionKind::Binary &&
