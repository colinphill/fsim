...
[[nodiscard]] std::optional<std::int64_t> exact_integer_value(
    const std::span<const Token> tokens) {
  bool negative{};
  std::string spelling;
  if (tokens.size() == 1U && tokens.front().kind == TokenKind::Number) {
    spelling = tokens.front().text;
  } else if (
      tokens.size() == 2U && tokens.front().kind == TokenKind::Minus
      && tokens.back().kind == TokenKind::Number) {
    negative = true;
    spelling = tokens.back().text;
  } else {
    return std::nullopt;
  }
  spelling.erase(
...
[[nodiscard]] std::optional<CoverageLiteral> coverage_literal(
  ...
  if (apostrophe == std::string::npos) {
    auto exact = exact_integer_value(tokens);
    if (!exact) return std::nullopt;
    if (negative) *exact = -*exact;
    return CoverageLiteral{
        *exact,
  ...
  if (apostrophe != 0U) {
    ...
    if (parsed.ec != std::errc{}
        ...
        || declared_width == 0U || declared_width > 64U) {
      return std::nullopt;
    }
  }
  ...
  if (base_character == 'b' || base_character == 'B') {
    ...
    value &= mask;
    auto exact = static_cast<std::int64_t>(value);
    if (signed_value && width < 64U
        && (value & (std::uint64_t{1} << (width - 1U))) != 0U) {
      exact = static_cast<std::int64_t>(value | ~mask);
    }
    if (negative) exact = -exact;
    return CoverageLiteral{exact, value, mask, width, false};
  } else {
    return std::nullopt;
  }

  const auto natural_width = static_cast<std::uint64_t>(suffix.size())
      * digit_width;
  const auto width = declared_width == 0U
      ? natural_width : static_cast<std::uint64_t>(declared_width);
  if (width == 0U || width > 64U || natural_width > 64U) {
    return std::nullopt;
  }
  std::uint64_t value{};
  std::uint64_t mask{};
  ...
  for (const auto character : suffix) {
    ...
    value |= digit;
    mask |= (std::uint64_t{1} << digit_width) - 1U;
  }
  const auto width_mask = width == 64U
      ? std::numeric_limits<std::uint64_t>::max()
      : (std::uint64_t{1} << width) - 1U;
  value &= width_mask;
  mask &= width_mask;
  auto exact = static_cast<std::int64_t>(value);
  if (!wildcard && signed_value && width < 64U
      && (value & (std::uint64_t{1} << (width - 1U))) != 0U) {
    exact = static_cast<std::int64_t>(value | ~width_mask);
  }
  if (negative) exact = -exact;
  return CoverageLiteral{
      exact, value, mask, static_cast<std::uint32_t>(width), wildcard};
}
...
[[nodiscard]] std::optional<SystemVerilogCoverageBinValue> coverage_bin_value(
  ...
  if (!literal || (literal->wildcard && !wildcard_bin)) {
    return std::nullopt;
  }
  if (literal->wildcard) {
...
[[nodiscard]] std::optional<std::uint32_t> transition_bound(
  ...
  if (!value || value->wildcard || value->exact < 0
...
[[nodiscard]] bool structure_transition_repetition(
  ...
  for (std::size_t index = 0; index < tokens.size(); ++index) {
    ...
    if (tokens[index].kind == TokenKind::LeftBracket) ++brackets;
    if (tokens[index].kind == TokenKind::RightBracket) --brackets;
    if (tokens[index].kind == TokenKind::LeftBrace) ++braces;
    if (tokens[index].kind == TokenKind::RightBrace) --braces;
  }
  if (!repetition_left) return true;
  const auto right = matching_right_bracket(tokens, *repetition_left);
  if (!right || *right + 1U != tokens.size()) return false;
  ...
  if (count_tokens.empty()) {
    ...
    const auto minimum_tokens = colon
        ? count_tokens.first(*colon) : count_tokens;
    if (minimum_tokens.empty()) return false;
...
structure_transition_step(
  ...
  for (const auto& value_tokens : values) {
    if (value_tokens.empty()) return std::nullopt;
...
structure_transition_delay(
  ...
  if (tokens[position].kind == TokenKind::LeftBracket) {
    ...
    if (!right || *right == position + 1U) return std::nullopt;
    ...
    if (!minimum || !maximum || *maximum < *minimum) return std::nullopt;
...
structure_transition_sequence(
  ...
  if (!right || *right + 1U != tokens.size()) return std::nullopt;
  ...
  for (std::size_t position = 0; position < inner.size();) {
    ...
    if (kind == TokenKind::RightParen) --parentheses;
    if (kind == TokenKind::LeftBracket) ++brackets;
    if (kind == TokenKind::RightBracket) --brackets;
    if (kind == TokenKind::LeftBrace) ++braces;
    if (kind == TokenKind::RightBrace) --braces;
    ++position;
  }
  const auto final_step = structure_transition_step(
      std::vector<Token>{
          ...
          inner.end()},
      wildcard_bin);
  if (!final_step) return std::nullopt;
...
structure_transition_sequences(
  ...
  for (const auto& sequence_tokens : sequences) {
    if (sequence_tokens.empty()) return std::nullopt;
...
void VerilogParser::structure_covergroup_options(
  ...
  for (std::size_t index = 0; index <= body.size(); ++index) {
    ...
    const bool terminator =
        !at_end && kind == TokenKind::Semicolon
        && parentheses == 0 && brackets == 0 && braces == 0;
  ...
  for (const auto& option : declaration.option_assignments) {
    ...
    if (!literal || literal->wildcard) continue;
...
void VerilogParser::structure_coverpoint_bins(
  ...
  for (auto statement : split_top_level(body, TokenKind::Semicolon)) {
    ...
    if (keyword_index >= statement.size()) continue;
    ...
    if (!declarator.empty()) {
      ...
      if (!extent.empty()) {
        ...
        if (!literal || literal->wildcard || literal->exact <= 0
    ...
    if (find_top_level_keyword(rhs, "with")
        || find_top_level_keyword(rhs, "matches")) {
      error(
          rhs.front(),
          "FSIM-SV-SEM-220",
          "with/matches bin selection is not supported by the bounded coverage model");
      continue;
    }
    if (rhs.size() == 1U && rhs.front().text == "default") {
      ...
      if (!right || *right + 1U != rhs.size()) continue;
    ...
    if (!bin.transitions.empty()) {
      ...
      for (std::size_t array_index = 0; array_index < bin_count;
        ...
        const auto remaining_sequences =
            bin.transitions.size() - sequence_offset;
        const auto remaining_bins = bin_count - array_index;
        const auto sequence_count =
            (remaining_sequences + remaining_bins - 1U) / remaining_bins;
        auto expanded = bin;
        expanded.name =
            source_name + "[" + std::to_string(array_index) + "]";
    ...
    for (const auto& value : bin.values) {
      ...
      for (;;) {
        ...
        if (expanded_values.size() > maximum_expanded_bins) break;
        if (current == *value.range_right) break;
    ...
    for (std::size_t array_index = 0; array_index < bin_count;
      ...
      const auto value_count =
          (remaining_values + remaining_bins - 1U) / remaining_bins;
      auto expanded = bin;
  ...
  if (!saw_bin_declaration) {
    ...
    automatic.span = declaration.body_span.empty()
        ? declaration.expression_span : declaration.body_span;
...
void VerilogParser::structure_cross_bins(
  ...
  for (auto statement : split_top_level(body, TokenKind::Semicolon)) {
    ...
    const auto selection = std::span<const Token>{statement}.subspan(
        *assignment + 1U);
    if (find_top_level_keyword(selection, "with")
        || find_top_level_keyword(selection, "matches")) {
      error(
          selection.front(),
          "FSIM-SV-SEM-220",
          "with/matches cross selection is not supported by the bounded coverage model");
      continue;
    }
    SystemVerilogCoverageBin bin;
    bin.kind = keyword == "ignore_bins"
...
void VerilogParser::structure_coverage_options(
  ...
  for (const auto& statement : split_top_level(body, TokenKind::Semicolon)) {
    ...
    const bool valid_name = name == "weight" || name == "goal"
        || name == "at_least";
    const bool valid_value = literal && !literal->wildcard
        && literal->exact >= 0
  ...
  const auto apply_scope = [&](const SystemVerilogCovergroupOptionScope scope) {
    for (const auto& option : declaration.option_assignments) {
      if (option.scope != scope) continue;
      const auto literal = coverage_literal(option.value_tokens);
      if (!literal) continue;
      if (option.name == "weight") {
        declaration.effective_weight =
            static_cast<std::uint32_t>(literal->exact);
      } else if (option.name == "goal") {
        declaration.effective_goal =
            static_cast<std::uint32_t>(literal->exact);
      } else {
        declaration.effective_at_least =
            static_cast<std::uint64_t>(literal->exact);
      }
    }
  };
