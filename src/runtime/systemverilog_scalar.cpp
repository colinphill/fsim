...
SystemVerilogScalarKind promoted_kind(
  ...
  return left == SystemVerilogScalarKind::Time
      ...
      ? SystemVerilogScalarKind::Time : SystemVerilogScalarKind::None;
}
...
SystemVerilogScalarResult real_arithmetic(
  ...
  Number value{};
  switch (operation) {
  case SystemVerilogScalarArithmetic::Add: value = left + right; break;
  case SystemVerilogScalarArithmetic::Subtract: value = left - right; break;
  case SystemVerilogScalarArithmetic::Multiply: value = left * right; break;
  case SystemVerilogScalarArithmetic::Divide: value = left / right; break;
  }
  if (!std::isfinite(value)) return {{}, SystemVerilogScalarError::Nonfinite};
  return {
...
SystemVerilogScalarResult time_arithmetic(
  ...
  case SystemVerilogScalarArithmetic::Add:
    if (left > maximum - right) return {{}, SystemVerilogScalarError::Overflow};
    value = left + right;
    break;
  case SystemVerilogScalarArithmetic::Subtract:
    if (left < right) return {{}, SystemVerilogScalarError::Overflow};
    value = left - right;
    break;
  ...
  case SystemVerilogScalarArithmetic::Divide:
    if (right == 0) return {{}, SystemVerilogScalarError::DivideByZero};
    value = left / right;
    break;
  }
...
std::optional<double> SystemVerilogScalarValue::as_real() const noexcept {
  ...
  return kind == SystemVerilogScalarKind::Time
      ? std::optional<double>{static_cast<double>(bits)} : std::nullopt;
}

std::optional<std::uint64_t> SystemVerilogScalarValue::as_time() const noexcept {
  return kind == SystemVerilogScalarKind::Time
      ? std::optional<std::uint64_t>{bits} : std::nullopt;
}
...
SystemVerilogScalarValue::as_chandle() const noexcept {
  return kind == SystemVerilogScalarKind::Chandle
      ? std::optional<SystemVerilogChandle>{bits} : std::nullopt;
}
...
bool comparison_value(
  ...
  switch (operation) {
  case SystemVerilogScalarComparison::Equal: return left == right;
  case SystemVerilogScalarComparison::NotEqual: return left != right;
  case SystemVerilogScalarComparison::Less: return left < right;
  case SystemVerilogScalarComparison::LessEqual: return left <= right;
  case SystemVerilogScalarComparison::Greater: return left > right;
  case SystemVerilogScalarComparison::GreaterEqual: return left >= right;
  }
  return false;
}
...
double rounded_value(
  ...
  case SystemVerilogScalarRounding::TowardZero: return std::trunc(value);
  case SystemVerilogScalarRounding::Floor: return std::floor(value);
  case SystemVerilogScalarRounding::Ceil: return std::ceil(value);
  }
  return value;
}
  ...
  if (integral_left && integral_right) {
    if (left.kind == right.kind) {
      return left.kind == SystemVerilogScalarKind::Time
          ...
          : SystemVerilogScalarPredicate{
                comparison_value(
                    operation, *left.as_integral(), *right.as_integral()), {}};
    }
    const auto signed_value = left.kind == SystemVerilogScalarKind::None
        ? *left.as_integral() : *right.as_integral();
    ...
    switch (operation) {
    case SystemVerilogScalarComparison::Equal: return {equal, {}};
    case SystemVerilogScalarComparison::NotEqual: return {!equal, {}};
    case SystemVerilogScalarComparison::Less: return {less, {}};
    case SystemVerilogScalarComparison::LessEqual: return {less || equal, {}};
    case SystemVerilogScalarComparison::Greater: return {!less && !equal, {}};
    case SystemVerilogScalarComparison::GreaterEqual: return {!less, {}};
    }
  }
...
SystemVerilogScalarPredicate systemverilog_scalar_truth(
  ...
  if (value.kind == SystemVerilogScalarKind::Time) return {value.bits != 0, {}};
  if (const auto integral = value.as_integral()) return {*integral != 0, {}};
...
SystemVerilogScalarResult convert_systemverilog_scalar(
  ...
  if (target == SystemVerilogScalarKind::None) {
    if (const auto integral = value.as_integral()) return {value, {}};
    ...
    if (const auto ticks = value.as_time()) return {value, {}};
...
SystemVerilogScalarResult systemverilog_scalar_from_packed(
  ...
  if (word.bval != 0) return {{}, SystemVerilogScalarError::UnknownValue};
  ...
  return convert_systemverilog_scalar(
      SystemVerilogScalarValue::integral(
          std::bit_cast<std::int64_t>(extended)), target);
}
...
SystemVerilogPackedScalarResult systemverilog_scalar_to_packed(
  ...
  const auto converted = convert_systemverilog_scalar(
      value, is_signed ? SystemVerilogScalarKind::None
                       : SystemVerilogScalarKind::Time,
      rounding);
  if (!converted) return {PackedLogic4{}, converted.error};
  std::uint64_t bits{};
  if (is_signed) {
...
SystemVerilogPackedScalarResult encode_systemverilog_scalar_payload(
  ...
  return width == 0
      ...
      : SystemVerilogPackedScalarResult{
            PackedLogic4::from_aval_bval(width, value.bits, 0), {}};
}
...
SystemVerilogPackedScalarResult systemverilog_scalar_binary_payload(
  ...
  if (!lhs) return {PackedLogic4{}, lhs.error};
  const auto rhs = decode_systemverilog_scalar_payload(right, right_kind);
  if (!rhs) return {PackedLogic4{}, rhs.error};
  ...
  if (!result) return {PackedLogic4{}, result.error};
...
SystemVerilogScalarError SystemVerilogScalarStorage::value_error(
  ...
  if (!real_kind(value.kind)) return SystemVerilogScalarError::InvalidKind;
  const auto number = value.as_real();
  return number && std::isfinite(*number)
      ? SystemVerilogScalarError::None : SystemVerilogScalarError::Nonfinite;
}
...
std::optional<SystemVerilogScalarValue> SystemVerilogScalarStorage::load(
  ...
  return id < values_.size()
      ? std::optional<SystemVerilogScalarValue>{values_[id]} : std::nullopt;
}

SystemVerilogScalarError SystemVerilogScalarStorage::store(
    const SystemVerilogScalarId id,
    const SystemVerilogScalarValue value) noexcept {
  if (id >= values_.size()) return SystemVerilogScalarError::InvalidId;
  if (const auto error = value_error(value);
...
SystemVerilogScalarMaterialization SystemVerilogScalarStorage::arithmetic(
  ...
  if (!result) return {0, result.error};
