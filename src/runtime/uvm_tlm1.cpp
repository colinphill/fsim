...
SystemVerilogUvmTlm1Error::SystemVerilogUvmTlm1Error(std::string code,
                                                     std::string message)
...
SystemVerilogUvmTlm1Service::SystemVerilogUvmTlm1Service(
  ...
  if (limits_.maximum_endpoints == 0 || limits_.maximum_connections == 0 ||
      limits_.maximum_endpoints_per_component == 0 ||
      limits_.maximum_name_bytes == 0 || limits_.maximum_profile_bytes == 0 ||
      limits_.maximum_fanout == 0 || limits_.maximum_depth == 0 ||
      limits_.maximum_resolution_work == 0 || limits_.maximum_mutations == 0 ||
      limits_.maximum_fifos == 0 || limits_.maximum_fifo_capacity == 0 ||
...
SystemVerilogUvmTlm1Service::SystemVerilogUvmTlm1Service(
    SystemVerilogClassHeap &heap, SystemVerilogUvmComponentService &components,
    SystemVerilogUvmTlm1Limits limits)
...
SystemVerilogUvmTlm1Service::~SystemVerilogUvmTlm1Service() {
  cancel_all();
  if (phases_)
    phases_->clear_tlm1_service(*this);
}
...
SystemVerilogUvmTlm1Service::register_endpoint(
  ...
  for (const auto &[slot, existing] : endpoints_) {
    (void)slot;
    if (existing.descriptor.component != descriptor.component)
      continue;
    ++component_endpoints;
  ...
  ++revision_;
  if (activity_) {
    activity_->publish({SystemVerilogUvmActivityKind::Connection,
                        SystemVerilogUvmActivityAction::Created,
                        endpoints_.at(slot).debug_name, "tlm1-endpoint",
                        endpoints_.at(slot).root});
  }
  return handle(slot);
}

void SystemVerilogUvmTlm1Service::release_endpoint(
    const SystemVerilogUvmTlm1EndpointHandle endpoint_handle) {
  Endpoint &selected = endpoint(endpoint_handle);
  require_live(selected);
  if (mutations_ >= limits_.maximum_mutations) {
    fail(kResourceLimit, "UVM TLM1 endpoint-release mutation ceiling exceeded");
  }
  const auto operation_uses_endpoint = [&](const auto &entry) {
    const auto &snapshot = entry.second.value;
    return snapshot.source == endpoint_handle ||
           snapshot.implementation == endpoint_handle;
  };
  if (std::ranges::any_of(operations_, operation_uses_endpoint)) {
    fail(kInvalidEndpoint, "UVM TLM1 endpoint has a retained operation");
  }
  const auto slot = selected.slot;
  const auto root = selected.root;
  const auto debug_name = selected.debug_name;
  const auto removed_connections =
      selected.outbound.size() + selected.inbound.size();
  if (removed_connections > connections_) {
    fail(kInvalidEndpoint, "UVM TLM1 connection accounting is inconsistent");
  }
  for (const auto target : selected.outbound) {
    auto &inbound = endpoints_.at(target).inbound;
    std::erase(inbound, slot);
  }
  for (const auto source : selected.inbound) {
    auto &outbound = endpoints_.at(source).outbound;
    std::erase(outbound, slot);
  }
  connections_ -= removed_connections;
  fifos_.erase(slot);
  transport_handlers_.erase(slot);
  analysis_subscribers_.erase(slot);
  endpoints_.erase(slot);
  ++mutations_;
  ++revision_;
  bound_revision_ = 0;
  if (activity_) {
    activity_->publish({SystemVerilogUvmActivityKind::Connection,
                        SystemVerilogUvmActivityAction::Cancelled, debug_name,
                        "tlm1-endpoint", root});
  }
}
...
void SystemVerilogUvmTlm1Service::connect(
  ...
  if (activity_) {
    activity_->publish({SystemVerilogUvmActivityKind::Connection,
                        ...
                        source.debug_name, target.debug_name, source.root,
                        connections_});
  }
}
...
void SystemVerilogUvmTlm1Service::resolve_all() {
  ...
  if (activity_) {
    activity_->publish({SystemVerilogUvmActivityKind::Connection,
                        SystemVerilogUvmActivityAction::Bound, "tlm1",
                        "resolved", 0, connections_});
  }
}

bool SystemVerilogUvmTlm1Service::contains(
    const SystemVerilogUvmTlm1EndpointHandle endpoint_handle) const noexcept {
  if (!endpoint_handle.valid() || endpoint_handle.owner_.get() != owner_.get())
    return false;
...
SystemVerilogUvmTlm1EndpointSnapshot SystemVerilogUvmTlm1Service::snapshot(
  ...
  for (const auto slot : value.outbound)
    result.outbound.push_back(handle(slot));
  for (const auto slot : value.inbound)
    result.inbound.push_back(handle(slot));
  if (binding_valid())
...
SystemVerilogUvmTlm1Service::Endpoint &SystemVerilogUvmTlm1Service::endpoint(
    const SystemVerilogUvmTlm1EndpointHandle endpoint_handle) {
...
SystemVerilogUvmTlm1Service::handle(const std::uint64_t slot) const {
  return SystemVerilogUvmTlm1EndpointHandle{owner_, slot,
                                            endpoints_.at(slot).generation};
}

void SystemVerilogUvmTlm1Service::require_live(const Endpoint &value) const {
...
bool SystemVerilogUvmTlm1Service::reaches(const std::uint64_t from,
...
void SystemVerilogUvmTlm1Service::collect_implementations(
    const std::uint64_t slot, const std::size_t depth, std::size_t &work,
    std::vector<SystemVerilogUvmTlm1EndpointHandle> &result,
