...
namespace {
        ...
        SystemVerilogDpiCompositeLayout& layout)
    {
                ...
                if (descriptor.enum_values.empty()
                    || std::ranges::any_of(
                        ...
                        [&](const PackedLogic4& value) {
                            if (value.width() != descriptor.width || value.is_logic9()) {
                                return true;
                            }
                            if (descriptor.scalar_domain
                                    == SystemVerilogDpiScalarDomain::Bit2
                                && std::ranges::any_of(
                                    value.bval_words(), [](const std::uint64_t word) {
                                        return word != 0U;
                                    })) {
                                return true;
                            }
                            return !identities.insert(value.to_msb_string()).second;
                        })) {
                    return SystemVerilogDpiMarshallingError::InvalidDescriptor;
                }
            }
        ...
        if (descriptor.kind == SystemVerilogDpiCompositeKind::FixedArray) {
            ...
            if (error != SystemVerilogDpiMarshallingError::None)
                return error;
            if (!checked_multiply(
    ...
    const SystemVerilogDpiTransferMode mode)
{
    ...
    for (std::size_t index { }; index < values.size(); ++index) {
        ...
        if (leaf.kind == SystemVerilogDpiCompositeKind::Enum) {
            if (std::ranges::find(leaf.enum_values, values[index])
    ...
    const SystemVerilogDpiTransferMode expected_mode)
{
    ...
    for (std::size_t index { }; index < value.leaves.size(); ++index) {
        ...
        if (leaf_descriptors[index]->kind
            ...
            if (std::ranges::find(
                    leaf_descriptors[index]->enum_values, decoded.value)
