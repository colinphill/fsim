...
std::uint64_t scale_for(
  ...
  const auto difference =
      static_cast<int>(profile.unit_exponent)
...
SystemVerilogVpiTimeService::SystemVerilogVpiTimeService(
  ...
  impl_->owner =
      next_time_service_owner.fetch_add(1, std::memory_order_relaxed);
...
SystemVerilogVpiTimeService::convert_delay(
  ...
  const auto scaled =
      static_cast<long double>(delay.scaled_real)
...
SystemVerilogVpiTimeQueryResult SystemVerilogVpiTimeService::query(
  ...
  if (format == SystemVerilogVpiTimeFormat::IntegerTicks) {
    result.value.high =
        static_cast<std::uint32_t>(result.ticks >> 32U);
    result.value.low = static_cast<std::uint32_t>(
        result.ticks & 0xffffffffULL);
  } else {
    result.value.scaled_real =
    ...
    position->second.scheduler_handle =
        impl_->scheduler->schedule_after_cancelable(
            ...
            [weak, id](Scheduler&) {
              ...
              query.value.high =
                  static_cast<std::uint32_t>(query.ticks >> 32U);
