...
constexpr std::size_t maximum_bits = 1U << 20U;
...
bool valid_scalar(
  ...
  switch (category) {
    ...
    case SystemVerilogVpiValueCategory::Integer4:
      return width != 0U && width <= maximum_bits;
  ...
  switch (descriptor.kind) {
      ...
      if (descriptor.is_signed) {
        if (descriptor.width == 64U) {
          ...
          const auto magnitude =
              std::int64_t{1} << (descriptor.width - 1U);
          minimum = -magnitude;
          maximum = magnitude - 1;
        }
      } else {
    ...
    case SystemVerilogVpiDescriptorKind::UnpackedArray: {
      ...
      for (const auto& range : descriptor.ranges) {
        ...
        if (range_size > maximum_elements
            || !checked_multiply(
      ...
      if (layout.fixed_bits > maximum_bits) {
        return SystemVerilogVpiDescriptorError::BitLimit;
      }
      if (element.dynamic) {
    ...
    case SystemVerilogVpiDescriptorKind::Class: {
      const bool class_type =
          descriptor.kind == SystemVerilogVpiDescriptorKind::Class;
      if (descriptor.category != SystemVerilogVpiValueCategory::None
          || descriptor.width != 0U || descriptor.is_signed
      ...
      for (const auto& child_descriptor : descriptor.children) {
        ...
        if (!class_type && !child.dynamic) {
          if (descriptor.kind == SystemVerilogVpiDescriptorKind::Struct) {
            ...
            layout.fixed_bits =
                std::max(layout.fixed_bits, child.fixed_bits);
          }
        }
      }
      if (class_type) {
      ...
      if (layout.fixed_element_count > maximum_elements) {
        return SystemVerilogVpiDescriptorError::ElementLimit;
      }
      if (layout.fixed_bits > maximum_bits) {
        return SystemVerilogVpiDescriptorError::BitLimit;
