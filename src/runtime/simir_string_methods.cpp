...
std::int32_t string_compare(
  ...
  if (!case_insensitive) return systemverilog_string_compare(lhs, rhs);
  ...
  return left_points.size() == right_points.size()
      ? 0 : left_points.size() < right_points.size() ? -1 : 1;
}
...
void string_putc(
  ...
  if (!index || !character || *index < 0) return;
  if (static_cast<std::size_t>(*index)
      >= systemverilog_string_length(value)) return;
...
void Interpreter::Impl::execute_string(
  ...
  if (result.scalar_bits) {
    get_register(process, operation.destination) =
        PackedLogic4::from_aval_bval(64, *result.scalar_bits, 0);
  }
  if (result.string) {
