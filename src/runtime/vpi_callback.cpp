...
SystemVerilogVpiCallbackRegistrationResult registration_failure(
    ...
    const SystemVerilogVpiTimeError time_error =
...
SystemVerilogVpiCallbackEvent make_event(
    ...
    std::optional<SystemVerilogVpiTimeQueryResult> supplied_time =
        std::nullopt) {
  ...
  if (registration.kind
    ...
    if (registration.delay) {
      const auto converted =
          impl_->time_service->convert_delay(*registration.delay);
      if (!converted) {
        return registration_failure(
