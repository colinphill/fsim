...
namespace {
    ...
    static_assert(sizeof(fsim_jit_runtime_v1) == 600);
    ...
    private:
        ...
        valid_native_object(const std::span<const std::byte> bytes) const
        {
            ...
            return (*parsed)->isRelocatableObject() && (*parsed)->getBytesInAddress() == sizeof(void*) && (*parsed)->getArch() == target_triple_.getArch() && (*parsed)->getTripleObjectFormat() == target_triple_.getObjectFormat();
        }
    ...
    if (!options.cache_directory.empty()) {
        ...
        builder.setCompileFunctionCreator(
            [object_cache](llvm::orc::JITTargetMachineBuilder machine_builder)
                ...
                std::unique_ptr<llvm::orc::IRCompileLayer::IRCompiler> compiler = std::make_unique<llvm::orc::TMOwningSimpleCompiler>(
    ...
    const std::span<const ValueKind> signal_value_kinds)
{
    ...
    for (const auto& entry : entries) {
        ...
        if (impl_->symbols.contains(owned_symbol) || !module_symbols.insert(owned_symbol).second) {
            throw LlvmJitError(
    ...
    if (impl_->module_identities.contains(owned_module_identity)) {
        throw LlvmJitError(
            "duplicate LLVM process module identity '" + owned_module_identity + "'");
    }
    ...
    if (auto message = verify_error(*module); !message.empty()) {
        throw LlvmJitError(
            "generated invalid LLVM IR for module '" + owned_module_identity + "': " + message);
    }
    ...
    if (auto message = verify_error(*module); !message.empty()) {
        throw LlvmJitError(
            "LLVM optimization produced invalid IR for module '" + owned_module_identity + "': " + message);
    }

    if (auto error = impl_->jit->addIRModule(llvm::orc::ThreadSafeModule(
            std::move(module), std::move(context)))) {
        throw LlvmJitError(
            "cannot add LLVM process module '" + owned_module_identity + "': " + llvm_error(std::move(error)));
    }
    ...
    fsim_jit_resume_result_v1& result) const
{
    ...
    if (entry.info.uses_containers) {
        if (runtime.struct_size
                < offsetof(fsim_jit_runtime_v1, force_signal_slice)
    ...
    const fsim_jit_runtime_v1& runtime) const
{
    ...
    std::vector<std::uint64_t> register_aval(
        found->second.info.frame_layout.register_word_count);
