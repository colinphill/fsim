...
std::optional<std::uint64_t> packed_index_offset(
  ...
  if (index < low || index > high) return std::nullopt;
...
std::optional<VerilogSpecifyTerminalInfo> specify_terminal_selection(
  ...
  if (expression.text == "+:" || expression.text == "-:") {
    if (*second <= 0) return std::nullopt;
  ...
  if (!left_offset || !right_offset) return std::nullopt;
...
void HierarchyBuilder::validate_verilog_specify(
    ...
    if (unit.verilog_specify_blocks.empty()) return;
    ...
    const auto declaration = [&](const std::string_view name)
      ...
      if (port != unit.ports.end()) return &*port;
    ...
    const auto resolve = [&](const frontend::Expression& expression,
      ...
      const bool input =
          declared->direction == frontend::PortDirection::Input
          || declared->direction == frontend::PortDirection::Inout;
      const bool output =
          declared->direction == frontend::PortDirection::Output
          || declared->direction == frontend::PortDirection::Inout;
    ...
    for (const auto& block : unit.verilog_specify_blocks) {
      ...
      for (const auto& module_path : block.module_paths) {
        ...
        normalized.destination_data_source =
        ...
        if (normalized.sources.size() == module_path.sources.size()
          ...
          if (module_path.conditional || module_path.ifnone) {
            const auto prior = std::ranges::find_if(
                std::span{
                    design_.verilog_specify_paths_}.subspan(
                        block_path_begin),
      ...
      for (const auto& pulse : block.pulse_declarations) {
        ...
        for (const auto& terminal : pulse.terminals) {
          if (const auto selected =
                  resolve(terminal, "pulse terminal", false, true)) {
      ...
      for (const auto& check : block.timing_checks) {
        ...
        const auto kind = [](const frontend::VerilogTimingCheckKind value) {
          ...
          switch (value) {
            case Frontend::Setup: return Runtime::setup;
            case Frontend::Hold: return Runtime::hold;
            case Frontend::SetupHold: return Runtime::setuphold;
            case Frontend::Recovery: return Runtime::recovery;
            case Frontend::Removal: return Runtime::removal;
            case Frontend::RecRem: return Runtime::recrem;
            case Frontend::Skew: return Runtime::skew;
            case Frontend::TimeSkew: return Runtime::timeskew;
            case Frontend::FullSkew: return Runtime::fullskew;
            case Frontend::Period: return Runtime::period;
            case Frontend::Width: return Runtime::width;
            case Frontend::NoChange: return Runtime::nochange;
          }
