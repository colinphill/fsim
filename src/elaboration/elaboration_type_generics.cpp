...
namespace {
        ...
        const std::vector<frontend::FunctionDeclaration>& visible)
    {
        ...
        while (!pending.empty()) {
            ...
            for (const auto& candidate : visible) {
                const bool called = std::ranges::any_of(
                                        caller->variables,
        ...
        std::vector<Diagnostic>& diagnostics)
    {
        if (derived.integer_range_expression) {
            ...
            base.integer_base_range_expression = base.integer_range_expression;
            base.integer_range = derived.integer_range;
            base.integer_range_expression = derived.integer_range_expression;
        }
        if (derived.discrete_range_expression) {
            ...
            if (base.domain == frontend::ValueDomain::Integer) {
                ...
                base.integer_range_expression = frontend::IntegerRangeExpression {
                    range.left,
                    range.right,
                    range.span,
                    range.descending
                ...
                base.enumeration_base_range = base.enumeration_range;
                base.enumeration_base_range_expression = base.enumeration_range_expression;
        ...
        if (base.vhdl_array
            ...
            if (derived.vhdl_array_constraints.size()
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHARRAY-008",
            ...
            for (std::size_t index = 0;
                ...
                if (!dimension.unconstrained
                    ...
                    || dimension.range) {
                    diagnostics.push_back({ "FSIM-ELAB-VHSUBTYPE-004",
                        "a constrained VHDL array dimension cannot be "
                        "constrained again",
                        derived.vhdl_array_constraints[index].span });
                    return std::nullopt;
                }
                dimension.constraint = derived.vhdl_array_constraints[index];
            ...
            if (array.dimensions.size() == 1) {
                const auto& constraint = derived.vhdl_array_constraints.front();
                base.packed_range_expression = frontend::PackedRangeExpression {
                    constraint.left,
                    constraint.right,
                    constraint.span,
            ...
            if (!is_packed_array_type(base)
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHSUBTYPE-003",
            ...
            base.packed_range_expression = derived.packed_range_expression;
            if (base.vhdl_array) {
        ...
        std::vector<Diagnostic>& diagnostics)
    {
        if (!actual.type_value) {
            ...
            derived.packed_range_expression = frontend::PackedRangeExpression {
    ...
    for (const auto& actual : overrides) {
        ...
        if (actual.name) {
            ...
            if (found == formals.end()) {
                diagnostics.push_back({ "FSIM-ELAB-GENERIC-001",
            ...
            if (saw_named) {
                diagnostics.push_back({ "FSIM-ELAB-GENERIC-003",
        ...
        if (actuals[*index]) {
            diagnostics.push_back({ "FSIM-ELAB-GENERIC-002",
    ...
    for (std::size_t index = 0; index < formals.size(); ++index) {
        ...
        if (formal.kind
            ...
            if (!formal.function_profile) {
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-001",
            ...
            if (association_language
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-002",
                    "interface function generic '" + formal.name
            ...
            if (actuals[index]) {
                ...
                if (actuals[index]->type_value
                    ...
                    diagnostics.push_back({ "FSIM-ELAB-VHFUNC-003",
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-004",
                    "interface function generic '" + formal.name
            ...
            if (matches.empty()) {
                diagnostics.push_back({ actual_name
            ...
            if (matches.size() != 1) {
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-006",
            ...
            if (!selected.defined) {
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-007",
                    "function actual '" + selected.name
            ...
            if (!selected.pure) {
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-008",
                    "function actual '" + selected.name
            ...
            if (std::ranges::any_of(
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHFUNC-009",
        ...
        if (formal.kind
            ...
            if (!formal.procedure_profile) {
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-001",
            ...
            if (association_language
                != frontend::Language::Vhdl2008) {
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-002",
                    "interface procedure generic '" + formal.name
            ...
            if (actuals[index]) {
                ...
                if (actuals[index]->type_value
                    || actuals[index]->value.kind
                        != frontend::ExpressionKind::Identifier
                    || !actuals[index]->value.operands.empty()) {
                    diagnostics.push_back({ "FSIM-ELAB-VHPROC-003",
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-004",
                    "interface procedure generic '" + formal.name
            ...
            if (matches.empty()) {
                const bool names_function = actual_name
                    && std::ranges::any_of(
                        parent_functions,
                        [&](const auto& function) {
                            return function.name == *actual_name;
                        });
                const bool generated_or_scoped = actual_name
                    && actual_name->find('.')
                        != std::string::npos;
                diagnostics.push_back({ names_function
            ...
            if (matches.size() != 1) {
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-006",
            ...
            if (!selected.defined) {
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-007",
                    "procedure actual '" + selected.name
                        + "' has no executable body",
                    selected.span });
                continue;
            ...
            if (procedure_has_timing_or_signal_update(
                    selected.statements)) {
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-009",
                    "procedure actual '" + selected.name
            ...
            if (std::ranges::any_of(
                ...
                diagnostics.push_back({ "FSIM-ELAB-VHPROC-011",
        ...
        if (formal.kind
            ...
            diagnostics.push_back({ "FSIM-ELAB-VHPKG-001",
        ...
        if (!actuals[index]) {
            diagnostics.push_back({ "FSIM-ELAB-GENTYPE-001",
        ...
        if (association_language
            != frontend::Language::Vhdl2008) {
            diagnostics.push_back({ "FSIM-ELAB-GENTYPE-004",
                "interface type generic '" + formal.name
        ...
        if (!actual_type
            ...
            const bool syntactic_subtype = actuals[index]->type_value.has_value()
                || (expression.kind
                        == frontend::ExpressionKind::Identifier
                    && expression.operands.empty())
                || (expression.kind
                    ...
                    && expression.operands.front().kind
                        == frontend::ExpressionKind::Identifier);
            diagnostics.push_back({ syntactic_subtype
