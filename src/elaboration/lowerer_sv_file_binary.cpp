    ...
    const Expression& expression)
{
    ...
    if (const auto local = container_locals_.find(target.text);
        ...
        const auto width = runtime::simir::container_packed_element_width(type);
        if (!type.fixed || !width
        ...
        const auto type = frontend_type != nullptr
            ? container_type(*frontend_type, target.span)
            : std::nullopt;
        ...
        operation.width = static_cast<std::uint32_t>(width);
        operation.two_state = is_two_state_domain(register_domain(packed_local->second));
