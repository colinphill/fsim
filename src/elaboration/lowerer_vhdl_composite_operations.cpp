...
[[nodiscard]] std::optional<frontend::Type> bounded_array_context(
  ...
  dimension.null = packed.descending
      ? packed.left < packed.right : packed.left > packed.right;
  dimension.stride = 1;
...
Lowerer::ExpressionAttempt Lowerer::lower_vhdl_composite_expression(
  ...
  if (expression.text == "&") {
    ...
    std::optional<frontend::Type> contextual_target =
        bounded_array_context(expected_type);
    std::optional<frontend::Type> inferred_target;
    const frontend::Type* target =
        expected_type != nullptr && expected_type->vhdl_array
        ? expected_type
        : contextual_target ? &*contextual_target : nullptr;
    if (target == nullptr) {
    ...
    for (const auto* operand_pointer : concatenands) {
      ...
      if (operand_width > 64 - result_width) {
        report(
            "FSIM-ELAB-VHCOMPOP-003",
            "VHDL concatenation result exceeds the 64-bit executable "
            "limit",
            expression.span);
        return std::nullopt;
      }
  ...
  const auto lhs_width = lhs_storage
      ? lhs_storage->width() : context->width();
  const auto rhs_width = rhs_storage
      ? rhs_storage->width() : context->width();
  if (!lhs_width || !rhs_width || *lhs_width > 64 || *rhs_width > 64) {
    report(
        "FSIM-ELAB-VHCOMPOP-001",
