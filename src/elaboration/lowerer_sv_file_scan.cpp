...
[[nodiscard]] InputScanFormat runtime_format(
  ...
  switch (format) {
  case frontend::InputScanFormat::Binary: return InputScanFormat::binary;
  case frontend::InputScanFormat::Octal: return InputScanFormat::octal;
  case frontend::InputScanFormat::Decimal: return InputScanFormat::decimal;
...
Lowerer::ExpressionAttempt Lowerer::lower_file_scan(
  ...
  if (string_source) {
    ...
    const auto* type = source.kind == ExpressionKind::Identifier
        ? object_type(source.text) : nullptr;
  ...
  for (const auto& parsed_conversion : parsed.conversions) {
    ...
    if (const auto local = string_locals_.find(target.text);
      ...
      if (width == 0 || width > 64) {
        report(
            "FSIM-ELAB-SVFILE-012",
            "packed scan targets must have a width from 1 through 64",
            target.span);
        return std::nullopt;
      }
      conversion.target = {
      ...
      if (width == 0 || width > 64 || type == nullptr) {
        report(
            "FSIM-ELAB-SVFILE-012",
            "packed scan targets must have a typed width from 1 through 64",
            target.span);
        return std::nullopt;
      }
      conversion.target = {
