...
void append_generated_body(
    ...
    for (auto& declaration : body.systemverilog_classes) {
        unit.systemverilog_classes.push_back(std::move(declaration));
    }
    for (auto& procedure : body.procedures) {
...
void expand_generate_regions(
    ...
    for (auto& generate : generates) {
        ...
        if (generate.kind == frontend::GenerateKind::Iterative) {
            ...
            std::size_t count = 0;
            std::unordered_set<std::int64_t> visited_values;
            while (true) {
                ...
                if (!visited_values.insert(value).second) {
                    diagnostics.push_back({
                        "FSIM-ELAB-GEN-014",
                        "loop-generate iteration revisits genvar value '"
                            + std::to_string(value) + "'",
