...
Lowerer::lower_unpacked_aggregate_member_read(
  ...
  const auto reference =
      unpacked_aggregate_member_reference(expression);
  if (!reference) {
    report(
  ...
  const auto destination =
      allocate_register(*width, reference->leaf_type->domain);
...
bool Lowerer::lower_unpacked_aggregate_pattern_value(
  ...
  if (expression.kind == ExpressionKind::Aggregate
    ...
    for (std::size_t item = 0;
      ...
      const auto& choices =
          expression.aggregate_choice_expressions[item];
      if (choice.empty()) {
        positional = true;
        if (!choices.empty()) {
          report(
    ...
    if (positional) {
      const auto expected = union_value
          ? std::size_t{1} : aggregate_type.packed_members.size();
...
Lowerer::lower_unpacked_aggregate_pattern(
    ...
    lower_dimension =
        [&](const Expression& pattern,
          ...
          for (std::size_t item = 0;
            ...
            const auto& choices =
                pattern.aggregate_choice_expressions[item];
          ...
          for (std::uint64_t ordinal = 0;
            ...
            const Expression* value = positional
                ? &pattern.operands[ordinal] : default_value;
  ...
  for (std::size_t item = 0;
    ...
    const auto converted =
        static_cast<std::uint64_t>(*key_value) & mask;
...
bool Lowerer::lower_unpacked_aggregate_assignment(
  ...
  if (statement.target.text.starts_with(member_index_prefix)) {
    const auto reference =
        unpacked_aggregate_member_reference(statement.target);
    const auto width = reference && reference->leaf_type
        ? reference->leaf_type->width() : std::nullopt;
