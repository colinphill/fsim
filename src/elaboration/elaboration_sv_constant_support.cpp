...
void normalize(Value& value) noexcept
{
    ...
    const auto aval = value.bits | (value.unknown_bits & ~value.high_impedance_bits);
...
{
    const bool logic9 = destination.packed.is_logic9() || operand.packed.is_logic9();
    ...
    const auto retained = destination.width > operand.width
        ? destination.width - operand.width
        : 0U;
    ...
    std::string& error)
{
    ...
    if (!explicit_width && digits.size() > maximum_constant_width) {
        error = "SystemVerilog decimal literal exceeds the 16,777,216-bit "
                "resource limit";
        return std::nullopt;
    }
    ...
    if (word_capacity != 0U
        ...
        error = "SystemVerilog decimal literal exceeds the constant-evaluation "
                "work limit";
        return std::nullopt;
    }
    ...
    for (const char character : digits) {
        ...
        for (std::size_t index = 0; index < processed_words; ++index) {
            ...
            const auto low_product = (word & 0xffffffffU) * 10U + carry;
            const auto high_product = (word >> 32U) * 10U + (low_product >> 32U);
            words[index] = (high_product << 32U)
                | (low_product & 0xffffffffU);
            carry = high_product >> 32U;
        }
        if (!explicit_width && carry != 0U) {
            if (used_words == word_capacity) {
                error = "SystemVerilog decimal literal exceeds the "
    ...
    const auto magnitude_width = words[used_words - 1U] == 0U
        ...
        : static_cast<std::uint32_t>(
              (used_words - 1U) * 64U
              + 64U - static_cast<std::size_t>(std::countl_zero(words[used_words - 1U])));
    ...
    std::string& error)
{
    ...
    const auto width_text = std::string_view { expression.text }.substr(0, quote);
    auto suffix = std::string_view { expression.text }.substr(quote + 1U);
    ...
    if (!width_text.empty()) {
        ...
        if (!explicit_width || *explicit_width == 0
            ...
            error = "SystemVerilog constant width exceeds the 16,777,216-bit "
                    "resource limit";
            return std::nullopt;
        }
    }
    ...
    if (digits.size()
        ...
        error = "SystemVerilog based literal exceeds the 16,777,216-bit "
                "resource limit";
        return std::nullopt;
    }
    const auto raw_width = static_cast<std::uint64_t>(digits.size()) * digit_width;
    const auto selected_width = explicit_width.value_or(
    ...
    std::string& error)
{
    ...
    const auto result_domain = lhs.domain == rhs.domain
        ? lhs.domain
        : frontend::ValueDomain::Logic4;
    ...
    if (width > maximum_constant_work_units / width) {
        error = "arbitrary-width multiplicative constant evaluation exceeds "
                "the work limit";
        return std::nullopt;
    }
    const auto multiplication_work = static_cast<std::uint64_t>(width) * width;
    if (operation == "*") {
        return make_result(
        ...
        while (remaining != 0U) {
            if ((remaining & 1U) != 0U) {
                if (work > maximum_constant_work_units - multiplication_work) {
                    error = "arbitrary-width power constant evaluation exceeds "
                            "the work limit";
                    return std::nullopt;
                }
    ...
    const bool negative_left = signed_result && packed_one(dividend, width - 1U);
    const bool negative_right = signed_result && packed_one(divisor, width - 1U);
    if (negative_left && negative_right
        && packed_is_signed_minimum(dividend, width)
        && packed_is_all_ones(divisor, width)) {
        error = "constant division overflows the signed destination width";
        return std::nullopt;
    }
    if (negative_left) {
    ...
    const frontend::SourceSpan& source)
{
    ...
    for (std::uint32_t bit = 0; bit < width; ++bit) {
        ...
        const bool lhs_known = lhs_state == Logic4::zero || lhs_state == Logic4::one;
        const bool rhs_known = rhs_state == Logic4::zero || rhs_state == Logic4::one;
        ...
        if (operation == "&") {
            state = (!lhs_one && lhs_known) || (!rhs_one && rhs_known)
                ? Logic4::zero
                : lhs_one && rhs_one ? Logic4::one
                                     : Logic4::x;
        } else if (operation == "|") {
            state = lhs_one || rhs_one
                ? Logic4::one
                : lhs_known && rhs_known ? Logic4::zero
                                         : Logic4::x;
        } else {
    ...
    std::string& error)
{
    ...
    if (expression.text == "&&" || expression.text == "||") {
        ...
        const auto result = expression.text == "&&"
    ...
    if (expression.text == "==" || expression.text == "!="
        || expression.text == "=" || expression.text == "/="
        || expression.text == "===" || expression.text == "!==") {
        const bool case_equality = expression.text == "===" || expression.text == "!==";
    ...
    const bool arithmetic_operation = expression.text == "+" || expression.text == "-"
        || expression.text == "*" || expression.text == "/"
        || expression.text == "%" || expression.text == "**";
    ...
    if (expression.text == "<" || expression.text == "<="
        ...
        const bool result = expression.text == "<" ? comparison < 0
    ...
    if (expression.text == "<<" || expression.text == "<<<"
        ...
        const bool arithmetic_right = expression.text == ">>>" && result.is_signed;
        ...
        if (arithmetic_right) {
            const auto first_extension = count >= result.width
                ? 0U
                : result.width - static_cast<std::uint32_t>(count);
    ...
    Value result = make_known(0, width, common_signed, false, expression.span);
    if (expression.text == "+") {
        ...
        if (!exponent) {
            error = "constant exponent is outside the bounded signed 64-bit "
                    "range";
            return std::nullopt;
        }
        if (*exponent < 0) {
        ...
        if (common_signed) {
            ...
            result.bits = expression.text == "/"
                ? lhs.bits / rhs.bits
                : lhs.bits % rhs.bits;
        }
    } else {
        error = "unsupported SystemVerilog binary constant operator '"
            + expression.text + "'";
        return std::nullopt;
    }
    normalize(result);
