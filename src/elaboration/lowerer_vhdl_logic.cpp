    ...
    const frontend::Type* const expected_type)
{
    ...
    if (!source_width || *source_width == 0) {
        report(
            "FSIM-ELAB-VHLOGIC-002",
            std::string { name } + " requires a nonempty packed operand",
            expression.operands.front().span);
        return std::nullopt;
    }
    ...
    if (bit_conversion || name == "to_01") {
        ...
        if (name == "to_x01z") {
            table[static_cast<std::size_t>(runtime::Logic9::z)] = runtime::Logic9::z;
        } else if (name == "to_ux01") {
            table[static_cast<std::size_t>(runtime::Logic9::u)] = runtime::Logic9::u;
        }
    }
    table[static_cast<std::size_t>(runtime::Logic9::zero)] = runtime::Logic9::zero;
    table[static_cast<std::size_t>(runtime::Logic9::l)] = runtime::Logic9::zero;
    table[static_cast<std::size_t>(runtime::Logic9::one)] = runtime::Logic9::one;
    table[static_cast<std::size_t>(runtime::Logic9::h)] = runtime::Logic9::one;
