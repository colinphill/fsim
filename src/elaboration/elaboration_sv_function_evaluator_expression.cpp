...
ConstantFunctionEvaluator::ConstantFunctionEvaluator(
    ...
    , call_cache_(!active_elaboration_call_caches.empty()
              ...
              ? *active_fold_call_cache
              : owned_call_cache_)
    , diagnostics_(active_constant_function_diagnostics)
    , messages_(active_constant_function_messages)
{
}

std::optional<Value> ConstantFunctionEvaluator::evaluate(
...
[[nodiscard]] std::optional<ConstantFunctionEvaluator::FixedArrayLayout>
ConstantFunctionEvaluator::fixed_array_layout(
    ...
    const SystemVerilogConstantEnvironment& environment) const
{
    ...
    if (type.vhdl_array
        ...
        if (!container.static_range_expressions.empty()) {
            std::string ignored;
            const auto left_value = evaluate_systemverilog_constant_expression(
...
ConstantFunctionEvaluator::fixed_array_offset(
...
std::optional<Value> ConstantFunctionEvaluator::evaluate_expression(
    const Expression& expression,
    ...
    const frontend::Type* expected_type)
{
    ...
    if (expression.kind == ExpressionKind::Call) {
        ...
        for (const auto* candidate : candidates) {
            ...
            named_function = true;
            const bool contextual_vhdl_array_result = expected_type != nullptr
                && function.language
                    == frontend::Language::Vhdl2008
                && unconstrained_vhdl_builtin_array(
                    function.return_type)
                && expected_type->width().value_or(0U) != 0U;
            const bool contextual_vhdl_result_mismatch = expected_type != nullptr
                && function.language
                    == frontend::Language::Vhdl2008
            ...
            bool obvious_match = expression.operands.size()
                    <= function.arguments.size()
            ...
            for (std::size_t index = 0;
                ...
                const auto domain = function.arguments[formal_index].type.domain;
    ...
    for (auto& operand : folded.operands) {
        const auto value = evaluate_expression(operand, environment, error);
        if (!value) {
            return std::nullopt;
        }
...
std::optional<ConstantFunctionEvaluator::ConstantPatternMatch>
ConstantFunctionEvaluator::evaluate_case_pattern(
    ...
    std::string& error)
{
    ...
    if (pattern.kind == ExpressionKind::Call
        ...
        const auto tag_width = std::max<std::size_t>(
            1U, static_cast<std::size_t>(std::bit_width(type->packed_members.size() - 1U)));
...
std::optional<Value> ConstantFunctionEvaluator::converted(
    const Expression& expression,
    const frontend::Type& type,
    const SystemVerilogConstantEnvironment& environment,
    std::string& error)
...
std::string ConstantFunctionEvaluator::result_alias(
...
bool ConstantFunctionEvaluator::unconstrained_vhdl_builtin_array(
    const frontend::Type& type)
{
