...
namespace {
    ...
    [[nodiscard]] bool same_expression_shape(
        ...
        const Expression& rhs)
    {
        ...
        for (std::size_t index = 0; index < lhs.operands.size(); ++index) {
            if (!same_expression_shape(lhs.operands[index], rhs.operands[index])) {
                return false;
            }
        }
        return true;
    ...
    [[nodiscard]] bool same_range(
        const std::optional<frontend::PackedRange>& lhs,
        const std::optional<frontend::PackedRange>& rhs)
    {
        return (!lhs && !rhs)
            || (lhs && rhs && lhs->left == rhs->left
                && lhs->right == rhs->right
                && lhs->descending == rhs->descending);
    }

    [[nodiscard]] bool same_range_expression(
        const frontend::PackedRangeExpression& lhs,
        const frontend::PackedRangeExpression& rhs)
    {
        return same_expression_shape(lhs.left, rhs.left)
            && same_expression_shape(lhs.right, rhs.right)
            && lhs.descending == rhs.descending;
    }
    ...
    {
        const bool lhs_nominal = is_systemverilog_nominal_packed_type(lhs);
        const bool rhs_nominal = is_systemverilog_nominal_packed_type(rhs);
        if (lhs_nominal || rhs_nominal) {
            return lhs_nominal && rhs_nominal
                && lhs.nominal_type == rhs.nominal_type;
        }
        if (lhs.domain != rhs.domain
            || lhs.systemverilog_scalar != rhs.systemverilog_scalar
            || lhs.is_signed != rhs.is_signed
            || lhs.packed_aggregate != rhs.packed_aggregate
            || !same_range(lhs.packed_range, rhs.packed_range)
            || lhs.systemverilog_virtual_interface
                != rhs.systemverilog_virtual_interface
            || lhs.systemverilog_interface_type
                != rhs.systemverilog_interface_type
            || lhs.systemverilog_interface_modport
                != rhs.systemverilog_interface_modport
            || lhs.systemverilog_class_declaration
                != rhs.systemverilog_class_declaration
            || lhs.systemverilog_packed_dimensions.size()
                != rhs.systemverilog_packed_dimensions.size()
            || lhs.packed_members.size() != rhs.packed_members.size()
            || lhs.systemverilog_container.has_value()
                != rhs.systemverilog_container.has_value()) {
            return false;
        }
        for (std::size_t index = 0;
            index < lhs.systemverilog_packed_dimensions.size(); ++index) {
            if (!same_range_expression(
                    lhs.systemverilog_packed_dimensions[index],
                    rhs.systemverilog_packed_dimensions[index])) {
                return false;
            }
        }
        for (std::size_t index = 0; index < lhs.packed_members.size(); ++index) {
            const auto& left = lhs.packed_members[index];
            const auto& right = rhs.packed_members[index];
            if (left.name != right.name
                || left.domain != right.domain
                || left.is_signed != right.is_signed
                || left.lsb_offset != right.lsb_offset
                || !same_range(left.packed_range, right.packed_range)
                || left.nested_types.size() != right.nested_types.size()) {
                return false;
            }
            for (std::size_t nested = 0;
                nested < left.nested_types.size(); ++nested) {
                if (!systemverilog_same_type(
                        left.nested_types[nested],
                        right.nested_types[nested])) {
                    return false;
                }
            }
        }
        if (!lhs.systemverilog_container) {
            return true;
        }
        const auto& left = *lhs.systemverilog_container;
        const auto& right = *rhs.systemverilog_container;
        if (left.kind != right.kind
            || !same_range(left.static_range, right.static_range)
            || left.static_range_expressions.size()
                != right.static_range_expressions.size()
            || left.element_types.size() != right.element_types.size()
            || static_cast<bool>(left.associative_index_type)
                != static_cast<bool>(right.associative_index_type)) {
            return false;
        }
        if (left.associative_index_type
            && !systemverilog_same_type(
                *left.associative_index_type,
                *right.associative_index_type)) {
            return false;
        }
        for (std::size_t index = 0;
            index < left.static_range_expressions.size(); ++index) {
            if (!same_range_expression(
                    left.static_range_expressions[index],
                    right.static_range_expressions[index])) {
                return false;
            }
        }
        for (std::size_t index = 0; index < left.element_types.size(); ++index) {
            if (!systemverilog_same_type(
                    left.element_types[index], right.element_types[index])) {
                return false;
            }
        }
        return true;
