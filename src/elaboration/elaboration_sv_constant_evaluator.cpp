    ...
    std::string& error)
{
    ...
    if (expression.kind == ExpressionKind::Identifier) {
        ...
        if (const auto fallback = fallback_environment.find(expression.text);
    ...
    if (expression.kind == ExpressionKind::Index
        ...
        if (position && *position >= 0
            ...
            if (base->packed.is_logic9()) {
                packed.set_logic9(
                    0, base->packed.get_logic9(static_cast<std::uint32_t>(*position)));
            } else {
                packed.set(
                    0, base->packed.get(static_cast<std::uint32_t>(*position)));
            }
        }
        return Value {
    ...
    if (expression.kind == ExpressionKind::Unary
        ...
        if (expression.text == "-") {
            ...
            operand->packed = packed_negate(operand->packed, operand->width);
        ...
        if (expression.text == "&" || expression.text == "|"
            ...
            if (expression.text == "~&" || expression.text == "~|"
                || expression.text == "~^" || expression.text == "^~") {
                reduced = reduced == Truth::True ? Truth::False
                    : reduced == Truth::False    ? Truth::True
                                                 : Truth::Unknown;
            }
            return logical_result(reduced, expression.span);
        }
        error = "unsupported SystemVerilog unary constant operator '"
            + expression.text + "'";
        return std::nullopt;
    }
    ...
    if (expression.kind == ExpressionKind::Call
        ...
        if (type_name == "bit") {
            ...
            error = "constant casts to named or nonintegral type '"
    ...
    if (expression.kind == ExpressionKind::Call
        ...
        if (expression.operands.size() < 2U) {
            error = "streaming concatenation requires a slice size and at "
                    "least one operand";
            return std::nullopt;
        }
        ...
        if (!slice_size || *slice_size == 0U
            ...
            if (slice_size) {
                error = "streaming concatenation slice size exceeds the "
        ...
        for (std::size_t index = 1;
            ...
            if (operand->width > maximum_constant_width - width) {
                error = "streaming concatenation result exceeds the constant-"
        ...
        if (operands.size() > maximum_constant_work_units / width) {
            error = "streaming concatenation exceeds the constant-evaluation "
                    "work limit";
            return std::nullopt;
        }
    ...
    const bool packed_query = expression.kind == ExpressionKind::Call
    ...
    if (packed_query) {
        const bool accepts_dimension = expression.text == "$left"
            || expression.text == "$right"
            || expression.text == "$low"
            || expression.text == "$high"
            || expression.text == "$size"
    ...
    if (expression.kind == ExpressionKind::Call
        ...
        const auto result = !found_one || highest_one == 0U
            ? 0U
    ...
    if (expression.kind == ExpressionKind::Call
        ...
        const bool common_signed = when_true->is_signed && when_false->is_signed;
        ...
        const bool logic9 = lhs.packed.is_logic9() || rhs.packed.is_logic9();
        if (logic9) {
            packed = packed.promoted_to_logic9();
        }
    ...
    const bool scalar_default_pattern = expression.kind == ExpressionKind::Aggregate
    ...
    const bool positional_pattern = expression.kind == ExpressionKind::Aggregate
        && expression.text == "sv-pattern"
        && std::ranges::all_of(
    ...
    if (expression.kind == ExpressionKind::Aggregate
        ...
        error = "keyed/default assignment patterns require aggregate type "
    ...
    if (expression.kind == ExpressionKind::Concatenation
        ...
        for (std::size_t index = first;
            ...
            if (operand->width > maximum_constant_width - element_width) {
                error = "concatenation element width exceeds the "
        ...
        if (element_width == 0
            ...
            error = "concatenation result exceeds the 16,777,216-bit "
                    "resource limit";
            return std::nullopt;
        }
        ...
        if (repetitions
            ...
            error = "concatenation or replication exceeds the constant-"
    ...
    error = "expression form is not a supported SystemVerilog integral "
            "constant expression";
    return std::nullopt;
}
