...
std::filesystem::path scratch_root(
  ...
  for (std::size_t attempt = 0; attempt < 100; ++attempt) {
    const auto serial =
        scratch_sequence.fetch_add(1, std::memory_order_relaxed);
    const auto tick =
        std::chrono::steady_clock::now().time_since_epoch().count();
    const auto path = parent / (
        "fsim-systemc-phase-" + std::to_string(tick) + "-"
...
std::filesystem::path support_library() {
#if defined(FSIM_SYSTEMC_SUPPORT_LIBRARY_PATH)
  auto result = std::filesystem::path{FSIM_SYSTEMC_SUPPORT_LIBRARY_PATH};
#if defined(FSIM_SYSTEMC_INSTALLED_SUPPORT_LIBRARY_PATH)
  std::error_code error;
  if (!std::filesystem::is_regular_file(result, error)) {
    result = FSIM_SYSTEMC_INSTALLED_SUPPORT_LIBRARY_PATH;
  }
#endif
  return result;
#else
...
std::optional<std::string> compiler_fingerprint(
    ...
    const std::filesystem::path& base,
    diagnostic::Engine& diagnostics) {
  auto abi_settings = settings;
  abi_settings.include_directories.clear();
  abi_settings.defines.clear();
  abi_settings.compile_options.clear();
  abi_settings.link_options.clear();
  abi_settings.libraries.clear();
  return plugin_host_fingerprint(abi_settings, base, diagnostics);
}
...
std::optional<std::vector<std::filesystem::path>> gcc_dependencies(
  ...
  const auto parsed = bytes
      ? parse_makefile_dependencies(*bytes) : std::nullopt;
  if (!parsed) {
    report(
        diagnostics, kCompileCode,
...
std::optional<std::vector<std::filesystem::path>> msvc_dependencies(
  ...
  const auto bytes = read_text_file(dependency_file, error);
  auto parsed = bytes
      ? parse_msvc_source_dependencies(*bytes) : std::nullopt;
  if (!parsed) {
    report(
        diagnostics, kCompileCode,
...
std::optional<std::vector<NativeInputIdentity>> input_identities(
  ...
  for (std::size_t index = 0; index < inputs.size(); ++index) {
    ...
    result.push_back({
        logical_input_name(inputs[index], source, index), std::move(*digest)});
  }
  return result;
}
  ...
  for (std::size_t index = 0; index < paths.size(); ++index) {
    ...
    if (!digest || *digest != expected[index].checksum) {
      report(
          ...
          "unpublished object was discarded", paths[index]);
      return false;
    }
  }
  return true;
...
std::optional<std::string> planned_object_input_digest(
  ...
  const auto compiler = settings.compiler.empty()
      ? default_compiler() : settings.compiler;
  ...
  metadata.defines_plugin_entry_point =
      source_bytes->find("fsim_plugin_init_v1") != std::string::npos;
  metadata.contains_macro_export =
      source_bytes->find("SC_FSIM_EXPORT") != std::string::npos;
...
bool link_incremental_plugin(
  ...
  const auto support = support_library();
  std::error_code error;
  ...
  if (needs_support
    ...
    report(
        diagnostics, kLinkCode,
        "cannot locate the fsim SystemC support library", support);
    return false;
  }
  const auto scratch = scratch_root(
      request.scratch_directory, base, diagnostics);
  ...
  if (toolchain == HostToolchain::msvc) {
    ...
    // Whole-archive is required because the support library's plug-in entry
    // point is not referenced by a translation-unit symbol.
    if (needs_support) {
      argv.emplace_back("-Wl,--whole-archive");
      argv.push_back(path_argument(support));
      argv.emplace_back("-Wl,--no-whole-archive");
    } else {
      argv.push_back(path_argument(support));
    }
    argv.emplace_back("-o");
...
std::shared_ptr<HierarchyRegistry> load_incremental_plugin(
  ...
  static std::unordered_map<
      std::string, std::weak_ptr<HierarchyRegistry>> cache;
  std::error_code path_error;
  const auto normalized = std::filesystem::weakly_canonical(
      directory / metadata->library, path_error);
  const auto cache_key = (path_error
      ? (directory / metadata->library).lexically_normal()
      : normalized).generic_string() + ":" + metadata->link_digest;
  const std::lock_guard lock{cache_mutex};
  if (const auto found = cache.find(cache_key); found != cache.end()) {
    if (auto existing = found->second.lock()) {
      if (!matching_inventory(*existing, metadata->factories, diagnostics)) {
        return {};
      }
      return existing;
    }
  ...
  std::string error;
  auto registry = HierarchyRegistry::load(directory / metadata->library, error);
  if (!registry) {
    report(
        diagnostics, kLinkCode,
  ...
  auto shared = std::shared_ptr<HierarchyRegistry>{std::move(registry)};
  cache.emplace(cache_key, shared);
  return shared;
}
