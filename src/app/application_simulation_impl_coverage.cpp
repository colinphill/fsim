...
Simulation::Impl::coverage_identity_from_digest(
...
void Simulation::Impl::coverage_hash_u64(
...
[[nodiscard]] artifact::CoverageDatabaseDigest
Simulation::Impl::coverage_digest(const support::Sha256::Digest& digest) noexcept
{
...
[[nodiscard]] artifact::CoverageDatabaseIdentity
Simulation::Impl::standard_run_identity()
{
...
Simulation::Impl::fallback_standard_coverage_database()
{
...
[[nodiscard]] std::optional<artifact::CoverageDatabaseContents>
Simulation::Impl::current_standard_coverage_database()
{
...
[[nodiscard]] std::optional<artifact::CoverageDatabaseContents>
Simulation::Impl::aggregate_standard_coverage_database()
{
...
[[nodiscard]] bool Simulation::Impl::standard_module_name_matches(
...
[[nodiscard]] std::optional<Simulation::Impl::StandardCoverageSelection>
Simulation::Impl::select_standard_coverage(
    ...
    if (const auto& inventory = built.design.code_coverage_inventory()) {
        for (const auto& instance : inventory->instances) {
            ...
            if (result.counters.size() != before) {
                ...
                result.identities.insert({ instance.identity.high, instance.identity.low });
            }
        }
    }
...
[[nodiscard]] std::int32_t Simulation::Impl::query_standard_coverage(
    const artifact::CoverageDatabaseContents& contents,
    const bool maximum,
    const std::set<artifact::CoverageDatabaseIdentity>* identities) noexcept
{
...
[[nodiscard]] std::int32_t Simulation::Impl::access_standard_coverage(
...
[[nodiscard]] std::int32_t Simulation::Impl::control_standard_coverage(
...
Simulation::Impl::standard_vpi_coverage_statistics(
...
[[nodiscard]] std::int32_t Simulation::Impl::control_standard_vpi_coverage(
...
void Simulation::Impl::configure_standard_vpi_coverage()
{
        ...
        if (roots == 1U) {
            ...
            if (object) {
                ...
                for (std::size_t index = 0U;
                    ...
                    ++index) {
                    counters.push_back({ static_cast<std::uint32_t>(index) });
                }
                if (counters.empty()) {
                    vpi_statement_counters.erase(object.value->handle);
                }
