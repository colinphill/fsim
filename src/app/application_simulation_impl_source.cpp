...
[[nodiscard]] const frontend::SystemVerilogClassMethodProfile*
Simulation::Impl::source_method_named(
    const runtime::SystemVerilogClassHandle handle,
...
Simulation::Impl::evaluate_source_string_expression(
...
[[nodiscard]] Simulation::Impl::SourceFunctionResult
Simulation::Impl::execute_source_function_statements(
...
[[nodiscard]] runtime::PackedLogic4 Simulation::Impl::invoke_source_profile(
    ...
    const std::span<const std::uint8_t> actual_directions)
{
    ...
    for (std::size_t formal = 0; formal < method.arguments.size(); ++formal) {
        ...
        if (argument.type.domain != frontend::ValueDomain::String)
            continue;
    ...
    for (std::size_t index = 0; index < actuals.size(); ++index) {
        ...
        if (direction != frontend::PortDirection::Input) {
            if (method.arguments[formal].type.domain
                ...
                string_actuals[index] = string_environment.at(method.arguments[formal].name);
            } else {
...
void Simulation::Impl::invoke_source_task_profile(
    const frontend::SystemVerilogClassMethodProfile& method,
    const runtime::SystemVerilogClassHandle handle,
    std::vector<runtime::PackedLogic4>& actuals)
{
    ...
    const bool virtual_dispatch)
{
    ...
    const auto uvm_base_method = canonical_identity.find("::uvm_object::") != std::string_view::npos;
    ...
    const auto uvm_component_base_method = canonical_identity.find("::uvm_component::")
...
[[nodiscard]] const frontend::SystemVerilogClassMethodProfile&
Simulation::Impl::source_static_method(const std::string_view canonical_identity) const
{
    ...
    for (const auto& specialization :
        ...
        const auto method = std::ranges::find(
            ...
            canonical_identity,
            &frontend::SystemVerilogClassMethodProfile::canonical_identity);
        if (method == specialization.methods.end())
            continue;
...
[[nodiscard]] runtime::PackedLogic4 Simulation::Impl::invoke_source_static_function(
    const std::string_view canonical_identity,
    std::vector<runtime::PackedLogic4>& actuals,
    std::vector<std::string>& string_actuals,
    const std::span<const std::string> actual_names,
    const std::span<const std::uint8_t> actual_directions)
{
    ...
    if (config_set || config_get) {
        ...
        } else if (const auto object = std::get_if<runtime::SystemVerilogClassHandle>(&*value)) {
