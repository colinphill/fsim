...
/// A predicate used by terminal consoles to accept only complete input.
/// When it returns false, Enter inserts a newline and keeps editing.
typedef bool (ic_input_complete_fun_t)(const char* input, void* arg);

/// Validate a previously generated completion set immediately before it is
/// applied. Return false when the application context has changed.
typedef bool (ic_completion_valid_fun_t)(void* arg);

/// A terminal-thread output sink. `style` is an Isocline style name and may
/// be NULL. Text is rendered as plain text, with terminal control bytes escaped.
typedef void (ic_console_output_emit_fun_t)(void* emit_arg, const char* text,
                                            const char* style);

/// A callback invoked periodically on the readline-owning thread. It can emit
/// queued output through `emit`, which redraws the current edit buffer after
/// output has been written.
// Return true to interrupt the current readline call. This is polled from the
// terminal-owning thread and lets another thread request interruption safely.
typedef bool (ic_console_idle_fun_t)(void* arg,
                                     ic_console_output_emit_fun_t* emit,
                                     void* emit_arg);

typedef enum ic_readline_status_e {
  IC_READLINE_ACCEPTED,
  IC_READLINE_CANCELLED,
  IC_READLINE_EOF,
  IC_READLINE_INTERRUPTED,
  IC_READLINE_ERROR
} ic_readline_status_t;
...
/// Read rich terminal input with an application supplied completeness check
/// and an output pump that runs on the terminal-owning thread. A false
/// completeness result makes Enter insert a newline. `idle_interval_ms` is
/// clamped to 10..1000 milliseconds when `idle` is non-NULL. This API requires
/// an interactive terminal; redirected input should use the caller's plain
/// stream path instead. The returned text must be freed with `ic_free`.
char* ic_readline_console(const char* prompt_text,
                          ...
                          ic_console_idle_fun_t* idle, void* idle_arg,
...
/// Set a plain contextual hint for the current completion request. It is shown
/// below the input when there is no unique inline completion.
void ic_add_hint(ic_completion_env_t* cenv, const char* hint);
