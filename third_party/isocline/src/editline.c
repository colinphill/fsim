...
typedef struct editor_s {
  ...
  ic_input_complete_fun_t* input_complete;
  void*         input_complete_arg;
  ic_completion_valid_fun_t* completions_valid;
  void*         completions_valid_arg;
  ic_console_idle_fun_t* idle;
  void*         idle_arg;
  long          idle_interval_ms;
  ic_readline_status_t* status;
  // caches
...
static char* edit_line( ic_env_t* env, const char* prompt_text,
                        ic_input_complete_fun_t* input_complete, void* input_complete_arg,
                        ic_completion_valid_fun_t* completions_valid, void* completions_valid_arg,
                        ic_console_idle_fun_t* idle, void* idle_arg, long idle_interval_ms,
                        ic_readline_status_t* status );  // defined below
static void edit_refresh(ic_env_t* env, editor_t* eb);

ic_private char* ic_editline(ic_env_t* env, const char* prompt_text) {
  return ic_editline_console(env, prompt_text, NULL, NULL, NULL, NULL, NULL, NULL,
                             0, NULL);
}

ic_private char* ic_editline_console(
  ...
  char* line = edit_line(env,prompt_text,input_complete,input_complete_arg,
                         completions_valid,completions_valid_arg,
                         idle,idle_arg,idle_interval_ms,status);
...
typedef struct edit_output_context_s {
  ic_env_t* env;
  editor_t* eb;
  bool emitted;
} edit_output_context_t;

static void edit_output_emit(void* arg, const char* text, const char* style) {
  edit_output_context_t* context = (edit_output_context_t*)arg;
  if (context == NULL || context->env == NULL || context->eb == NULL) return;
  ic_env_t* env = context->env;
  editor_t* eb = context->eb;
  if (!context->emitted) {
    edit_clear(env, eb);
    term_down(env->term, eb->cur_rows - eb->cur_row);
    context->emitted = true;
  }

  const char* active_style = (style != NULL && *style != 0 ? style : NULL);
  if (active_style != NULL) term_set_attr(env->term, bbcode_style(env->bbcode, active_style));
  ...
  while (*p != 0) {
    if (*p == 0x1B && p[1] == '[') {
      const unsigned char* final = p + 2;
      while (*final != 0 && *final != 'm' && final - p <= 8) final++;
      if (*final == 'm') {
        const size_t code_len = (size_t)(final - p + 1);
        const char* next_style = NULL;
        bool recognized = true;
        if (code_len == 4 && memcmp(p, "\x1B[0m", 4) == 0) next_style = NULL;
        else if (code_len == 5 && memcmp(p, "\x1B[36m", 5) == 0) next_style = "console-note";
        else if (code_len == 5 && memcmp(p, "\x1B[33m", 5) == 0) next_style = "console-warning";
        else if (code_len == 5 && memcmp(p, "\x1B[31m", 5) == 0) next_style = "console-error";
        else if (code_len == 7 && memcmp(p, "\x1B[1;31m", 7) == 0) next_style = "console-fatal";
        else recognized = false;
        if (recognized) {
          safe[safe_len] = 0;
          term_write(env->term, safe);
          safe_len = 0;
          term_attr_reset(env->term);
          active_style = next_style;
          if (active_style != NULL) {
            term_set_attr(env->term, bbcode_style(env->bbcode, active_style));
          }
          p = final + 1;
    ...
    if (*p == '\n') {
      ...
      safe[safe_len++] = '\\';
      safe[safe_len++] = 'x';
      safe[safe_len++] = hex[(*p >> 4) & 0x0F];
      safe[safe_len++] = hex[*p & 0x0F];
      ended_with_newline = false;
      p++;
    }
    else {
      safe[safe_len++] = (char)*p;
      ended_with_newline = false;
      p++;
    }
    if (safe_len >= sizeof(safe) - 5) {
      safe[safe_len] = 0;
      term_write(env->term, safe);
      safe_len = 0;
    }
  ...
  term_write(env->term, safe);
  term_attr_reset(env->term);
  if (!ended_with_newline) term_writeln(env->term, "");
  term_flush(env->term);
}
...
static void edit_refresh_hint(ic_env_t* env, editor_t* eb) {
  ...
  if (sbuf_len(eb->hint) == 0) {
    const char* context_hint = completions_get_context_hint(env->completions);
    if (context_hint != NULL && *context_hint != 0) {
      sbuf_appendf(eb->extra, "[ic-info]%s[/ic-info]", context_hint);
    }
  }
...
static void edit_insert_paste(ic_env_t* env, editor_t* eb) {
  ssize_t paste_len = 0;
  bool overflow = false;
  const char* paste = tty_paste_data(env->tty, &paste_len, &overflow);
  if (paste == NULL || paste_len <= 0 || overflow) {
    tty_paste_clear(env->tty);
    term_write(env->term, "\a");
    return;
  }
  ...
  for (ssize_t i = 0; i < paste_len; i++) {
    const unsigned char c = (unsigned char)paste[i];
    if (c == '\r') {
      if (i + 1 < paste_len && paste[i + 1] == '\n') i++;
      sbuf_append_char(normalized, (env->singleline_only ? ' ' : '\n'));
    }
    else if (c == '\n') {
      sbuf_append_char(normalized, (env->singleline_only ? ' ' : '\n'));
    }
    else if (c == '\t' || (c >= 0x20 && c != 0x7f)) {
      // Drop terminal control bytes from paste while preserving UTF-8 bytes.
      sbuf_append_char(normalized, (char)c);
    }
  }

  const ssize_t normalized_len = sbuf_len(normalized);
  if (normalized_len > 0) {
    editor_start_modify(eb);
    const ssize_t next_pos = sbuf_insert_at_n(eb->input, sbuf_string(normalized),
                                               normalized_len, eb->pos);
    if (next_pos >= 0) {
      eb->pos = next_pos;
      eb->modified = true;
      edit_refresh_hint(env, eb);
...
static char* edit_line( ic_env_t* env, const char* prompt_text,
                        ...
                        ic_readline_status_t* status )
{
  if (status != NULL) *status = IC_READLINE_ERROR;
  ...
  eb.history_idx   = 0;  
  eb.input_complete = input_complete;
  eb.input_complete_arg = input_complete_arg;
  eb.completions_valid = completions_valid;
  eb.completions_valid_arg = completions_valid_arg;
  eb.idle = idle;
  eb.idle_arg = idle_arg;
  eb.idle_interval_ms = idle_interval_ms;
  eb.status = status;
  ...
  if (status != NULL) *status = IC_READLINE_ACCEPTED;
  ...
  while(true) {    
    ...
    term_flush(env->term);
    if (eb.idle != NULL) {
      long interval = eb.idle_interval_ms;
      if (interval < 10) interval = 10;
      if (interval > 1000) interval = 1000;
      if (!tty_read_timeout(env->tty, interval, &c)) {
        if (tty_term_resize_event(env->tty)) edit_resize(env, &eb);
        if (sbuf_len(eb.hint) > 0 || sbuf_len(eb.extra) > 0) edit_refresh(env, &eb);
        struct edit_output_context_s output_context = { env, &eb, false };
        const bool interrupted = eb.idle(eb.idle_arg, &edit_output_emit, &output_context);
        if (output_context.emitted) {
          eb.cur_rows = 1;
          eb.cur_row = 0;
          edit_refresh(env, &eb);
        }
    ...
    else if (env->hint_delay <= 0 ||
             (sbuf_len(eb.hint) == 0 && sbuf_len(eb.extra) == 0)) {
      // blocking read
      c = tty_read(env->tty);
    }
    else {
      ...
      if (!tty_read_timeout(env->tty, env->hint_delay, &c)) {
        // timed-out
        if (sbuf_len(eb.hint) > 0 || sbuf_len(eb.extra) > 0) {
          // display hint
          edit_refresh(env, &eb);
        }
        c = tty_read(env->tty);
    ...
    if (c == KEY_ENTER) {
      ...
      else if (eb.input_complete != NULL &&
               !eb.input_complete(sbuf_string(eb.input), eb.input_complete_arg)) {
  ...
  char* res; 
  if (status != NULL) {
    if (c == KEY_EVENT_STOP) *status = IC_READLINE_INTERRUPTED;
    else if (c == KEY_CTRL_D && sbuf_len(eb.input) == 0) *status = IC_READLINE_EOF;
    else if (c == KEY_CTRL_C || c == KEY_BELL || c == KEY_ESC) *status = IC_READLINE_CANCELLED;
    else *status = IC_READLINE_ACCEPTED;
  }
