...
ic_private void completions_clear(completions_t* cms) {  
  ...
  mem_free(cms->mem, (void*)cms->context_hint);
  cms->context_hint = NULL;
}
...
ic_private const char* completions_get_context_hint(completions_t* cms) {
  return (cms == NULL ? NULL : cms->context_hint);
}

ic_private void completions_set_context_hint(completions_t* cms, const char* hint) {
  if (cms == NULL) return;
  mem_free(cms->mem, (void*)cms->context_hint);
  cms->context_hint = (hint == NULL ? NULL : mem_strdup(cms->mem, hint));
}
...
ic_public const char* ic_completion_input(ic_completion_env_t* cenv, long* cursor) {
  if (cursor != NULL) *cursor = (cenv == NULL ? 0 : cenv->cursor);
  return (cenv == NULL ? NULL : cenv->input);
}
...
ic_public void ic_add_hint(ic_completion_env_t* cenv, const char* hint) {
  if (cenv == NULL || cenv->env == NULL || cenv->env->completions == NULL) return;
  completions_set_context_hint(cenv->env->completions, hint);
}
