...
static code_t tty_read_bracketed_paste(tty_t* tty) {
  static const uint8_t end_marker[] = { 0x1B, '[', '2', '0', '1', '~' };
  uint8_t pending[sizeof(end_marker)];
  size_t pending_len = 0;
  tty_paste_begin(tty);
  uint8_t c;
  while (tty_readc_noblock(tty, &c, -1)) {
    pending[pending_len++] = c;
    while (pending_len > 0 &&
           memcmp(pending, end_marker, pending_len) != 0) {
      tty_paste_append(tty, pending[0]);
      memmove(pending, pending + 1, --pending_len);
    }
    if (pending_len == sizeof(end_marker)) {
      tty_paste_end(tty);
      return KEY_PASTE;
    }
  }
...
static code_t tty_read_csi(tty_t* tty, uint8_t c1, uint8_t peek, code_t mods0, long esc_timeout) {
  ...
  if (c1 == '[' && special == 0 && num1 == 200 && final == '~') {
    return tty_read_bracketed_paste(tty);
  }
