...
static bool edit_complete(ic_env_t* env, editor_t* eb, ssize_t idx) {
  if (eb->completions_valid != NULL &&
      !eb->completions_valid(eb->completions_valid_arg)) {
    return false;
  }
...
again:
  ...
  // Read here; poll output on the terminal-owning thread while the menu is open.
  code_t c;
  if (eb->idle != NULL) {
    long interval = eb->idle_interval_ms;
    if (interval < 10) interval = 10;
    if (interval > 1000) interval = 1000;
    while (!tty_read_timeout(env->tty, interval, &c)) {
      if (tty_term_resize_event(env->tty)) edit_resize(env, eb);
      edit_output_context_t output_context = { env, eb, false };
  ...
  if (eb->completions_valid != NULL &&
    ...
    sbuf_clear(eb->extra);
    edit_refresh(env, eb);
    if (c != KEY_ENTER && c != KEY_ESC && c != KEY_EVENT_STOP) {
