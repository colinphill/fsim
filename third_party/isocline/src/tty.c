...
#define TTY_MAX_PASTE_BYTES (4 * 1024 * 1024)
...
ic_private void tty_paste_begin(tty_t* tty) {
  if (tty == NULL) return;
  if (tty->paste == NULL) tty->paste = sbuf_new(tty->mem);
  if (tty->paste != NULL) sbuf_clear(tty->paste);
  tty->paste_ready = false;
  tty->paste_overflow = (tty->paste == NULL);
}

ic_private void tty_paste_append(tty_t* tty, uint8_t c) {
  if (tty == NULL || tty->paste == NULL) {
    if (tty != NULL) tty->paste_overflow = true;
    return;
  }
  if (sbuf_len(tty->paste) >= TTY_MAX_PASTE_BYTES) {
    tty->paste_overflow = true;
    return;
  }
  if (sbuf_append_char(tty->paste, (char)c) < 0) tty->paste_overflow = true;
}

ic_private void tty_paste_end(tty_t* tty) {
  if (tty != NULL) tty->paste_ready = true;
}

ic_private const char* tty_paste_data(tty_t* tty, ssize_t* len, bool* overflow) {
  if (len != NULL) *len = 0;
  if (overflow != NULL) *overflow = false;
  if (tty == NULL || !tty->paste_ready || tty->paste == NULL) return NULL;
  if (len != NULL) *len = sbuf_len(tty->paste);
  if (overflow != NULL) *overflow = tty->paste_overflow;
  return sbuf_string(tty->paste);
}

ic_private void tty_paste_clear(tty_t* tty) {
  if (tty == NULL) return;
  if (tty->paste != NULL) sbuf_clear(tty->paste);
  tty->paste_ready = false;
  tty->paste_overflow = false;
}
