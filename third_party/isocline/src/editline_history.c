...
again:
  ...
  // Wait for input, polling output and interruption while the search prompt is
  // active just as the main editor does.
  code_t c = KEY_NONE;
  if (hentry == NULL) {
    c = KEY_ESC;
  }
  else if (eb->idle != NULL) {
    ...
    while (!tty_read_timeout(env->tty, interval, &c)) {
      ...
      const bool interrupted = eb->idle(eb->idle_arg,
          &edit_output_emit, &output_context);
  ...
  else if (c == KEY_ESC || c == KEY_BELL /* ^G */ || c == KEY_CTRL_C) {
    c = 0;
    eb->disable_undo = false;
    editor_undo_restore(eb, false);
  } 
