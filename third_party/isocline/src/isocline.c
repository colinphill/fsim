...
ic_public char* ic_readline_console(
  ...
  ic_env_t* env = ic_get_env();
  if (env == NULL || env->noedit) return NULL;

  // Save and restore the process-global defaults around this terminal call.
  ic_completer_fun_t* prev_completer;
  void* prev_completer_arg;
  completions_get_completer(env->completions, &prev_completer, &prev_completer_arg);
  ic_highlight_fun_t* prev_highlighter = env->highlighter;
  ...
  char* result = ic_editline_console(
      env, prompt_text, input_complete, input_complete_arg,
