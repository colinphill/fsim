...
set(FSIM_INVENTORY
  "${FSIM_SOURCE_DIR}/tests/feature_matrix/systemc_accellera_inventory.tsv")
...
foreach(FSIM_INPUT IN ITEMS "${FSIM_INVENTORY}" "${FSIM_PLAN}" "${FSIM_TEST_CMAKE}")
  if(NOT EXISTS "${FSIM_INPUT}")
    message(FATAL_ERROR "SystemC Accellera inventory input is missing: ${FSIM_INPUT}")
  endif()
endforeach()
...
if(NOT FSIM_ACTUAL_DIGEST STREQUAL FSIM_EXPECTED_DIGEST)
  message(FATAL_ERROR
    "SystemC Accellera inventory digest changed: expected ${FSIM_EXPECTED_DIGEST}, got ${FSIM_ACTUAL_DIGEST}")
endif()

file(READ "${FSIM_PLAN}" FSIM_PLAN_TEXT)
string(FIND "${FSIM_PLAN_TEXT}"
  "### Batch 172 - Accellera SystemC 3.0.2" FSIM_BATCH_START)
string(FIND "${FSIM_PLAN_TEXT}"
  "### Batch 173 - SCV 2.0.1" FSIM_BATCH_END)
if(FSIM_BATCH_START EQUAL -1 OR FSIM_BATCH_END EQUAL -1 OR
   FSIM_BATCH_END LESS_EQUAL FSIM_BATCH_START)
  message(FATAL_ERROR "authoritative Batch 172 plan boundary is missing")
endif()
...
foreach(FSIM_TOKEN IN ITEMS
    "Accellera SystemC 3.0.2 shared runtime per fsim process"
    "opaque transport-neutral backend"
    "Native TLM 1.0/2.0"
    "partitioner"
    "Release build/testing is"
    "reserved for Change 20"
    "formatting-only header churn is forbidden"
    "current no-CI instruction")
  string(FIND "${FSIM_BATCH_TEXT}" "${FSIM_TOKEN}" FSIM_TOKEN_OFFSET)
  if(FSIM_TOKEN_OFFSET EQUAL -1)
    message(FATAL_ERROR "Batch 172 plan lost required token: ${FSIM_TOKEN}")
  endif()
endforeach()
foreach(FSIM_CHANGE RANGE 1 20)
  ...
  if(FSIM_CHANGE_OFFSET EQUAL -1)
    message(FATAL_ERROR "Batch 172 plan lost exact Change ${FSIM_CHANGE}")
  endif()
  ...
  if(NOT FSIM_DUPLICATE EQUAL -1)
    message(FATAL_ERROR "Batch 172 plan duplicates Change ${FSIM_CHANGE}")
  endif()
endforeach()

file(READ "${FSIM_TEST_CMAKE}" FSIM_TEST_CMAKE_TEXT)
foreach(FSIM_TOKEN IN ITEMS
    "NAME fsim.systemc-accellera-inventory"
    "CheckSystemCAccelleraInventory.cmake"
    "fsim.systemc-accellera-inventory")
  string(FIND "${FSIM_TEST_CMAKE_TEXT}" "${FSIM_TOKEN}" FSIM_TOKEN_OFFSET)
  if(FSIM_TOKEN_OFFSET EQUAL -1)
    message(FATAL_ERROR
      "SystemC Accellera inventory registration lost token: ${FSIM_TOKEN}")
  endif()
endforeach()
...
set(FSIM_EXPECTED_RELEASE "AccelleraSystemC3.0.2,TLM1.0,TLM2.0")
...
function(fsim_require_systemc_owner FSIM_ID FSIM_FIELD FSIM_PATH FSIM_PREFIX FSIM_EXISTS)
...
if(NOT FSIM_ROW_COUNT EQUAL 20)
  message(FATAL_ERROR
    "SystemC Accellera inventory requires SPDX, header and 18 rows")
endif()
...
if(NOT FSIM_SPDX STREQUAL "# SPDX-License-Identifier: Apache-2.0" OR
  ...
  message(FATAL_ERROR "SystemC Accellera inventory header or SPDX policy changed")
endif()
...
foreach(FSIM_INDEX RANGE 2 19)
  ...
  if(NOT FSIM_FIELD_COUNT EQUAL 15)
    message(FATAL_ERROR
      "SystemC Accellera row ${FSIM_INDEX} does not have fifteen fields")
  endif()
  ...
  if(NOT FSIM_ID STREQUAL "SC302-C${FSIM_CHANGE_TEXT}" OR
     ...
     NOT FSIM_CLOSURE STREQUAL "B172-C${FSIM_CHANGE_TEXT}" OR
  ...
  if(NOT FSIM_DUPLICATE_ID EQUAL -1 OR NOT FSIM_DUPLICATE_CLOSURE EQUAL -1)
    message(FATAL_ERROR "duplicate SystemC Accellera id or closure: ${FSIM_ID}")
  endif()
  ...
  fsim_require_systemc_owner("${FSIM_ID}" implementation
  ...
  foreach(FSIM_EVIDENCE_INDEX RANGE 8 12)
    ...
    fsim_require_systemc_owner("${FSIM_ID}" "evidence-${FSIM_EVIDENCE_INDEX}"
  ...
  if(NOT FSIM_DIAGNOSTIC_OWNER STREQUAL "docs/diagnostics.md" OR
     NOT FSIM_RESOURCE_OWNER STREQUAL
       "cmake/CheckSystemCAccelleraPortabilityContract.cmake")
...
if(NOT FSIM_ID_COUNT EQUAL 18 OR NOT FSIM_CLOSURE_COUNT EQUAL 18 OR
  ...
  message(FATAL_ERROR
    "SystemC Accellera boundary mismatch: ids=${FSIM_ID_COUNT}, closures=${FSIM_CLOSURE_COUNT}, preserved=${FSIM_PRESERVED_COUNT}, active=${FSIM_ACTIVE_COUNT}")
endif()

message(STATUS
  "SystemC Accellera inventory passed: rows=18 preserved=${FSIM_PRESERVED_COUNT} active=${FSIM_ACTIVE_COUNT} digest=${FSIM_ACTUAL_DIGEST}")
