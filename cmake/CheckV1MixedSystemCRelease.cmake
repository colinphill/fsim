...
set(FSIM_AUDIT "${FSIM_SOURCE_DIR}/docs/v1-mixed-systemc-release-audit.md")
...
foreach(FSIM_INPUT IN ITEMS "${FSIM_MATRIX}" "${FSIM_AUDIT}" "${FSIM_TEST_CMAKE}")
  if(NOT EXISTS "${FSIM_INPUT}")
    message(FATAL_ERROR "mixed/SystemC release-audit input not found: ${FSIM_INPUT}")
  endif()
endforeach()
...
foreach(FSIM_LINE IN LISTS FSIM_MATRIX_LINES)
  if(NOT FSIM_LINE MATCHES "^\\| ((ML|SC)-[0-9]+) \\|")
    continue()
  endif()
  ...
  if(FSIM_FIELD_COUNT LESS 9)
    message(FATAL_ERROR "malformed mixed/SystemC release row: ${FSIM_ID}")
  endif()
...
if(FSIM_RUNTIME_OWNER_COUNT LESS 8)
  message(FATAL_ERROR
    "expected at least 8 mixed/SystemC runtime owners, found "
    "${FSIM_RUNTIME_OWNER_COUNT}")
endif()

set(FSIM_REVIEW_IDS
  B130-T4-MIXED-TYPES
  B130-T4-MIXED-SCHEDULING
  B130-T4-SYSTEMC-FACADE
  B130-T4-SYSTEMC-LIFECYCLE
  B130-T4-NATIVE
)
foreach(FSIM_REVIEW_ID IN LISTS FSIM_REVIEW_IDS)
  ...
  if(FSIM_REVIEW_INDEX EQUAL -1)
    message(FATAL_ERROR "mixed/SystemC audit omits review ${FSIM_REVIEW_ID}")
  endif()
endforeach()

string(FIND
  "${FSIM_TEST_CMAKE_CONTENTS}"
  "NAME fsim.v1-mixed-systemc-release"
  FSIM_REGISTRATION_INDEX
)
if(FSIM_REGISTRATION_INDEX EQUAL -1)
  message(FATAL_ERROR "fsim.v1-mixed-systemc-release is not registered")
endif()

message(STATUS
  "final mixed/SystemC audit covers ${FSIM_ROW_COUNT} rows with "
