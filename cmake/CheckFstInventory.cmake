...
set(FSIM_INVENTORY "${FSIM_SOURCE_DIR}/tests/feature_matrix/fst_inventory.tsv")
...
foreach(FSIM_INPUT IN ITEMS "${FSIM_INVENTORY}" "${FSIM_PLAN}" "${FSIM_TEST_CMAKE}")
  if(NOT EXISTS "${FSIM_INPUT}")
    message(FATAL_ERROR "FST inventory input is missing: ${FSIM_INPUT}")
  endif()
endforeach()
...
if(NOT FSIM_ACTUAL_DIGEST STREQUAL FSIM_EXPECTED_DIGEST)
  message(FATAL_ERROR
    "FST inventory digest changed: expected ${FSIM_EXPECTED_DIGEST}, got ${FSIM_ACTUAL_DIGEST}")
endif()

file(READ "${FSIM_PLAN}" FSIM_PLAN_TEXT)
string(FIND "${FSIM_PLAN_TEXT}" "### Batch 171 - FST tracing closure" FSIM_BATCH_START)
string(FIND "${FSIM_PLAN_TEXT}" "### Batch 172 - Accellera SystemC 3.0.2" FSIM_BATCH_END)
if(FSIM_BATCH_START EQUAL -1 OR FSIM_BATCH_END EQUAL -1 OR
   FSIM_BATCH_END LESS_EQUAL FSIM_BATCH_START)
  message(FATAL_ERROR "authoritative Batch 171 plan boundary is missing")
endif()
...
foreach(FSIM_TOKEN IN ITEMS
    "deterministic FST output"
    "clean-room under Apache-2.0"
    "GTKWave/libfst"
    "GPL-2.0"
    "Release build/testing"
    "Avoid header formatting changes"
    "not a sanitizer or hosted-CI monitoring")
  string(FIND "${FSIM_BATCH_TEXT}" "${FSIM_TOKEN}" FSIM_TOKEN_OFFSET)
  if(FSIM_TOKEN_OFFSET EQUAL -1)
    message(FATAL_ERROR "Batch 171 plan lost required token: ${FSIM_TOKEN}")
  endif()
endforeach()
foreach(FSIM_CHANGE RANGE 1 20)
  ...
  if(FSIM_CHANGE_OFFSET EQUAL -1)
    message(FATAL_ERROR "Batch 171 plan lost exact Change ${FSIM_CHANGE}")
  endif()
  ...
  if(NOT FSIM_DUPLICATE EQUAL -1)
    message(FATAL_ERROR "Batch 171 plan duplicates Change ${FSIM_CHANGE}")
  endif()
endforeach()

file(READ "${FSIM_TEST_CMAKE}" FSIM_TEST_CMAKE_TEXT)
foreach(FSIM_TOKEN IN ITEMS
    "NAME fsim.fst-inventory"
    "CheckFstInventory.cmake"
    "fsim.fst-inventory")
  string(FIND "${FSIM_TEST_CMAKE_TEXT}" "${FSIM_TOKEN}" FSIM_TOKEN_OFFSET)
  if(FSIM_TOKEN_OFFSET EQUAL -1)
    message(FATAL_ERROR "FST inventory registration lost token: ${FSIM_TOKEN}")
  endif()
endforeach()

set(FSIM_HEADER
  "id\tformats\thdl_profiles\tdomain\tboundary\tclosure\tobligation\timplementation_owner\tpositive_evidence\tnegative_evidence\tequivalence_evidence\tphase_evidence\tartifact_evidence\tdiagnostic_owner\tresource_owner")
set(FSIM_EXPECTED_FORMATS "VCD,FST,Internal")
...
set(FSIM_EXPECTED_DOMAINS
  trace-model fst-container-hierarchy format-selection scalar-vector-values
  extended-typed-values nine-state-aggregate-strength ordered-changes-aliases
  mixed-roots-boundaries dynamic-observation selection-lifecycle
  failure-containment public-controls artifact-relocation
  deterministic-compression reader-equivalence negative-resource-platform
  corpus-closure)

function(fsim_require_fst_path FSIM_ID FSIM_FIELD FSIM_PATH FSIM_PREFIX FSIM_EXISTS)
...
if(NOT FSIM_ROW_COUNT EQUAL 19)
  message(FATAL_ERROR "FST inventory requires SPDX, header and 17 rows")
endif()
...
if(NOT FSIM_SPDX STREQUAL "# SPDX-License-Identifier: Apache-2.0" OR
  ...
  message(FATAL_ERROR "FST inventory header or SPDX policy changed")
endif()
...
foreach(FSIM_INDEX RANGE 2 18)
  ...
  if(NOT FSIM_FIELD_COUNT EQUAL 15)
    message(FATAL_ERROR "FST row ${FSIM_INDEX} does not have fifteen fields")
  endif()
  ...
  if(NOT FSIM_ID STREQUAL "FST-C${FSIM_CHANGE_TEXT}" OR
     NOT FSIM_FORMATS STREQUAL FSIM_EXPECTED_FORMATS OR
     ...
     NOT FSIM_CLOSURE STREQUAL "B171-C${FSIM_CHANGE_TEXT}" OR
  ...
  if(NOT FSIM_DUPLICATE_ID EQUAL -1 OR NOT FSIM_DUPLICATE_CLOSURE EQUAL -1)
    message(FATAL_ERROR "duplicate FST id or closure: ${FSIM_ID}")
  endif()
  ...
  fsim_require_fst_path("${FSIM_ID}" implementation
  ...
  foreach(FSIM_EVIDENCE_INDEX RANGE 8 12)
    ...
    fsim_require_fst_path("${FSIM_ID}" "evidence-${FSIM_EVIDENCE_INDEX}"
...
if(NOT FSIM_ID_COUNT EQUAL 17 OR NOT FSIM_CLOSURE_COUNT EQUAL 17 OR
  ...
  message(FATAL_ERROR
    "FST boundary mismatch: ids=${FSIM_ID_COUNT}, closures=${FSIM_CLOSURE_COUNT}, preserved=${FSIM_PRESERVED_COUNT}, active=${FSIM_ACTIVE_COUNT}")
endif()

message(STATUS
  "FST inventory passed: rows=17 preserved=${FSIM_PRESERVED_COUNT} active=${FSIM_ACTIVE_COUNT} digest=${FSIM_ACTUAL_DIGEST}")
