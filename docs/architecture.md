...
- constant loads, signal reads, and delta-scoped signal-event queries;
...
`write_after`, and `signal_event` callbacks are append-only extensions of the
v1 table: original field offsets remain fixed, and each compiled process checks
`struct_size` only for the callback tail it actually uses. A process using only
an earlier operation set therefore remains valid with the corresponding v1
prefix. `SignalEvent` consults the kernel-owned `(time, delta)` change stamp
rather than copying scheduler state into generated code. CMake requires
...
update-phase writes, delayed writes, signal-event queries, design stop, and
halt.
