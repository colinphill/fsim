...
| `FSIM-COV-015` | error | Verilog/SystemVerilog atomic-condition discovery received an unsupported language, invalid or ambiguous authenticated source map, missing or malformed decision expression, invalid atom span, duplicate point, or excessive source, statement, expression, condition, path, or nesting count. |
| `FSIM-COV-016` | error | VHDL atomic-condition discovery received a non-VHDL language family, unknown VHDL revision, invalid or ambiguous authenticated source map, missing or malformed Boolean decision, invalid atom span, duplicate point, or excessive bounded input. |
| `FSIM-COV-017` | error | Atomic-condition evaluation received an empty, incomplete, conflicting, noncanonical, duplicate, or excessive decision plan, an invalid point identity, or a failed atom evaluator; no partial observations are published. |
| `FSIM-COV-018` | error | Atomic-condition outcome recording received an invalid or duplicate point owner, inconsistent saturation state, invalid truth value, mismatched or duplicate observation, or excessive table/batch size; validation precedes mutation. |
| `FSIM-COV-019` | error | Expression-combination inventory construction received no atoms, an invalid or duplicate atom identity, or an excessive atom/resource request; bounded-away combinations remain explicit exact or symbolic omissions. |
| `FSIM-COV-020` | error | Binary/four-state toggle recording received an invalid or duplicate point/bit owner, invalid logic encoding, inconsistent scored or X/Z diagnostic saturation state, mismatched transition ownership, or excessive outcome/transition input; validation precedes mutation. |
| `FSIM-COV-021` | error | Verilog/SystemVerilog toggle inventory construction received an invalid language or instantiated-unit owner, unauthenticated source/span, noncanonical or duplicate hierarchical object, unspecialized packed width, or excessive source/object/bit/path input; construction is transactional. |
| `FSIM-COV-022` | error | VHDL toggle inventory construction received an invalid revision, architecture/entity-interface owner, unauthenticated architecture or entity source/span, noncanonical or duplicate hierarchical object, unspecialized packed width, or excessive source/object/bit/path input; construction is transactional. |
| `FSIM-COV-023` | error | Default toggle-selection construction received an invalid language/unit/instance owner, unauthenticated source/span, noncanonical or duplicate excluded object, invalid line, or excessive source/declaration/exclusion/reason/path/depth input; construction is transactional. |
| `FSIM-COV-024` | error | Explicit memory/array toggle selection received invalid default ownership, an unknown or duplicate object rule, an implicit/malformed/out-of-range element selector, unsupported or unresolved element shape, overlapping bit ownership, or excessive rule/key/range/element/bit input; construction is transactional. |
| `FSIM-COV-025` | error | FSM object inference received invalid language/unit/instance ownership, unauthenticated source/span data, a malformed or incompatible SystemVerilog pragma or validated VHDL/manifest hint, duplicate or invalid current/next/legal-state identity, or excessive source/object/case/choice/assignment/description/state/legal-set/name/path/depth/line input; construction is transactional. |
| `FSIM-COV-026` | error | VHDL source or language-neutral manifest FSM hint construction received an invalid VHDL revision, malformed declaration/specification, invalid exact instance/current/next/legal description, or excessive attribute/entity/hint/state/name/value/instance input; construction is transactional. |
| `FSIM-COV-027` | error | FSM runtime construction or recording received an invalid instance/object/state/bin identity, duplicate or non-canonical machine/state/transition ownership, inconsistent saturation or previous-state state, an observation owned by another machine, or excessive machine/state/transition/observation/path input; validation is transactional. |
| `FSIM-COV-028` | warning | FSM description inference found multiple viable current-state objects or next-state relations; the ambiguous relation is omitted while independently unambiguous inference remains available. |
| `FSIM-COV-029` | warning | FSM description inference found an explicit scalar current-state object without a legal-state universe or an exact case that covers only part of its enum universe; incomplete evidence is not credited. |
| `FSIM-COV-030` | warning | FSM description inference found incompatible case, assignment, SystemVerilog pragma, VHDL source-hint, or manifest descriptions; conflicting evidence is not credited while independent enum/case fallback remains available. |
| `FSIM-COV-031` | error | A `.fsimcov` container has the wrong v3 magic, schema, byte order, fixed header or namespace directory; unsupported flags or encoding; noncanonical code/SystemVerilog-functional/PSL namespace order or schema; a gap, overlap, size disagreement, arithmetic overflow, or bounded resource violation. |
| `FSIM-COV-032` | error | Unified coverage database contents have an invalid v3 model fingerprint, source, run, namespace/family/scope pairing, bin or exclusion owner, source/run reference, saturation state, canonical order, duplicate identity, unsafe or excessive text, count, allocation, or arithmetic resource failure. |
| `FSIM-COV-033` | error | SystemVerilog functional coverage cannot be projected transactionally into its v3 database namespace because the run, source binding, declaration, instance, bin, namespace ownership, saturation metadata, database model, or bounded resource contract is invalid. |
| `FSIM-COV-034` | error | PSL coverage cannot be projected transactionally into its v3 database namespace because the run, source binding, directive identity/kind/outcome accounting, instance, namespace ownership, saturation metadata, database model, or bounded resource contract is invalid. |
| `FSIM-COV-035` | error | Unified coverage database serialization, digest verification, bounded decoding, file I/O, interrupted-write recovery, or atomic replacement failed; no partially decoded model or newly written destination is published. |
| `FSIM-COV-036` | error | Strict unified coverage database merge failed because an input is invalid, describes a different model, source inventory, or exclusion policy, repeats a run identity, or exceeds a resource ceiling; no partial merge is published. |
| `FSIM-COV-037` | error | Explicit partial coverage merge failed because the target or history is invalid, a retained run identity is repeated, or input, examined-record, or output ceilings are exceeded; the target remains unchanged and no partial result is published. |
| `FSIM-COV-038` | error | SystemVerilog `$coverage_control` is malformed, uses the wrong language profile, or does not provide exactly four positional command, coverage-type, scope, and module-or-instance selector arguments. |
| `FSIM-COV-039` | error | SystemVerilog `$coverage_get`, `$coverage_get_max`, `$coverage_merge`, or `$coverage_save` is malformed, uses the wrong language profile, has named or invalid arguments, omits the required scope and module-or-instance selector from a query, or supplies a non-string database filename. |
| `FSIM-COV-040` | error | Standard VPI coverage publication, control, property access, FSM relationship traversal, iterator use, or exact state-value access failed because a handle, owner, relation, metric, statistic, state encoding, callback, or resource bound is invalid. |
| `FSIM-COV-041` | error | A Verilog, SystemVerilog, or VHDL `fsim coverage off/on` source control is malformed, names an unknown metric, omits the required exclusion reason, overlaps an incompatible control, exceeds a resource ceiling, or cannot be applied to its authenticated source. |
| `FSIM-COV-042` | error | An external coverage exclusion has a missing or invalid source, hierarchy, object, metric, or reason selector; duplicates or conflicts with another rule; carries a corrupt plan identity; names an invalid target; or exceeds a rule, text, target, or matching-work ceiling. |
| `FSIM-COV-043` | error | Coverage exclusion persistence or report projection failed because a point, source, instance, metric, reason, external plan, database model, canonical order, or resource ceiling is invalid. |
| `FSIM-COV-044` | error | Coverage source, instance, or combined report-model construction failed because the database or exclusion projection is invalid, a point or identity cannot be owned, or a report resource ceiling is exceeded. |
| `FSIM-COV-045` | error | Coverage text, HTML, or full-fidelity JSON rendering failed because the database-backed report model or requested format is invalid, or deterministic output exceeds its bounded resource contract. |
| `FSIM-COV-046` | error | LCOV or Cobertura projection failed because the database-backed report model, source location, source path, requested format, or bounded line, branch, or output resource contract is invalid. |
| `FSIM-COV-047` | error | Coverage merge or report execution failed because an input database, strict or explicit partial merge, deterministic report rendering, atomic destination replacement, metric threshold, or output operation is invalid; unmet thresholds return the dedicated CI exit status after publishing the report. |
...
| `FSIM-SDF-ENDPOINT-006` | error | An SDF IOPATH or timing check matches multiple elaborated targets after exact instance, selection, role, edge, and condition filtering. |
...
| `FSIM-SV-PARSE-374` | error | A named checker connection omits its closing parenthesis. |
| `FSIM-SV-PARSE-375` | error | A checker connection list omits its closing parenthesis. |
| `FSIM-SV-PARSE-376` | error | A checker instantiation omits its terminating semicolon. |
| `FSIM-SV-PARSE-377` | error | A `unique` constraint omits its opening brace. |
| `FSIM-SV-PARSE-378` | error | A `unique` constraint has no variable item. |
| `FSIM-SV-PARSE-379` | error | A `unique` constraint omits its closing brace. |
| `FSIM-SV-PARSE-380` | error | A `unique` constraint omits its terminating semicolon. |
| `FSIM-SV-PARSE-381` | error | A program elaboration-severity item or anonymous-program interface class is used before the SystemVerilog-2023 profile. |
| `FSIM-SV-PARSE-382` | error | Reserved program-item recovery identity. |
| `FSIM-SV-PARSE-383` | error | An anonymous program `interface` item is not followed by a class declaration. |
...
| `FSIM-SV-SEM-250` | error | A deferred immediate assertion action is not a permitted null, severity-system-task, or observed subroutine-call action. |
| `FSIM-SV-SEM-251` | error | A concurrent `sequence` directive is used with `assert`, `assume`, or `restrict` instead of `cover`. |
| `FSIM-SV-SEM-252` | error | A checker connection list mixes ordered and named forms. |
| `FSIM-SV-SEM-253` | error | A checker instance or named connection does not resolve to a visible declaration or formal. |
| `FSIM-SV-SEM-254` | error | A checker formal is connected more than once. |
| `FSIM-SV-SEM-255` | error | A checker instance has too many ordered connections or leaves a required formal unconnected. |
| `FSIM-SV-SEM-256` | error | A design unit repeats a checker instance name. |
| `FSIM-SV-SEM-257` | error | `option.cross_retain_auto_bins` is used before the SystemVerilog-2023 profile. |
| `FSIM-SV-SEM-258` | error | `cross_retain_auto_bins` is not assigned once as the bounded instance option bit. |
| `FSIM-SV-SEM-259` | error | A covergroup extension is used before the SystemVerilog-2023 profile. |
| `FSIM-SV-SEM-260` | error | A covergroup extension is declared outside a class. |
| `FSIM-SV-SEM-261` | error | A covergroup extension adds a constructor or sampling profile instead of inheriting the parent profile. |
| `FSIM-SV-SEM-262` | error | A covergroup extension has no same-named declaration in its parent-class chain. |
| `FSIM-SV-SEM-263` | error | Covergroup inheritance contains a cycle. |
| `FSIM-SV-SEM-264` | error | A real-valued coverpoint, tolerance bin, or `type_option.real_interval` is used before the SystemVerilog-2023 profile. |
| `FSIM-SV-SEM-265` | error | `type_option.real_interval` is not assigned once as a finite positive type option in its allowed scope. |
...
| `FSIM-ELAB-SVIFACE-012` | error | A virtual-interface assignment has a different interface specialization, widens a selected modport, rebinds it to another modport, or has a non-interface source. |
...
| `FSIM-SV-SEM-389` | error | A switch-instance array omits its required instance name. |
...
| `FSIM-SV-UNSUPPORTED-025` | error | An anonymous program item is outside the supported empty, task, function, class, and SystemVerilog-2023 interface-class subset. |
...
| `FSIM-ELAB-GEN-014` | error | A loop-generate iteration revisits an earlier genvar value and therefore cannot terminate. |
| `FSIM-ELAB-GEN-015` | error | A selected generated class cannot be attached to its owning SystemVerilog design unit. |
...
| `FSIM-ELAB-SVSPEC-003` | error | A specify path or timing-event terminal is not a module port, or has a direction incompatible with its role. |
...
| `FSIM-ELAB-SVSPEC-015` | error | A timing-check notifier is not a scalar variable. |
