...
| CM-007 | SimIR delayed writes, dynamic waits, yields, and assertions | v1 target | — | — | — | — |
| CM-008 | Timed wait and positive-edge static sensitivity | execute | — | — | [elaborator test](../tests/elaboration/elaborator_test.cpp) | [SimIR tests](../tests/runtime/runtime_tests.cpp) |
...
| CM-015 | LLVM 22.1.8 ORC O0/O2 Logic4 CFGs through versioned plain-C runtime/frame/result ABIs and the shared checked ≤64-bit `Logic4Word` path, including timed-wait/yield/stop resumption and native-object keys limited to referenced signal IDs/widths | execute | [LLVM resume and CFG test](../tests/compiler/llvm_jit_test.cpp), [single-word runtime test](../tests/runtime/runtime_tests.cpp), [C ABI header test](../tests/compiler/jit_runtime_c_test.c) | [typed capability, malformed IR, ABI/layout, symbol, zero-time-cycle, and cache-corruption rejection](../tests/compiler/llvm_jit_test.cpp) | [LLVM adapter test](../tests/compiler/llvm_jit_test.cpp) | [O0/O2 resume/interpreter differential and referenced/unreferenced-width cache tests](../tests/compiler/llvm_jit_test.cpp), [exact-version CI job](../.github/workflows/ci.yml) |
| CM-016 | Interpreter/LLVM differential execution of every semantic simulation test | v1 target | — | — | — | — |
| CM-017 | Application check/build/cache path, native-cache telemetry, O0/O2 interpreter-versus-hybrid execution for a bounded SystemVerilog hierarchy, and O2 differential execution for the bounded SV/VHDL/SV hierarchy; eligible processes compile while typed capability misses remain on the reference evaluator | execute | [application test](../tests/app/application_test.cpp) | — | [application test](../tests/app/application_test.cpp) | [O0/O2 application differential, O2 mixed-language differential, and cold/warm telemetry test](../tests/app/application_test.cpp) |
...
CM-015 is a bounded compiled-engine slice. `WaitFor`, `Yield`, `Stop`, and
loops cut by those suspension points use caller-owned frames; `WriteUpdate`,
`WriteAfter`, `WaitOn`, `WaitSensitivity`, reachable zero-time cycles, and
values wider than 64 bits remain unsupported. LLVM-enabled build/run selects
the adapter cache and falls back only for typed capability misses. Cache
identity includes only signal IDs and widths referenced by the process, and
the application exposes and tests cold/warm cache counters. The current
`fsim debug` path remains interpreter-only. CM-015 and CM-017 provide bounded
single- and mixed-language differential evidence, but do not satisfy CM-016:
the full semantic fixture set, assertion metadata, normalized VCD, and Windows
execution evidence remain open. The release build must use LLVM 22.1.8; a test
compiled against another LLVM version is development evidence only.
...
| V1-CM-02 | Per-specialization resumable LLVM state machines with O2 run/O0 debug semantic equivalence | v1 target | [bounded caller-owned frame ABI](../tests/compiler/llvm_jit_test.cpp) | [frame/layout/capability checks](../tests/compiler/llvm_jit_test.cpp) | — | [bounded O0/O2 resume/interpreter differential](../tests/compiler/llvm_jit_test.cpp), [bounded application interpreter/hybrid differential](../tests/app/application_test.cpp) |
| V1-CM-03 | Complete native-object cache key, corruption recovery, per-key concurrency, and compatibility eviction | v1 target | [hardened cache primitive and canonical LLVM adapter key](../tests/compiler/cache_test.cpp) | [live-lock and LLVM corrupt/incompatible-object rejection](../tests/compiler/llvm_jit_test.cpp) | — | [LLVM O0/O2 cold/warm/corrupt/referenced-width invalidation tests](../tests/compiler/llvm_jit_test.cpp), [application cold/warm telemetry test](../tests/app/application_test.cpp) |
...
| V1-CM-05 | Driver transactions, multi-driver resolution, update fanout, and committed-change visibility | v1 target | [single-signal update coalescing only](../tests/runtime/runtime_tests.cpp) | [unimplemented resolver rejection](../tests/elaboration/elaborator_test.cpp) | — | — |
