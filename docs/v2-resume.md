...
9. Batch 170 Change 1 is complete in an intentionally dirty worktree. The
   registered seventeen-row `sdf_vital_inventory.tsv` assigns Changes 2-18
   exactly across VHDL/VITAL target plans, paths/checks/models, precedence,
   scheduling, negative checks, reannotation, mixed boundaries, multiple-root
   resolution, foreign interfaces, observability, artifacts, phases, corpora
   and closure. All rows are active with SHA-256 identity
   `22128b976f653acaa41ff468e53fe35aa6812772519317663cc47e5b92a150f0`.
   The exact-LLVM Debug tree regenerates without compilation; inventory,
   diagnostic-catalog and source-budget gates pass 3/3 in 0.26 seconds. The
   refreshed graph has 32,235 nodes and 148,502 edges. No Release build/test,
   sanitizer, hosted-CI inspection or header formatting ran. Preserve Change 1
   and proceed to Change 2 without reset, commit or push.
10. Batch 170 Change 2 is complete in the same intentionally dirty worktree.
    Immutable schema-1 `SdfVitalTargetPlan` records stable VHDL entity/
    architecture, library, instance, exact port type/shape/direction/domain/
    resolution/select and sorted generic value/semantic identities. Six
    cataloged diagnostics reject stale/non-VHDL state, missing specialization,
    interface/generic mismatch, duplicate ownership and resource overflow
    atomically. The warning-clean eight-worker Debug target builds in three
    steps; the focused SDF/VITAL target, resolution, inventory, catalog and
    source slice passes 11/11 in 0.87 seconds. The refreshed graph has 32,321
    nodes and 148,894 edges. The ledger preserves Change 2, keeps sixteen rows
    active and has SHA-256 identity
    ...
    reset, commit or push ran. Preserve Changes 1-2 and proceed to Change 3.
11. Batch 170 Change 3 is complete in the same intentionally dirty worktree.
    Immutable schema-1 `SdfVitalPathTimingPlan` records bind each target to one
    specialization-owned `VitalDelay` or `VitalTimingCheck` process/instruction
    identity and retain endpoint signals, edge roles, conditions, source
    coordinates and exact static before/selected after tick values. Six
    cataloged diagnostics reject stale targets, missing/ambiguous calls,
    timing-kind mismatch, arity, non-static source values and resource or
    ownership overflow without partial publication. The warning-clean eight-
    worker Debug target rebuilds in three steps; the focused test passes 1/1
    and the preserved SDF/VITAL application, inventory, diagnostic and source
    slice passes 11/11 in 0.89 seconds. The refreshed graph has 32,397 nodes and
    149,230 edges; the public builder has cyclomatic complexity 14, cognitive
    complexity 24, three loops, maximum loop depth two and no recursion or
    linear scan in a loop. The ledger preserves Changes 2-3, keeps fifteen rows
    active and has SHA-256 identity
    `7b3ed97ea6a994273315b536e333154d8ce47a22fdb949cc717481cefbbce58d`.
    Formatting touched only changed `.cpp` files; semantic header additions
    were not formatting-churned. No Release build/test, sanitizer, hosted-CI
    inspection, reset, commit or push ran. Preserve Changes 1-3 and proceed to
    Change 4.
12. Batch 170 Change 4 is complete in the same intentionally dirty worktree.
    Immutable schema-1 `SdfVitalModelPlan` records classify specialization-
    owned SimIR structure as primitive, state-table or memory-path behavior,
    retain exact process/call/target/path identities and accept arbitrary
    wrapper names only through explicit governance plus complete port identity,
    signal, role, width, direction and domain profiles. Six cataloged
    diagnostics reject stale inputs, missing process ownership, ambiguous
    state-table/memory shapes, wrapper mismatch or nonuse, duplicate ownership
    and resource overflow without partial publication. An overly explicit
    regeneration command invalidated the exact-Clang Debug cache; it was
    restored with Clang 22.1.8 pinned to the complete GCC 13 runtime and the
    dependency target completed after one compile-only fixture namespace
    correction. Pre-existing warnings appeared in unrelated rebuilt units; the
    new Change 4 sources are warning-clean. The focused model test passes 1/1
    and the preserved SDF/VITAL application, inventory, diagnostic and source
    slice passes 12/12 in 0.86 seconds. The refreshed graph has 32,494 nodes and
    149,700 edges; the public builder has cyclomatic complexity 26, cognitive
    complexity 41, six single-depth loops and no recursion. Its four graph-
    reported lookup-in-loop sites are keyed hash/set lookups rather than
    sequence scans. The ledger preserves Changes 2-4, keeps fourteen rows
    active and has SHA-256 identity
...
13. Batch 170 Change 5 is complete in the same intentionally dirty worktree.
    Immutable schema-1 `SdfVitalPrecedenceApplication` starts from stable
    source VITAL call/value records, applies exact timing-generic base
    overrides, then evaluates absolute and incremental revisions in file order.
    Per-revision and global delay/check controls retain disabled steps in the
    audit trail without changing effective values; signed timing checks, exact
    min/typ/max command selection and fully disabled/no-file identity remain
    explicit. Six cataloged diagnostics reject incomplete source plans, stale
    revisions, duplicate or stale generics/controls, missing application modes,
    selection mismatch, overflow and resource exhaustion atomically. The
    eight-worker exact-Clang Debug target builds in twelve steps after one
    narrow semantic header-include correction. The focused precedence test
    passes 1/1 and the preserved SDF/VITAL application, inventory, diagnostic
    and source slice passes 13/13 in 1.40 seconds. The refreshed graph has
    32,598 nodes and 150,160 edges; the public builder has cyclomatic complexity
    ...
    ledger preserves Changes 2-5, keeps thirteen rows active and has SHA-256
    identity
    ...
    proceed to Change 6.
14. Batch 170 Change 6 is complete in the same intentionally dirty worktree.
    Immutable schema-1 `SdfVitalSchedulingApplication` validates every stable
    delay call, process, instruction, shape, transition mode, source value and
    effective value before rewriting a copied elaborated design. It preserves
    native VITAL transport, inertial, on-detect/on-event, fast-reject and
    negative-preemption behavior and publishes exact source/effective ticks,
    endpoints and provenance. Direct static loads and lowered packed-constant
    `Extract` definitions are both supported; this correction also makes
    Change 3 source-value capture work against real lowered VHDL rather than
    manual fixtures alone. Six cataloged diagnostics reject incomplete
    ...
    overflow atomically. The warning-clean eight-worker Debug target rebuilds
    in ten steps after compile-only fixture and extracted-constant corrections.
    The focused scheduler test passes 1/1 in 0.04 seconds, including a real
    compiled VHDL VITAL path whose annotated ticks 9 and 29 and nine-state `Z`
    transition match exactly between interpreter and LLVM. The preserved SDF/
    VITAL slice passes 14/14 in 1.45 seconds. The refreshed graph has 32,669
    nodes and 150,627 edges; the public builder has cyclomatic complexity 13,
    cognitive complexity 18, three single-depth loops and no recursion. Its
    reported lookup sites are keyed maps/sets; no runtime hierarchy lookup was
    added. The ledger preserves Changes 2-6, keeps twelve rows active and has
    SHA-256 identity
    `67a512b9e6ed004aa4eeb78f9c4d495233c0581efe6fce384f3c60ea87f77e00`.
    Formatting touched only changed `.cpp` files; the semantic scheduling
    header was not formatting-churned. No Release build/test, sanitizer,
    hosted-CI inspection, reset, commit or push ran. Preserve Changes 1-6 and
    proceed to Change 7.
15. Batch 170 Change 7 is complete in the same intentionally dirty worktree.
    Immutable schema-1 `SdfVitalTimingCheckApplication` transactionally
    rewrites the four retained limits for setup/hold, recovery/removal,
    period/pulse and both in-/out-phase skew calls while preserving test/
    reference/trigger endpoints, edge and condition identities, per-role
    enables, `CheckEnabled`, `XOn`, `MsgOn`, severity, message and both VHDL
    violation and SDF annotation coordinates. Six cataloged diagnostics reject
    incomplete scheduling, stale ownership or arity, missing/noncontiguous/
    negative limits, unsupported kinds, invalid publication and resource
    overflow without partial state. The warning-clean eight-worker Debug target
    links after one compile-only fixture correction. The focused test passes
    1/1 in 0.01 seconds, publishes all five runtime check families, rejects a
    negative effective limit and executes one annotated period/pulse violation
    at tick 10 with its exact message. The preserved SDF/VITAL slice passes
    15/15 in 1.43 seconds. The refreshed graph has 32,726 nodes and 150,994
    edges; the public builder has cyclomatic complexity 13, cognitive
    complexity 18, three single-depth loops and no recursion. Its reported
    lookup sites are keyed maps/sets. The ledger preserves Changes 2-7, keeps
    eleven rows active and has SHA-256 identity
...
16. Batch 170 Change 8 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfVitalReannotationApplication` validates complete delay/check
    call topology before deterministic explicit file/cell precedence selection
    across multiple roots and full/relative wildcard cells. It retains exact
    timing-generic provenance, ordered revisions and one semantic generation;
    duplicate/conflicting precedence, stale static definitions, unmatched
    scopes and resource overflow reject without mutating the baseline. Live
    commit is legal before start or at a scheduler safe point, explicitly
    preserves already-projected transactions or rejects pending work, and
    preserves or resets VITAL timing-check history. The interpreter first
    validates all check limits and all direct or extracted delay definitions,
    including duplicate/conflicting replacement detection, then performs its
    nonthrowing mutation phase. The focused Debug test passes 1/1 in 0.02
    seconds and proves multi-file/root wildcard ordering, generic provenance,
    rollback, outside-safe-point rejection, same-safe-point generation,
    pending rejection, an old-timing pending write, a new-timing later write,
    and distinct preserve/reset period-check history. All six VITAL SDF
    application tests pass in 0.10 seconds; the complete Debug SDF gate passes
    42/42 in 63.48 seconds, including closure and inventory. The refreshed
    graph has 32,830 nodes and 151,517 edges. The public transaction builder
    has cyclomatic complexity 28, cognitive complexity 52, eight loops,
    maximum loop depth two and no recursion; runtime commit has complexity 3,
    no loops and no recursion. Both new sources are below 2,000 lines. The
    ledger preserves Changes 2-8, keeps ten rows active and has SHA-256
    identity
    `29ae8d31a6439434c51162164c5c7cdecbf998bf3159f484da38c1bb996d1536`.
    The required runtime/API header additions caused one eight-worker Debug
    dependency rebuild; formatting touched only the three changed `.cpp`
    files and did not format either header. No Release build/test, sanitizer,
    hosted-CI inspection, reset, commit or push ran. Preserve Changes 1-8 and
    proceed to Change 9.

17. Batch 170 Change 9 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfMixedVerilogApplication` publishes exact VHDL-to-Verilog and
    Verilog-to-VHDL INTERCONNECT records from the existing endpoint-conversion
    metadata and elaborated boundary profiles. Each record retains stable
    source/destination and formal/actual signal identities, canonical hierarchy
    paths, exact Logic4/Logic9 domains, widths, signedness, resolved-net kinds,
    adapter process, source-driver and destination-load owners, delay values
    and annotation provenance. VHDL endpoints enter the earlier interconnect
    layer only with complete conversion metadata. Mixed publication validates
    peer identity, endpoint role/language/direction, live signal profiles and
    adapter source-load/destination-drive topology without scheduler or hot-
    path hierarchy lookup. Four cataloged diagnostics reject incomplete,
    stale/ambiguous, topologically incompatible and resource-overflow state
    atomically. The warning-clean eight-worker Debug targets build; the focused
    interconnect/mixed/inventory/catalog/source slice passes 5/5 in 0.48
    seconds and the complete Debug SDF gate passes 43/43 in 64.00 seconds,
    including both retained closure stages. The refreshed graph has 32,913
    nodes and 151,889 edges. The public builder has cyclomatic complexity 12,
    cognitive complexity 21, one single-depth loop and no recursion or linear
    scan in a loop. Both new sources are below 2,000 lines. The ledger preserves
    Changes 2-9, keeps nine rows active and has SHA-256 identity
    `95dcd42180f5280da068b39f805369e8d8fcb6dd82746ec29e543e9c353ba3fd`.
    Formatting touched only the new `.cpp` implementation/test; the new
    semantic header and narrowly changed existing implementation were not
    formatting-churned. No Release build/test, sanitizer, hosted-CI inspection,
    reset, commit or push ran. Preserve Changes 1-9 and proceed to Change 10.

18. Batch 170 Change 10 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfMixedSystemVerilogApplication` binds the SystemVerilog side of
    each Change 9 boundary to exact retained-HIR interface, program, class,
    concurrent-assertion and package-visible owners. Signal, width and domain
    must remain exact. Interface/package/class endpoints remain active,
    programs remain reactive, and assertions preserve preponed sampling,
    observed evaluation and reactive action as three separate identities.
    Four cataloged diagnostics reject incomplete input, stale/duplicate owners,
    HIR/type/region mismatch and resource overflow atomically. Focused evidence
    covers all five owner kinds, stable replay identity, missing ownership,
    implicit widening, program-region drift and bounded rollback. The warning-
    clean eight-worker Debug target builds; the mixed/inventory/catalog/source
    slice passes 5/5 in 0.48 seconds and the complete Debug SDF gate passes
    44/44 in 64.96 seconds, including both retained closure stages. The
    refreshed graph has 32,997 nodes and 152,206 edges. The public builder has
    cyclomatic complexity 9, cognitive complexity 14, one single-depth loop
    and no recursion or linear scan in a loop. Both new sources are below 2,000
    lines. The ledger preserves Changes 2-10, keeps eight rows active and has
    SHA-256 identity
    ...
    semantic header was not formatting-churned. No Release build/test,
    sanitizer, hosted-CI inspection, reset, commit or push ran. Preserve
    Changes 1-10 and proceed to Change 11.

19. Batch 170 Change 11 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfMixedSystemCApplication` binds exact SDF INTERCONNECT targets
    to canonical SystemC named objects, native handles and dense typed signals.
    It retains object/parent/type identity, width/domain/resolution, transition
    delays and full arbitrary-width Logic9 samples ordered by exact tick/delta.
    The interconnect layer now accepts canonical typed SystemC port/signal/
    export endpoints; metadata-only primitive channels reject explicitly
    instead of acquiring fabricated signal semantics. Four cataloged
    diagnostics reject incomplete input, stale/duplicate objects, unsupported/
    type/time mismatch and resource overflow atomically. Focused evidence
    covers a 257-bit Logic9 proxy, exact native/delay/time/delta identity,
    deterministic replay, custom-channel rejection, width and same-delta
    mismatch, and sample limits. The warning-clean eight-worker Debug targets
    build; the interconnect/SystemC/inventory/catalog/source slice passes 5/5
    in 0.48 seconds and the complete Debug SDF gate passes 45/45 in 62.76
    seconds with both retained closure stages. The refreshed graph has 33,068
    nodes and 152,514 edges. The public builder has cyclomatic complexity 11,
    cognitive complexity 18, one single-depth loop and no recursion or linear
    scan in a loop. Both new sources are below 2,000 lines. The ledger preserves
    Changes 2-11, keeps seven rows active and has SHA-256 identity
...
20. Batch 170 Change 12 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfMixedResolutionApplication` consumes the already-resolved
    Change 9-11 Verilog, SystemVerilog-owned and SystemC boundary layers and
    publishes one canonical immutable view across multiple roots, libraries
    and nested language boundaries. Every record retains exact root/library,
    source/destination language and hierarchy path, boundary or proxy target,
    owner/endpoint and lower-layer source identity. Direction must cross a VHDL
    boundary; duplicate direction-aware ownership keys reject, and canonical
    sorting makes source order irrelevant without hierarchy lookup. Four
    cataloged diagnostics reject incomplete layers, stale/duplicate identities,
    invalid direction or ambiguity and resource overflow atomically. Focused
    evidence covers two roots, distinct libraries, a Verilog boundary, a nested
    SystemVerilog interface owner and a native SystemC proxy, plus replay
    ...
    resolution/inventory/catalog/source slice passes 7/7 in 0.52 seconds and
    the complete Debug SDF gate passes 46/46 in 62.71 seconds with both retained
    ...
    single-depth loop and no recursion or linear scan in a loop. Both new
    sources are below 2,000 lines. The ledger preserves Changes 2-12, keeps six
    rows active and has SHA-256 identity
...
21. Batch 170 Change 13 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfForeignInterfaceApplication` publishes effective VITAL
    delays/checks and mixed-boundary transition values as sorted immutable
    VHPI/VPI timing objects. Each ABI receives deterministic dense handles,
    while canonical identity retains interface/timing kind, generation, root/
    library, hierarchy path and lower-layer source identity. Mixed resolution
    now carries exact effective ticks forward as required semantic data;
    Verilog/SystemVerilog boundaries publish VHPI and VPI views, while native
    SystemC proxies remain visible from their VHDL VHPI side. Bounded offset
    enumeration, callback registration/removal, exact time/delta delivery and
    read-only deposit/force/release behavior are explicit. Disabled observation
    returns before callback delivery and never touches the scheduler; focused
    evidence retains a queued tick-50 task, time zero and delta zero across the
    disabled publication. Four cataloged diagnostics reject incomplete layers,
    duplicate stable identities, stale/missing effective values and resource
    overflow atomically; callback ceilings reject locally without replacing
    registrations. The warning-clean eight-worker Debug targets build; the
    foreign/mixed/inventory/catalog/source slice passes 5/5 in 0.48 seconds and
    the complete Debug SDF gate passes 47/47 in 63.91 seconds with both retained
    closure stages. The refreshed graph has 33,239 nodes and 153,201 edges. The
    public publisher has cyclomatic complexity 8, cognitive complexity 9, four
    single-depth loops and no recursion or linear scan in a loop. New sources
    are below 2,000 lines. The ledger preserves Changes 2-13, keeps five rows
    active and has SHA-256 identity
    `94e5bca237a69be569069cd4ddd229f7de3bc1daa8f1fbfc4e3f849c051d8392`.
    ...
    sanitizer, hosted-CI inspection, reset, commit or push ran. Preserve
    Changes 1-13 and proceed to Change 14.

22. Batch 170 Change 14 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfVitalObservabilityApplication` maps Change 13's immutable
    foreign timing set into stable debugger, callback, internal-trace and VCD
    objects without copying scheduler ownership. Exact effective values,
    pending transaction publication time and violation before/after values fan
    out atomically to enabled surfaces with one sequence and deterministic time,
    delta and scheduler-region order. VCD names derive from ABI kind plus stable
    handle. Out-of-order coordinates, invalid shapes and partial surface
    overflow reject before publication; a recorder with no enabled surfaces
    reserves zero event slots and returns before lookup. Four cataloged
    diagnostics reject incomplete input, duplicate stable objects, stale
    effective records and object/value/identity overflow. Focused evidence
    covers all four views, effective/pending/violation kinds, future transaction
    time, observed-region order, disabled zero-allocation, coordinate regression
    and capacity rollback. The warning-clean eight-worker Debug target builds;
    the foreign/observability/inventory/catalog/source slice passes 5/5 in 0.48
    seconds and the complete Debug SDF gate passes 48/48 in 64.54 seconds with
    both retained closure stages. The refreshed graph has 33,323 nodes and
    153,548 edges. The public builder has cyclomatic complexity 9, cognitive
    complexity 13, two single-depth loops and no recursion; its two graph-
    reported lookup sites are bounded keyed identity-set operations rather than
    hierarchy or sequence scans. New sources are below 2,000 lines. The ledger
    preserves Changes 2-14, keeps four rows active and has SHA-256 identity
    `e6936ca126912cedfb5f4c2aad4a134ffb63e41cdbbd8f314ce5a87766bbff39`.
    Formatting touched only new `.cpp` implementation/test files; the new
    semantic header was not formatting-churned. No Release build/test,
    sanitizer, hosted-CI inspection, reset, commit or push ran. Preserve
    Changes 1-14 and proceed to Change 15.

23. Batch 170 Change 15 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfVitalArchiveApplication` translates Change 13 foreign timing
    plus Change 8 VITAL state into portable records retaining exact source/
    effective values, roots, hierarchy paths, generation, min/typ/max selection,
    pending/timing-history policies and semantic provenance. The existing
    checksummed effective-archive codec now has a VITAL facade for object,
    design, mapped-library, native-cache and checkpoint forms; all five round-
    trip one identical snapshot, and logical producer identity keeps mapped-
    library bytes stable across physical relocation. Stale kind/schema/checksum
    and corrupt producer/policy/value/provenance mismatches reject
    transactionally through four cataloged diagnostics. Focused evidence covers
    all forms, cache/checkpoint replay, source/effective distinction, relocation,
    corruption, cross-kind mismatch and resource rollback. The eight-worker
    Debug target builds; only two pre-existing deleted-default-equality warnings
    surface through the earlier SDF header chain, while new Change 15 code adds
    no warning. The archive/observability/inventory/catalog/source slice passes
    5/5 in 0.49 seconds and the complete Debug SDF gate passes 49/49 in 64.53
    seconds with both retained closure stages. The refreshed graph has 33,358
    nodes and 153,767 edges. The public builder has cyclomatic complexity 12,
    cognitive complexity 24, one single-depth loop and no recursion or linear
    scan in a loop. New sources are below 2,000 lines. The ledger preserves
    Changes 2-15, keeps three rows active and has SHA-256 identity
    `140966b8eca3aa54fdd96435995a55c254229d29aa34bd8e0f9d6b4c27cc84b5`.
    Formatting touched only new `.cpp` implementation/test files; the new
    semantic header was not formatting-churned. No Release build/test,
    sanitizer, hosted-CI inspection, reset, commit or push ran. Preserve
    Changes 1-15 and proceed to Change 16.

24. Batch 170 Change 16 is complete in the same intentionally dirty worktree.
    Schema-1 `SdfVitalPhaseApplication` projects one immutable Change 15
    effective archive through project, CLI, Tcl, C API, C++ API and direct
    non-project controls. The full 36-case surface/cold-warm-relocated/
    interpreter-LLVM matrix retains identical archive bytes, archive identity,
    effective timing controls and behavior identity while each summary reports
    its exact entry surface, execution phase and engine. Unsupported controls
    produce the same code and message at all six surfaces; incompatible archive
    and archive/summary resource failures roll back without a partial
    application. Four cataloged diagnostics cover incomplete input, unsupported
    controls, archive incompatibility and resource exhaustion. The eight-worker
    Debug target builds; only the same two pre-existing deleted-default-equality
    warnings surface through the earlier SDF header chain. The archive/phase/
    inventory/catalog/source slice passes 5/5 in 0.49 seconds and the complete
    Debug SDF gate passes 50/50 in 64.18 seconds with both retained closure
    stages. The refreshed graph has 33,427 nodes and 154,032 edges. The public
    builder has cyclomatic and cognitive complexity 4 with no loops, recursion
    or linear scan in a loop. New sources are below 2,000 lines. The ledger
    preserves Changes 2-16, keeps two rows active and has SHA-256 identity
    `62a97f286bfa870aa53862b4ecff7d2999d227da7b144f9045ac923e67a1b125`.
    Formatting touched only new `.cpp` implementation/test files; the new
    semantic header was not formatting-churned. No Release build/test,
    sanitizer, hosted-CI inspection, reset, commit or push ran. Preserve
    Changes 1-16 and proceed to Change 17.

25. Batch 170 Change 17 is complete in the same intentionally dirty worktree.
    Eleven Apache-2.0-owned SDF, VHDL, Verilog, SystemVerilog and SystemC
    fixtures cover SDF 2.1/3.0/4.0, VHDL 87/93/2000/2002/2008, standard-cell,
    primitive, state-table, memory and wrapper models, and both directions of
    every VHDL-Verilog/SystemVerilog/SystemC boundary. The corpus runs the exact
    2,700-case cross-product through interpreter/LLVM and cold/warm/relocated
    execution and proves identical behavior identity and archive bytes within
    each semantic case. Its dedicated closure driver retains console, result
    and corpus logs and rejects missing fixtures or missing revision/model/
    direction/engine/phase/time-advance/PASS/clean-exit markers. The eight-
    worker Debug target builds with only the same two pre-existing warnings
    from the earlier SDF header chain. The corpus/closure/inventory/source/
    resource slice passes 5/5 in 0.35 seconds and the complete Debug SDF gate
    passes 52/52 in 64.86 seconds. The refreshed graph has 33,437 nodes and
    154,117 edges; fixture sources are intentionally excluded from indexing.
    The bounded test-only six-dimensional matrix driver has cyclomatic
    complexity 7, cognitive complexity 28, six nested constant-size loops and
    no recursion or linear scan in a loop; no production hot path was added.
    New sources are below 2,000 lines. The ledger preserves Changes 2-17, keeps
    one row active and has SHA-256 identity
...
26. Batch 170 Change 18 is complete in the same intentionally dirty worktree.
    The SDF/VITAL closure now runs the positive corpus, four governed negative
    applications and the Linux/Windows resource-portability contract as three
    separately retained nonempty logs with a TSV PASS result. Ambiguous state-
    table/memory shape, wrapper mismatch, unsupported unowned model process,
    identity overflow and zero resources reject atomically. The corpus also
    re-proves cross-artifact mismatch, unsupported phase control, summary
    overflow and archive exhaustion, plus a real two-root effective archive
    round-tripped through object, design, mapped-library, native-cache and
    checkpoint forms. Closure requires both engines, every phase and artifact,
    multiple roots, both platform contracts, 2,700 time advances, PASS and
    clean exit. The eight-worker Debug targets build with only the same two pre-
    existing warnings from the older SDF header chain. The models/corpus/
    closure/inventory/source/resource slice passes 6/6 in 0.48 seconds and the
    complete Debug SDF gate passes 52/52 in 63.76 seconds. The refreshed graph
    has 33,438 nodes and 154,153 edges. The test-only application closure helper
    has cyclomatic and cognitive complexity 1, one single-depth five-element
    artifact loop and no recursion or linear scan in a loop. New sources are
    below 2,000 lines. The 17-row ledger has zero active obligations and SHA-256
    identity
...
27. Batch 170 Change 19 is complete in the same intentionally dirty worktree.
    README, SDF, language-support, architecture and feature-matrix docs now
    describe the completed VHDL/VITAL and bidirectional VHDL-Verilog/
    SystemVerilog/SystemC timing boundary. A clean-room mixed VHDL/
    SystemVerilog/SDF/Tcl example, zero-active release audit and machine-
    checked documentation/license/resource contract bind the same profiles,
    models, public surfaces, artifacts, negative families and closure tokens.
    The Batch 171 restart checkpoint below preserves the zero-active ledger,
    schemas, 11 fixtures, 2,700-case corpus, retained logs, eight-worker rule,
    final-check-only Release policy and no header-formatting churn. Static
    inventory is 2,461 diagnostics, 1,020 bounded sources and 1,205 Apache-2.0-
    owned files. The documentation/release slice passes 7/7 in 1.60 seconds and
    the complete Debug SDF gate passes 53/53 in 64.42 seconds. The refreshed
    graph has 33,440 nodes and 154,158 edges; docs/examples/fixtures are
    intentionally excluded from code indexing. The ledger remains 17/17
    preserved and zero active at SHA-256
    `3e84f643e6df24090efb1161e0d6836847784268a139e3da3c2fda4568913c6f`.
    Change 19 touched documentation, examples, CMake contracts and tests only;
    no semantic header formatting ran. No Release build/test, sanitizer,
    hosted-CI inspection, reset, commit or push ran. Preserve Changes 1-19 and
    run the complete Change 20 final checks.

28. Batch 170 Change 20 is complete. The LLVM-disabled GCC ASan/UBSan clean
    build completed 888/888 in 29:39.40; every affected test host was rebuilt
    after the phase-isolation, LLVM-off and governed-limit repairs, and the
    definitive regression passed 204/204 in 2,141.52 seconds (35:41.55) with
    4,372,608 KiB peak RSS and no sanitizer finding. Leak detection was
    disabled for the managed runner and allocator quarantine/history were
    bounded. The final eight-worker exact-LLVM Debug clean-first build passed
    1,810/1,810 steps in 14:13.67 and its complete regression passed 205/205 in
    981.31 seconds. The required final Release clean-first rebuild passed
    919/919 retained-dependency steps in 11:47.29 and its complete regression
    passed 205/205 in 858.07 seconds. Both clean logs contain no warning or
    error. Static evidence remains 2,461 diagnostics, 1,020 bounded authored
    sources, 1,205 Apache-2.0-owned files and 379 test/control files. The
    refreshed graph contains 33,521 nodes and 154,412 edges. The 17-row ledger
    remains zero-active at SHA-256
    `3e84f643e6df24090efb1161e0d6836847784268a139e3da3c2fda4568913c6f`.
    Release ran only at the final checks. No header formatting churn occurred;
    the sole header edit is the functional ASan guard on the governed test-
    process ceiling. The single Batch 170 closeout commit/push carries this
    evidence. Verify every non-documentation hosted-CI job on that pushed
    commit is green, then start Batch 171 only from the clean synchronized
    closeout.

## Batch 171 planned restart checkpoint - 2026-08-13

1. Start in `/home/colin/projects/fsim`, read this section and the locked Batch
   171 allocation in `docs/implementation_plan_v2.md`, then verify branch
   `codex/v2` is clean and synchronized at the pushed Batch 170 closeout commit
   recorded by Batch 170 Change 20 below. Do not use stale conversational state
   as an implementation handoff.
2. Preserve Batch 170's zero-active 17-row SDF/VITAL ledger with SHA-256
   `3e84f643e6df24090efb1161e0d6836847784268a139e3da3c2fda4568913c6f`,
   schema-1 archive/phase identities, all 11 owned corpus fixtures, the exact
   2,700-case transcript and three retained-log closure stages.
3. Batch 171 is FST tracing closure only. Expand its locked grouped 1-20
   allocation into exact changes in the authoritative plan and this handoff
   before implementation; do not broaden it into waveform GUI, database or
   unrelated trace work.
4. Preserve existing VCD and internal-trace behavior while adding the FST
   writer, values, aliases, mixed roots, callbacks, selective/late tracing,
   public controls, artifacts, relocation, compression, reader equivalence,
   negatives, docs and release evidence assigned to Batch 171.
5. Use at least eight workers for local builds. Release build testing is not
   required except during final Batch 171 checks. Avoid header formatting
   changes that would induce long rebuilds. Batch 171 is not a sanitizer or
   hosted-CI monitoring boundary.
6. Do not reset, commit or push during Changes 1-19. Change 20 owns the complete
...
1. Start in `/home/colin/projects/fsim`, read this section and the locked Batch
   ...
   `codex/v2` is clean and synchronized at pushed Batch 171 closeout commit
   `8c2c89ed678a044d8f522b83aafe7b9657c7354f`. Do not use stale conversational
   state as an implementation handoff.
...
16. The grouped Batch 172 allocation is expanded into exactly twenty bounded
    changes before semantic implementation. Change 1 registers an eighteen-row
    one-to-one ownership ledger for Changes 2-19 covering official provenance,
    one runtime, ABI/cache identity, the opaque protocol/session/execution/
    loopback/safe-point seams, arbitrary-width codecs, TLM 1/2, complete
    inventory/trace/debug observation, compatibility/artifacts, corpus/resources
    and documentation/closure. Preserve this planning-only Change 1 in the
    intentionally dirty worktree and do not begin Change 2 until its ledger and
    registration are validated. No workflow audit or hosted-CI action,
    inspection, restart or monitoring is permitted under the current no-CI
    instruction.
17. Change 1 is complete without semantic implementation. The registered
    eighteen-row `systemc_accellera_inventory.tsv` assigns Changes 2-19 exactly
    across official source provenance, the one-runtime build, compatibility/
    ABI/cache identity, backend protocol/session/execution/loopback/safe-point
    seams, arbitrary-width codecs, TLM 1/2, signal/channel and binding/alias
    inventories, trace/debug transaction observation, compatibility/artifacts,
    corpus/resources and documentation/closure. All eighteen rows are active
    at normalized SHA-256
    `a31b86ab78bcfd9862477073217fff40ec5b4cbc9f6c199f88946eaf37a371ad`.
    The direct workflow-independent contract passes; the refreshed graph has
    34,911 nodes and 162,000 edges. No semantic SystemC code, Release build/test,
    sanitizer, workflow audit, hosted-CI action, reset, commit, push or header
    formatting ran. Preserve Change 1 in the intentionally dirty Batch 172
    worktree and implement only Change 2's official upstream provenance next.
...
18. The authoritative plan labels Batch 172 a CI monitoring boundary. Keep
    hosted CI untouched through Changes 1-19. At Change 20, before the sole
    implementation push, inspect the most recent CI log and reproduce/fix its
    errors locally; after all final local gates pass, push and monitor the new
    run. Do not inspect, monitor, restart, cancel or modify hosted CI earlier.
19. Batch 171 Change 20 must replace this planned handoff's closeout reference
    with the exact synchronized commit and retained final-check evidence before
    Batch 172 implementation begins.
20. Change 2 is complete in the same intentionally dirty worktree. The official
    Accellera `3.0.2` tag archive is vendored unmodified at 4,573,759 bytes with
    release commit `70b0fc8e4a74acc677b0fc73cea08f940c2115d5`, SHA-256
    `9b3693ed286aab958b9e5d79bb0ad3bc523bbc46931100553275352038f4a0c4`
    and deterministic 4,456-file tree identity
    `b1fbb8b7bcb76e3f803155585f2c3e1073e4c990de72302040cfab283c9f9fd0`.
    Its byte-identical Apache-2.0 license/notice, source manifest and SPDX 2.3
    component are retained beside the archive. Configure validates the exact
    offline archive before any SystemC target; isolated materialization checks
    all files and emits a reproducible manifest. Focused negative evidence
    rejects corrupt, wrong-version and incomplete inputs and unsafe work roots.
    The warning-free Debug tree regenerates without compilation, the registered
    provenance/inventory tests pass 2/2 in 0.56 seconds, and a separate corrupt
    archive configure fails at the intended pre-target validation. The ledger
    has one preserved and seventeen active rows at normalized SHA-256
    `713a4dfd5019ce8f17027596975a558d5ed74f8bd68bd4a3efe758f8f45864e1`.
    No upstream runtime has been built or linked yet. No Release test,
    sanitizer, workflow/hosted-CI activity, commit, push or header formatting
    ran. Preserve Changes 1-2 and implement only Change 3's one-shared-runtime
    build boundary next.
21. Change 3 is complete in the accumulated worktree. The official target now
    builds one shared C++20 SystemC 3.0.2 runtime plus one versioned fsim bridge;
    the runtime identity includes the pinned archive digest, compiler and
    standard and rejects null, wrong-version and wrong-compiler identities.
    A configure-time inventory proves all 124 non-interface fsim binaries,
    libraries and plug-ins carry the bridge, and direct source/incremental
    plug-in linkers add the same governed library. On Linux, all seven configured
    SystemC plug-in modules and the compiler consumer name one bridge DSO, the
    bridge resolves one `libsystemc.so.3.0`, and direct and bridge callers share
    the same current `sc_simcontext`. The staged install retains byte-identical
    official `systemc`, `systemc.h`, `tlm` and `tlm.h`, exact relocatable CMake
    and pkg-config metadata, relative DSO/executable runpaths and one resolved
    upstream runtime. Its independent installed consumer configures and builds
    with eight workers against exact package versions
    `SystemCLanguage 3.0.2.20251031` and `SystemCTLM 2.0.6.20191203`, then runs
    successfully. The warning-free Debug build and focused shared-runtime,
    provenance, plug-in, incremental, matrix, application and typed-boundary
    gates pass 13/13 in 95.00 seconds. The ledger has two preserved and sixteen
    ...
    No Release build/test, sanitizer, CI/workflow activity, commit, push or
    header formatting ran. Preserve Changes 1-3 and implement only Change 4's
    compatibility, ABI and cache identity boundary next.
22. Change 4 is complete in the same intentionally dirty worktree. Public fsim
    compatibility headers now use official Accellera definitions by default;
    the temporary custom implementation is reachable only through an explicit
    legacy execution adapter. The installed adapter retains typed factory, HDL
    proxy and export source forms, and its exact upstream SHA-256, bridge
    revision 1 and compiler-specific standard-library identity join the runtime
    in one compatibility fingerprint. Plug-in ABI 2 and source/incremental
    compile/link keys carry that identity. Direct plug-ins, incremental caches,
    mapped libraries and embedded designs reject or fall back from internally
    consistent producers whose runtime/SystemC ABI, target or upstream/compiler/
    stdlib/bridge fingerprint is stale. The compatibility archive and exact
    adapter header install beside the one shared runtime. The warning-free full
    Debug build completes; the focused installed/runtime/header/ABI/compiler/
    incremental/mapped/design/application slice passes 18/18 in 60.93 seconds,
    and inventory/catalog/source gates pass 3/3 in 0.33 seconds. The refreshed
    graph has 35,002 nodes and 162,311 edges; the loader has cognitive
    complexity 13 with no loop or recursion. The ledger preserves Changes 2-4
    with fifteen active rows and digest
    `cbbf2cbd3b9aa2ae07f16d2f631b9993345fe24c5c1fb48856a95030b2771a71`.
    Required header changes were semantic and formatting touched only changed
    implementation/test sources. No Release build/test, sanitizer, CI/workflow
    activity, reset, commit or push ran. Preserve Changes 1-4 and implement only
    Change 5's bounded backend message and stable identity boundary next.
23. Change 5 is complete in the accumulated worktree. The opaque
    `SystemCKernelBackend` accepts and returns only encoded bytes plus transport
    status. Domain-separated SHA-256 identities provide typed stable island,
    hierarchy, object, endpoint and transaction IDs; nonzero sequence IDs are
    island scoped. Schema 1 freezes a 128-byte canonical little-endian header,
    fifteen operation identities, direction/status/correlation rules, flags,
    reserved zero bytes and exact payload length. Governed identity, payload
    and message ceilings reject invalid parents, zero sequences, incomplete
    chains, unknown fields, malformed/trailing/reserved encodings and resource
    excess under three cataloged diagnostics. A source contract proves the
    public boundary contains no upstream SystemC/TLM type, object/channel/socket
    pointer, coroutine or thread handle, and exact identity/message hashes
    freeze the encoding. The warning-free full Debug build completes; protocol,
    installed, runtime, artifact, application, diagnostics and source evidence
    passes 9/9 in 55.02 seconds. The graph has 35,124 nodes and 162,705 edges;
    the loop-free validator has cognitive complexity 16 and the decoder has
    complexity 4 with no loop or recursion. The ledger preserves Changes 2-5
    with fourteen active rows and normalized SHA-256
    `817f1eadb7d4957f576cba04dd32da2a5c95e0d2e0a780163e1e0ea032fe0733`.
    Formatting touched only the new header/implementation/test. No Release
    build/test, sanitizer, CI/workflow activity, reset, commit or push ran.
    Preserve Changes 1-5 and implement only Change 6's one-session lifecycle,
    rollback and teardown boundary next.
24. Change 6 is complete in the same intentionally dirty worktree. Bounded
    schema-1 payloads describe session identity and plug-in path, native roots,
    typed construction parameters and endpoint/interface binding; lifecycle
    receipts retain staged/published counts, context generation, terminal
    state and one of four stable diagnostic classifications. One backend owns
    one official `sc_simcontext` and factory registry, swaps that context only
    under the serialized upstream global-context guard, and exposes no native
    type through its public protocol/session headers. `backend_port` performs
    real typed upstream binding. Official elaboration, start and initial delta
    draining reach zero-time quiescence before staged roots publish. Repeated
    session/object construction and native factory, binding, elaboration or
    start failures roll back every object, plug-in and context; terminal close
    is idempotent. The focused test covers malformed/trailing/duplicate and
    resource payloads, parameter application, nested hierarchy names, a
    throwing factory, missing binding, unbound elaboration, lifecycle callbacks,
    initial evaluation, same-named roots in two simultaneously live contexts,
    independent teardown and clean recovery. The warning-free eight-worker
    full Debug build completes in 113 steps. The protocol/session/runtime/
    compatibility/install/artifact/application/catalog/source/inventory/
    contract slice passes 12/12 in 59.85 seconds, followed by a post-format
    5/5 gate in 0.49 seconds. The graph has 35,307 nodes and 163,353 edges;
    session methods have maximum cognitive complexity 19, loop depth one, no
    nested loop and no recursion. The ledger preserves Changes 2-6 with
    thirteen active rows and normalized SHA-256
    `f8f1a38518c01dc945f542041e9f563ddf4f3d3e4589a579e4e1a6cbc176e721`.
    Formatting touched only the new header/implementation/tests. The existing
    adapter-header edit was semantic and was not reformatted. No Release
    build/test, sanitizer, CI/workflow activity, reset, commit or push ran.
    Preserve Changes 1-6 and implement only Change 7's bounded execution
    protocol next.
25. Change 7 is complete in the same intentionally dirty worktree. A public
    schema-1 execution header carries bounded scalar input and exact delta/time
    advance payloads plus ordered receipts for next activity, dirty output,
    report, inspection and snapshot operations. No upstream kernel, process,
    channel, socket, coroutine or thread object crosses the boundary. Each
    receipt orders exact femtosecond time, delta, Accellera region, island and
    nonzero result sequence. The official adapter now applies or samples
    integral values through real bound signal interfaces with exact width and
    signedness; arbitrary-width two-/four-/nine-state values remain owned by
    Change 10. Official `sc_start` reaches bounded delta quiescence or an exact
    time boundary, reports absolute next activity, and distinguishes safe,
    paused, stopped and failed outcomes. Dirty output values drain exactly once;
    inspection, snapshot and bounded reports remain available at safe points.
    A native execution exception removes every root, plug-in and context and
    returns an error receipt; explicit stop remains observable until idempotent
    teardown. Direct tests cover malformed/trailing/noncanonical payloads,
    ordering, wrong directions, delta/time and future activity, dirty-once
    behavior, report/inspect/snapshot, pause/resume, stop, exception rollback,
    resource limits and clean context recovery. The warning-free eight-worker
    full Debug build completes in 121 steps. The dependent installed/runtime/
    protocol/session/execution/artifact/application/catalog/source/inventory/
    contract slice passes 12/12 in 60.03 seconds; the final post-documentation
    rerun passes 12/12 in 59.93 seconds. The refreshed graph has 35,514 nodes
    and 164,183 edges; native advance has maximum cognitive complexity 16, loop
    depth one, no
    nested loop and no recursion. The ledger preserves Changes 2-7 with twelve
    active rows and normalized SHA-256
    ...
    changed session implementation; the required existing adapter-header edit
    was semantic and was not reformatted. No Release build/test, sanitizer,
    CI/workflow activity, reset, commit or push ran. Preserve Changes 1-7 and
    implement only Change 8's serialized loopback transport, replay ordering
    and failure-containment boundary next.
26. Change 8 is complete in the accumulated Batch 172 worktree. One public
    loopback wrapper forwards copied request bytes to any serialized backend
    and returns copied response bytes; the session factory therefore exercises
    the identical protocol path intended for a future worker without adding a
    launcher or partitioner. Protocol-valid responses may use an independent
    sequence but must correlate to the request and preserve its operation and
    complete typed identity chain. Per-island request sequences reject mutated
    duplicates, stale, skipped and exhausted order; byte-identical replayable
    duplicates return the cached deterministic response without peer execution.
    Replay entries, buffers and forwarded exchanges are bounded with stable
    FIFO eviction. Disconnect, malformed/rejected/excessive response and wrong
    correlation/identity all close and clear the wrapper; a new wrapper proves
    clean recovery. Tests compare direct and loopback real-session lifecycle and
    execution receipts across two official contexts and separately cover byte
    forwarding, independent response sequences, replay, eviction, ordering,
    bounds and contained peer failures. The warning-free eight-worker full
    Debug rebuild completes in 68 steps, and the dependent artifact/catalog/
    source/inventory/contract/install/runtime/protocol/session/execution/
    loopback/application slice passes 13/13 in 67.74 seconds. The graph has
    35,614 nodes and 164,606 edges; implementation maximum cognitive complexity
    is 11 with no loops or recursion. The ledger preserves Changes 2-8 with
    eleven active rows and normalized SHA-256
    ...
    ran. Preserve Changes 1-8 and implement only Change 9's exact Accellera
    safe-point crossing and batched mixed-HDL synchronization boundary next.
27. Change 9 is complete in the same intentionally dirty worktree. A public
    transport-neutral synchronizer accepts only caller-owned absolute time/delta
    safe points; it contains no scheduler. Before execution, each attachment
    freezes a stable island, Verilog/SystemVerilog/VHDL host identity, next
    serialized request sequence, current time and sorted pre-resolved output
    endpoints. Each call sorts and applies the complete HDL input batch, moves
    every official context to the same exact time barrier, then drains native
    evaluate/update/notification work to quiescence before any dirty output is
    published. A flattened endpoint list avoids per-event hierarchy lookup.
    Crossing records retain input, time-arrival, kernel-quiescent and output
    stages; dirty samples retain native update-region, time, delta, island and
    result order. Final receipts merge activity and earliest-next-time only from
    quiescent islands. Invalid/duplicate inputs, stale points, late attachment
    and resource excess reject without mutation; any peer, receipt, exact-time
    or update-order failure closes every island with no partial receipt. The
    focused test uses one direct SystemVerilog-owned context and one loopback
    VHDL-owned context, intentionally reverses attachment and input order, and
    proves deterministic delta/time barriers, values, dirty-once output, next
    activity, nonterminal validation recovery and terminal disconnect
    containment. The warning-free eight-worker full Debug rebuild completes in
    69 steps; the dependent artifact/catalog/source/inventory/contract/install/
    runtime/protocol/session/execution/loopback/synchronization/application
    slice passes 14/14 in 66.91 seconds. The graph has 35,746 nodes and 165,126
    edges; implementation maximum cognitive complexity is 11, loop depth one,
    with no recursion. The ledger preserves Changes 2-9 with ten active rows and
    normalized SHA-256
    ...
    ran. Preserve Changes 1-9 and implement only Change 10's bounded arbitrary-
    width two-state, four-state and VHDL nine-state crossing codec next.
28. Change 10 is complete in the same intentionally dirty worktree. A canonical
    schema-1 bounded value codec retains arbitrary-width bit2, logic4 and VHDL
    logic9 planes, signedness, exact ascending/descending signed ranges,
    enumeration tables and nonzero femtosecond time units. Execution schema 2
    embeds those values while retaining the scalar source interface. Public
    Accellera adapters drive and sample `sc_bv<129>` and `sc_lv<257>` through
    real direct and serialized-loopback sessions; inputs, post-update dirty
    outputs, inspection and snapshots remain exact. Generated 129/130/257-bit
    cases cover every logic9 state without oversized string literals. Protocol,
    artifact and the real 137-bit mixed-language typed-boundary application
    retain every limb and unknown plane. Four cataloged diagnostics reject
    unsupported/lossy projection, malformed/noncanonical metadata and padding,
    truncation/trailing bytes and governed width/text/table/payload excess
    without partial mutation. The warning-clean eight-worker full Debug rebuild
    completes in 85 steps; the dependent codec/protocol/session/execution/
    loopback/synchronization/runtime/artifact/application/inventory/contract/
    install/catalog slice passes 17/17 in 90.71 seconds, and the source-line gate
    passes separately. The refreshed graph contains 35,911 nodes and 165,839
    edges; codec functions have maximum cognitive complexity 11, decoder
    complexity 9, loop depth two and no recursion. The ledger preserves Changes
    2-10 with nine active rows and digest
    ...
    required existing-header edits were semantic and not reformatted. No
    Release build/test, sanitizer, CI/workflow activity, reset, commit or push
    ran. Preserve Changes 1-10 and implement only Change 11's native TLM 1.0
    in-island interfaces next. Keep hosted CI untouched until Change 20, where
    the latest user instruction requires inspecting the most recent CI log and
    fixing its errors locally before the final push and monitoring run.
29. Change 11 is complete in the same intentionally dirty worktree. Real
    Accellera TLM1 FIFO, blocking/nonblocking put/get/peek, transport and
    analysis interfaces retain native blocking, event ordering and analysis
    fan-out inside one island. The bounded registry validates reciprocal typed
    connections, bridge agreement, all interface kinds, canonical stable
    endpoint/sequence/transaction identities and legal begun/blocked/terminal
    transitions. Arbitrary-width request/response values serialize only for an
    explicit bridge; native co-located traffic rejects serialization. The
    application compiler now uses the configured build compiler and its driver
    defaults, preserves the legacy `<systemc>` adapter while allowing official
    TLM headers, and links Accellera macro exports through the exports-only
    support archive. This prevents the legacy `sc_module_name` implementation
    from interposing on the shared upstream runtime; mixed legacy/Accellera
    object links reject, while the existing legacy datatype application still
    passes. Protocol nesting and design-artifact persistence round-trip exact
    TLM1 transactions. Four cataloged diagnostics cover metadata, lifecycle,
    payload and resource failures. The warning-clean eight-worker full Debug
    rebuild completes 516 steps in 244.39 seconds with peak RSS 1,600,180 KiB.
    The dependent artifact/catalog/source/inventory/contract/install/runtime/
    protocol/session/execution/loopback/synchronization/value/TLM/compiler/
    incremental/application slice passes 20/20 in 43.45 seconds; the
    Windows-critical typed-boundary case passes in 23.63 seconds. The refreshed
    graph contains 36,061 nodes and 166,409 edges; TLM1 functions have maximum
    cognitive complexity 11, loop depth one and no recursion. The ledger
    preserves Changes 2-11 with eight active rows and normalized SHA-256
    `30dc77d91ac2ab49dd5f87f7dd2a8ca0d8080c7b03503733fe5ba28ec6a126e4`.
    Required existing-header changes were semantic and were not reformatted.
    No Release build/test, sanitizer, hosted-CI/workflow action, reset, commit
    or push ran. Preserve Changes 1-11 and implement only Change 12's native
    TLM 2.0 boundary. Keep hosted CI untouched until Change 20, where the latest
    user instruction requires inspecting the most recent CI log and fixing its
    errors locally before the final push and monitoring run.
30. Change 12 is complete in the same intentionally dirty worktree. Real
    Accellera TLM2 initiator/target sockets retain blocking transport,
    nonblocking forward/backward phases, response status, generic-payload
    extensions, DMI plus invalidation, debug transport and `tlm_utils` quantum
    keeping inside one island. The bounded registry validates reciprocal socket
    graphs, directions, bus widths, bridge agreement, canonical stable
    endpoint/sequence/transaction identities and legal lifecycle state while
    exposing only owned data and pointer-free DMI metadata. Native co-located
    traffic rejects serialization; explicit bridges use a canonical schema-1
    codec with overflow-safe length accounting. The combined TLM1/TLM2
    application, generic protocol nesting and design-artifact persistence all
    pass. Four cataloged diagnostics cover metadata, lifecycle, payload and
    resource failures. The warning-clean eight-worker full Debug dependency
    build completes in 78 steps. The dependent artifact/catalog/source/
    inventory/contract/install/provenance/runtime/protocol/session/execution/
    loopback/synchronization/value/TLM/compiler/incremental/application slice
    passes 21/21 in 47.47 seconds, including the Windows-critical typed-boundary
    case in 26.42 seconds. The refreshed graph contains 36,464 nodes and
    170,546 edges; TLM2 functions have maximum cognitive complexity 11, loop
    depth one and no recursion. The ledger preserves Changes 2-12 with seven
    active rows and normalized SHA-256
    ...
    CI/workflow action, reset, commit or push ran. Preserve Changes 1-12 and
    implement only Change 13's immutable signal/channel inventory next. Keep
    hosted CI untouched until Change 20, when the most recent completed CI log
    must be inspected and its failures reproduced and fixed locally before the
    final push and monitoring run. The legacy SystemC interface is no longer
    required: Change 17 must remove it and every residual custom-kernel path,
    and Change 20 must verify only the official Accellera interface remains.
31. Change 13 is complete in the same intentionally dirty worktree. A bounded
    schema-1 post-binding registry freezes canonical channel paths and stable
    object/channel identities with exact value-codec domain, width, signedness,
    writer policy, update ownership and observation mode. Official adapters
    cover built-in integral, `sc_bit`, `sc_logic`, `sc_int`, `sc_uint`,
    `sc_bigint`, `sc_biguint`, `sc_bv` and `sc_lv` signals plus buffers, clocks,
    resolved scalar/vector signals, mutexes, semaphores and event queues. The
    native proof includes 17- and 129-bit signals. Duplicate paths, mutation
    after freeze, disappearing/changed live channels, unsupported custom
    primitive channels, malformed/trailing/reserved encodings and resource
    exhaustion reject without partial publication. Generic protocol nesting,
    trace-hierarchy path identity and design-artifact persistence round-trip
    exact inventories. Four cataloged diagnostics cover metadata, lifecycle,
    unsupported adapters and resources. The warning-clean eight-worker full
    Debug dependency build completes in 74 steps. The dependent artifact/
    catalog/source/inventory/contract/install/provenance/runtime/protocol/
    session/execution/loopback/synchronization/value/TLM/channel-inventory/
    compiler/incremental/trace/application slice passes 23/23 in 42.60 seconds,
    including the Windows-critical typed-boundary case in 23.86 seconds. The
    refreshed graph contains 36,659 nodes and 171,150 edges; inventory
    functions have maximum cognitive complexity 11, loop depth one and no
    recursion. The ledger preserves Changes 2-13 with six active rows and
    normalized SHA-256
    `0ab6d52101b206895318e09252d62331112249e39d4ef3b8f4c3c9587625ebf7`.
    Formatting touched only the new headers, implementation and direct test;
    no existing header was reformatted. No Release build/test, sanitizer,
    hosted-CI/workflow action, reset, commit or push ran. Preserve Changes 1-13
    and implement only Change 14's binding/alias inventory next. Keep the
    legacy-interface removal assigned to Change 17 and hosted CI untouched
    until Change 20's last-log audit, local repairs, final checks and push.
32. Change 14 is complete in the same intentionally dirty worktree. A bounded
    schema-1 binding registry resolves official `sc_in`, `sc_out`, `sc_inout`,
    generic ports and exports once onto Change 13's immutable channel IDs. Each
    declaration retains its canonical path, interface, direction, all sorted
    targets, complete hierarchical alias chain and optional Verilog,
    SystemVerilog or VHDL endpoint identity; the frozen execution-facing graph
    needs no path lookup. Native coverage includes direct input/output, a child
    port through its parent, export binding, two-target multi-bind and a
    VHDL-correlated proxy. Missing channels, duplicates, cyclic/repeated chains,
    bad lifecycle, malformed/trailing/reserved encodings and resource
    exhaustion reject transactionally. Generic protocol nesting, trace alias
    identity and design-artifact persistence round-trip exact graphs. Four
    cataloged diagnostics cover metadata, topology, lifecycle and resources.
    The warning-clean eight-worker full Debug dependency build completes in 80
    steps. The dependent artifact/catalog/source/inventory/contract/install/
    provenance/runtime/protocol/session/execution/loopback/synchronization/
    value/TLM/channel/binding/compiler/incremental/trace/application slice
    passes 24/24 in 42.63 seconds, including the Windows-critical typed-boundary
    case in 24.55 seconds. The refreshed graph contains 36,783 nodes and
    171,722 edges; binding functions have maximum cognitive complexity 11,
    maximum loop depth two and no recursion. The ledger preserves Changes 2-14
    with five active rows and normalized SHA-256
    `615583497a88c0edc084f79b7fcd6f474d6169a8008809b90dc1af295660121c`.
    Formatting touched only new headers, implementation and direct test; no
    existing header was reformatted. No Release build/test, sanitizer, hosted-
    CI/workflow action, reset, commit or push ran. Preserve Changes 1-14 and
    implement only Change 15's post-update trace dirty hooks next. Keep the
    legacy-interface removal assigned to Change 17 and hosted CI untouched
    until Change 20.
33. Change 15 is complete in the same intentionally dirty worktree. Official
    post-update `value_changed_event` hooks now queue lossless ordered batches
    by frozen endpoint ID without polling channel paths. Disabled observers
    avoid value capture; late enablement publishes an immediate snapshot;
    aliases validate against Change 14's frozen bindings; arbitrary-width
    values and repeats reach the immutable observation model, forced VCD
    checkpoints and FST. Queue saturation returns retryable bounded
    backpressure without mutation, and flush/close contain writer failure in a
    terminal state. A real `sc_buffer<sc_lv<257>>` and Boolean signal prove
    selective tracing, disabled cost, repeated post-update capture, late
    snapshot, aliasing, backpressure/retry and lifecycle behavior. Protocol,
    phase and design-artifact tests preserve the same exact repeated value.
    Four cataloged diagnostics cover metadata/order, lifecycle, backpressure
    and writers. The warning-clean eight-worker full Debug dependency build
    completes in 60 steps; the dependent artifact/catalog/source/inventory/
    contract/install/provenance/runtime/protocol/session/execution/loopback/
    synchronization/value/TLM/channel/binding/compiler/incremental/trace/
    application slice passes 26/26 in 42.81 seconds, including the Windows-
    critical typed-boundary case in 24.51 seconds. The refreshed graph has
    36,787 nodes and 169,225 edges; trace functions have maximum cognitive
    complexity 10, loop depth one and no recursion. The ledger preserves
    Changes 2-15 with four active rows and normalized SHA-256
    ...
    no existing header was reformatted. No Release build/test, sanitizer,
    hosted-CI/workflow action, reset, commit or push ran. Preserve Changes 1-15
    and implement only Change 16's safe-point debugger and correlated TLM
    transaction observation next. Change 17 still owns complete removal of the
    legacy SystemC interface and every residual custom-kernel execution path;
    keep hosted CI untouched until Change 20.
34. Change 16 is complete in the same intentionally dirty worktree. A bounded
    schema-1 safe-point observer publishes debugger reads/writes, inventory
    queries and reports only at quiescent owning-island points, with explicit
    frozen-endpoint adapters and mutation-free rejection before unsafe,
    unsupported, out-of-order or resource-exhausted callbacks. Unsupported
    custom primitive channels now remain visible and round-trip exactly in the
    immutable inventory instead of disappearing; registering an adapter enables
    their arbitrary-width debugger values without changing that inventory.
    Native TLM1 begin/update/end and TLM2 begin/phase/DMI/invalidation/debug/end
    activity is recorded with stable endpoint, peer, sequence and transaction
    identity plus exact canonical payload bytes, never as fabricated signal
    changes. Nested payload/correlation validation, generic protocol transport,
    the real SystemC/TLM application phase and exact design-artifact
    publication/load are covered. Four cataloged diagnostics own metadata,
    safe-point, unsupported-adapter and resource failures. The warning-clean
    eight-worker full Debug dependency build completes in 77 steps; the
    dependent artifact/catalog/source/inventory/contract/install/provenance/
    runtime/protocol/session/execution/loopback/synchronization/value/TLM/
    channel/binding/observation/compiler/incremental/trace/application slice
    passes 27/27 in 43.79 seconds, including the Windows-critical typed-boundary
    case in 24.32 seconds. A stale portability contract was aligned with the
    official-runtime linker by forbidding whole-archive legacy shim import. The
    refreshed graph has 36,902 nodes and 169,950 edges; observation functions
    have maximum cognitive complexity 13, loop depth one and no recursion. The
    ledger preserves Changes 2-16 with three active rows and normalized SHA-256
    `9e865d40fd801716bc50af244730a4038f4a199e8424e7855a494c3688f9c848`.
    Formatting touched only the new header, implementation and direct test; no
    existing header was reformatted. No Release build/test, sanitizer, hosted-
    CI/workflow action, reset, commit or push ran. Preserve Changes 1-16 and
    implement only Change 17 next: remove the legacy SystemC interface and all
    residual custom-kernel execution paths while preserving source,
    incremental, mapped-library, multi-root/plugin, mixed-language, API,
    relocation, cache and replay behavior through official Accellera SystemC.
    Keep hosted CI untouched until Change 20.
35. Change 17 is complete in the same intentionally dirty worktree. SystemC
    source and incremental builds, mapped libraries, multiple roots/plugins,
    relocation, cold/warm/edit caches, mixed HDL boundaries, public APIs and
    artifact/checkpoint replay now use only the official Accellera runtime.
    The custom kernel, scheduler, fiber/Boost.Context path, compatibility
    headers and support archive are removed. ABI v3 and the official bridge no
    longer expose reverse SystemC-to-HDL proxy callbacks, declarations or
    elaboration; HDL owns HDL child composition. A governed build overlay fixes
    upstream teardown ordering by destroying the process table before the
    coroutine package without modifying vendored source. The warning-clean
    eight-worker full Debug rebuild completes in 1,062 steps. The fresh
    official application matrix passes in 145.28 seconds; the dependent slice
    passes 36/36 in 49.53 seconds, including typed boundaries in 49.52 seconds;
    direct elaboration, ABI, plug-in, portability and public-release checks
    pass 5/5 in 1.06 seconds. The three-language example now uses HDL-owned
    SystemC and VHDL siblings and passes check, eight-worker build, execution,
    PASS and clean stop. The refreshed graph contains 36,298 nodes and 160,328
    edges. The complete authored-source audit finds removed names only in
    negative contracts or historical records. The ledger preserves Changes
    2-17 with two active rows at normalized SHA-256
    `0c1398bf3dc97bd04061ae47caadc03ceb5db753b8c5c9cb551dc492a0b8b388`.
    Required public-header edits were semantic and no existing header was
    reformatted. No Release build/test, sanitizer, hosted-CI/workflow action,
    reset, commit or push ran. Preserve Changes 1-17 and implement only Change
    18's upstream/fsim corpus, platform and governed-resource boundary next;
    keep hosted CI untouched until Change 20's final last-log audit, local
    repairs, checks, commit and push.
36. Change 18 is complete in the same intentionally dirty worktree. Three
    byte-unmodified official programs from the pinned tree now run: simple
    FIFO, the IEEE 1666 event-list regression and the TLM FIFO regression. A
    portable exported entry shim provides shared-runtime discovery without
    patching upstream source. The fsim-owned corpus runs two roots with
    generated 129-bit two-state and 257-bit four-state values, ports, an export,
    signals, clock, enabled/disabled observation, bounded TLM1 FIFO and TLM2
    socket traffic. It proves exact time advancement, one runtime/context,
    4,096 crossings, 1,792 observations, 2,048 TLM1 and 4,096 TLM2
    transactions, 8,192 touched stack bytes, 9,120 object bytes, 2,040
    backpressure events, PASS, stopped status and two-root destruction.
    Throughput is reported without a machine-speed threshold. The four new
    tests pass 4/4 in 0.02 seconds; the dependent session/plugin/loopback/
    failure/replay/artifact/trace/typed-boundary slice passes 30/30 in 47.45
    seconds, including typed boundaries in 47.44 seconds. Unchanged configure
    runs no longer rewrite the governed upstream overlay, avoiding unnecessary
    runtime rebuilds. The refreshed graph contains 36,340 nodes and 160,474
    edges; new functions have maximum cognitive complexity 6, loop depth one
    and no recursion. The ledger preserves Changes 2-18 with one active row at
    normalized SHA-256
    `5ab59debdba3be19aeffb70b345865a76b8f78b9ff2092a5ee0911c2a423c47e`.
    No Release build/test, sanitizer, hosted-CI/workflow action, reset, commit
    or push ran. Preserve Changes 1-18 and implement only Change 19's public
    documentation, zero-active ledger, regression de-duplication and exact
    Batch 173 handoff next. Keep hosted CI untouched until Change 20.
37. Change 19 is complete in the same intentionally dirty worktree. README,
    architecture, language support and the SystemC integration reference now
    expose only the official Accellera 3.0.2 runtime, native TLM-1/TLM-2,
    pointer-free direct/loopback backend, safe-point observation, exact
    artifact/ABI identities and the explicit post-v2 parallelism boundary.
    Nine recursive closure drivers use CTest fixtures during ordinary
    regression, eliminating nested reruns while preserving explicit standalone
    retained-evidence commands. The new Accellera closure passes four corpus,
    thirteen backend, twelve integration and six contract witnesses; its
    integration stage completes in 138.00 seconds and all retained logs are
    nonempty. The SDF/VITAL audit now tracks 2,516 diagnostics, 1,100 bounded
    sources and 1,304 SPDX-owned authored files while excluding generated
    example cache and trace output. The final focused inventory, provenance,
    installed-public, portability, de-duplication, SDF/VITAL, public-release,
    catalog and source-policy slice passes 10/10, and the fixture-only closure
    driver passes separately. The refreshed graph has 36,345 nodes and 160,486
    edges. The final eight-worker Debug dependency build completes 124 link
    steps warning-clean. The ledger has eighteen
    preserved rows, zero active rows and normalized SHA-256
    `f0a4261bc193480d573128edef7c0b3aa890e43f472bdf3cdac358eb00927d4e`.
38. Preserve all Batch 172 Changes 1-19 and perform only Change 20 next. Before
    pushing, inspect the most recent completed CI log once and reproduce/fix
    its errors locally, including the audited MSVC test-string size boundary,
    Windows `FSIM-SC-I004` cache-lock path, one-dimensional Verilog memory
    concrete bound, SDF/VITAL release audit and duplicate regression coverage.
    Make every CI timeout 120 minutes. Then run the LLVM-disabled sanitizer and
    fresh exact-LLVM Debug/Release builds and complete product gates with at
    least eight workers, commit and push the accumulated batch once, and wait
    for a Windows run to complete before declaring the boundary finished. Until
    that final boundary begins, perform no hosted-CI inspection, restart or
    monitoring. Do not reintroduce the legacy SystemC interface or any custom
    kernel path, and do not create formatting-only header churn.
39. Change 20 is in progress at a restart-safe local checkpoint. Branch
    `codex/v2`, `HEAD` and `origin/codex/v2` still identify
    `8c2c89ed678a044d8f522b83aafe7b9657c7354f`; nothing from Batch 172 has been
    committed or pushed. The intentionally dirty worktree contains 179 paths:
    105 modified, 11 deleted and 63 untracked. `git diff --check` passes. The
    most recent completed hosted run, `31866595347` at this same base, was
    inspected once before local repair; do not inspect, restart or monitor
    hosted CI again until the accumulated closeout has been committed and
    pushed. All five workflow job timeouts in `.github/workflows/ci.yml` are
    already 120 minutes. The complete MSVC test-string audit scans 408 C/C++
    test files with a conservative 16,000-source-byte limit and passes; both
    the largest token and adjacent group are 14,622 bytes at
    `tests/app/vhdl_ieee_integration_application_test.cpp:729`.

    The audited CI repairs are present but uncommitted. Trace-archive tests use
    portable temporary producer/consumer roots; the FST corpus accepts CRLF by
    normalizing it before enforcing canonical manifest content; the stale
    three-language preprocessing assertions use the official HDL-sibling
    hierarchy and final values; the VHDL/PSL digest and SDF/VITAL authored-file
    count are synchronized; and incremental SystemC cache locks use a
    32-character digest filename to protect the Windows path budget. The exact
    LLVM Debug focused slice passes 7/7 for frontend, trace archive, FST corpus,
    typed boundaries, preprocessing CLI, VHDL/PSL documentation and SDF/VITAL
    release audit. This directly passes the reported `FSIM-SC-I004` typed-
    boundary case locally. The current frontend suite also passes without an
    additional source repair for the reported one-dimensional Verilog-memory
    bound. The sanitizer build is complete with eight workers after making
    three backend integral decoders warning-clean. Accellera's standards-legal
    derived-module member-function conversion triggers GCC's `vptr` sanitizer,
    so only the upstream runtime target is compiled with
    `-fno-sanitize=vptr`; ASan and every other UBSan check remain enabled.
    With leak detection disabled for the ptrace-constrained environment, the
    focused SystemC datatype and typed-boundary sanitizer tests pass 2/2 in
    224.55 seconds.

    The first complete 243-test sanitizer attempt intentionally ran to its
    1,200-second matrix timeout and leaves authoritative logs in
    `build/ci-sanitizers/Testing/Temporary/`. It records 226 passes and 17
    failed/not-run entries. Five closure drivers were not run only because of
    fixture dependencies; they are not independent failures.

    All four diagnosed root causes from that run now have local repairs. The C
    API test follows the official hierarchy, validates the single
    `named.$accellera_kernel` process and explicitly rejects removed legacy
    `pulse`, `metadata` and `run` objects; `fsim.api` passes under the sanitizer
    in 44.48 seconds. Incremental plug-in teardown now retains the mapped image
    and its host/type-info resources for process life, because Accellera/TLM's
    global extension registry owns `type_index` entries after logical registry
    teardown; `fsim.application.systemc_tlm1` passes twice under the sanitizer
    in 13.04 and 13.20 seconds without the former heap use after free, and
    `fsim.systemc.matrix` passes in 40.79 seconds. The release/audit evidence is
    synchronized at 2,516 diagnostics, 1,100 bounded sources, 1,304 authored
    files, 100 corpus expectations and zero `SC-020`/`ML-011` positive-owner
    gaps. The V1 release, inventory, mixed-SystemC, FST, SDF application,
    VHDL/PSL, Verilog, UVM, differential and release-candidate checks all pass;
    the feature matrix digest is
    `bbef73769bca32d42b22fae66968b47bb4f15f59f51d8266353fbbb1f29db141`.
    `fsim.application.systemc_matrix` now has the required 7,200-second
    (120-minute) CTest timeout rather than the obsolete 1,200-second ceiling.

    An isolated sanitizer run of `fsim.application.systemc_matrix` remained
    active and silent for more than ten minutes. It was interrupted with exit
    130 solely to create this restart boundary, so it has no pass/fail verdict
    and no test process remains. On restart, rerun that exact isolated case
    first with
    `ASAN_OPTIONS=detect_leaks=0:strict_string_checks=1` and
    `UBSAN_OPTIONS=print_stacktrace=1:halt_on_error=1`, allowing its full
    120-minute timeout. If it passes, rerun the focused prior-failure slice and
    then all 243 sanitizer tests with the same environment and at least eight
    workers. Only after the sanitizer suite is clean run fresh eight-worker
    exact-LLVM Debug and Release builds/tests and the remaining product gates.
    Re-run the MSVC string audit, timeout audit, release audits and
    `git diff --check`; update Change 20 evidence; commit and push the
    accumulated Batch 172 exactly once; then wait for a Windows run to finish
    before declaring the boundary complete. Do not inspect, restart or monitor
    hosted CI before that push. Do not start Batch 173 from this dirty state,
    do not reintroduce any legacy/custom SystemC interface, and avoid
    formatting-only edits to existing headers.

    The exact first command is:

    ```sh
    ASAN_OPTIONS=detect_leaks=0:strict_string_checks=1 \
    UBSAN_OPTIONS=print_stacktrace=1:halt_on_error=1 \
      ctest --test-dir build/ci-sanitizers --output-on-failure \
      -R '^fsim\\.application\\.systemc_matrix$'
    ```

    After it passes, rebuild the sanitizer tree if any source or CMake input
    changed, rerun the focused prior-failure tests, then rerun all 243 with
    `--parallel 8`. The final closeout order remains sanitizer, fresh exact-
    LLVM Debug, fresh exact-LLVM Release, audits, one commit/push and one
    completed Windows run.

40. Change 20 local qualification is complete and no test process remains.
    Branch `codex/v2`, `HEAD` and `origin/codex/v2` still identify
    `8c2c89ed678a044d8f522b83aafe7b9657c7354f`; nothing from Batch 172 has yet
    been committed or pushed. The intentionally dirty worktree contains 179
    paths: 105 modified, 11 deleted and 63 untracked. `git diff --check`
    passes. Do not inspect, restart or monitor hosted CI until the accumulated
    batch has been committed and pushed once.

    The isolated LLVM-disabled sanitizer SystemC application matrix passes
    1/1 in 1,615.29 seconds under its corrected 7,200-second timeout. The
    focused prior-failure slice passes 11/11 in 12.51 seconds, including C API,
    TLM1, V1 differential/release-candidate, inventory, mixed-SystemC, FST,
    SDF, Verilog, VHDL/PSL and UVM evidence. The complete sanitizer regression
    passes 243/243 in 1,718.11 seconds with no ASan/UBSan marker. A fresh clean-
    first eight-worker exact-LLVM Debug build completes 2,162 steps and its
    full regression passes 246/246 in 157.02 seconds.

    Release qualification found two optimizer/linker-only build defects. The
    synthesized official-SystemC process now directly emplaces Halt or
    WaitSensitivity instead of moving a nested variant through a conditional
    temporary, clearing GCC's `-Werror=maybe-uninitialized`. The process-wide
    bridge exports its executable-interposable `sc_main` fallback normally and
    carries Linux `--no-as-needed`, so GNU ld retains the bridge before the
    official runtime introduces its standard `sc_main` reference. The final
    Release build succeeds with eight workers and its complete regression
    passes 246/246 in 272.63 seconds. The Release protocol executable resolves
    one fsim Accellera bridge and one official `libsystemc.so.3.0`. Per the
    explicit user instruction, neither sanitizer nor Debug was rerun after
    these Release-only repairs; do not rerun either before the commit.

    The final MSVC source-string audit passes 408 C/C++ test files at the
    conservative 16,000-byte threshold; its maximum token and adjacent group
    are 14,622 bytes at
    `tests/app/vhdl_ieee_integration_application_test.cpp:729`. All five
    workflow job timeouts remain 120 minutes, the SystemC application matrix
    timeout is 7,200 seconds, SDF/VITAL and every other release/audit gate pass
    in the final Release regression, and removed legacy SystemC source/header
    names occur only in the negative portability contract or historical
    documentation. The next bounded action is to re-index the final graph,
    stage the complete 179-path Batch 172 changeset, audit the staged diff,
    commit and push exactly once, then wait for a Windows run to finish. Do not
...
50. Generated-plug-in repair `4b655f1` is pushed, and run `31933240042`
    ...
    The Windows continuation migrated the matrix to LLVM-MinGW in `534ddb8`,
    repaired LLVM package discovery in `e1ae900`, and corrected VHPI teardown
    under LLVM libc++ in `771b7af`. The user confirms CI is clean across the
    board. The live branch and `origin/codex/v2` are synchronized at full SHA
    `771b7afc19af6c5c71332cf28460437174b00b0a`; Batch 172 is complete, all
    workflow job timeouts remain 120 minutes and the legacy SystemC interface
    remains absent.
...
1. Start in `/home/colin/projects/fsim`, read this section and Batch 173 in
   `docs/implementation_plan_v2.md`, and verify branch `codex/v2` is clean and
   synchronized at pushed Batch 172 closeout
   `771b7afc19af6c5c71332cf28460437174b00b0a`. Do not trust a stale checkpoint
   hash.
2. Preserve the pinned official Accellera SystemC 3.0.2 archive/runtime, native
   TLM-1/TLM-2 behavior, ABI v3, bridge revision 2, zero-active eighteen-row
   ledger and retained four-stage closure. The removed fsim SystemC facade,
   reverse HDL proxy, custom kernel/scheduler, coroutine backend, custom
   channels/datatypes and Boost.Context dependency must remain absent.
3. Before semantic SCV work, expand Batch 173's grouped allocation into exactly
   twenty bounded changes and freeze a one-to-one ownership ledger for official
   SCV 2.0.1 provenance, compatibility patches, one shared library, ABI/cache,
   randomization/introspection, recording, backend transport, artifacts,
   platform/resources, documentation and closure. Change 1 is planning only.
4. Changes 1-19 use focused Debug validation. Batch 173 runs no Release build or
   test, sanitizer, or hosted-CI monitoring; all three obligations are deferred
   explicitly to final release Batch 177. Use at least eight local build
   workers, avoid formatting existing headers, and preserve unrelated dirty
   work. Change 20 performs only fresh full exact-LLVM Debug qualification,
   then the single Batch 173 commit and push.
5. Change 1 is complete in the intentionally dirty Batch 173 worktree. The
   authoritative eighteen-row `scv_inventory.tsv` assigns Changes 2-19 exactly
   across official provenance, patch governance, one shared install surface,
   ABI/cache and producer/artifact boundaries, pointer-free protocol identities,
   randomization, smart-pointer lifetime, constraints, introspection, common and
   native transaction records, trace/debug correlation, loopback merge,
   corpus/platform/teardown, resources and documentation/closure. Every row is
   active at normalized SHA-256
   `e16bc1a45eb2cda64aa56bb89dcaf0dbe8fcd353b55402cb090660d010e0380e`.
   The direct workflow-independent contract passes, and after regenerating only
   the existing Debug test registry the SCV plus retained SystemC inventory pair
   passes 2/2 in 0.04 seconds. No semantic SCV source, Release build/test,
   sanitizer, hosted-CI action, commit, push or header formatting ran. Preserve
   this planning-only worktree and implement only Change 2's official SCV 2.0.1
   provenance next.
6. Change 2 is complete in the same intentionally dirty worktree. The exact
   official Accellera `scv-2.0.1.tar.gz` archive is retained at 2,835,735 bytes
   and SHA-256
   `7bd1c4037f3c108d02f45cae003d112efdb788d469cb029fada247d330ca4881`;
   its deterministic 578-file tree identity is
   `85cc2e4e3ee1893878de2567f253003859c847a151f080e48f185825c34f933c`.
   The governed bundle carries the exact Apache-2.0 license and notice, an
   upstream URL/version manifest and SPDX 2.3 component. Accellera publishes
   the release as a direct 2017-12-08 standards download, so no unsupported
   public source tag or commit is claimed. Package metadata and release notes
   identify 2.0.1 while the byte-pinned legacy `scv_ver.h` remains 2.0.0 dated
   20140417; the provenance test freezes both facts without rewriting upstream.
   Offline materialization and distinct corrupt, wrong-version, incomplete and
   unsafe-root rejection pass directly. After regenerating only the existing
   Debug registry, SCV inventory plus retained SCV/SystemC provenance pass 3/3
   in 0.65 seconds. The ledger preserves Change 2 with seventeen active rows at
   normalized SHA-256
   `6644d790c9e687fe6e404e52f8668a88f6818b7f406bf449e8c75ca0fac7ad48`.
   No compilation, source patch, Release test, sanitizer, hosted-CI action,
   commit, push or header formatting ran. Preserve Changes 1-2 and proceed only
   to Change 3's unmodified-build and governed compatibility-patch decision.
7. Change 3 is complete in the same dirty worktree. The byte-pinned 578-file
   source configures with GCC 13 and builds its shared and static SCV libraries
   unmodified against the governed SystemC 3.0.2 runtime with eight workers in
   18.02 seconds. The retained transcript is
   `build/llvm22-ninja-debug/tests/scv-2.0.1-unmodified-gcc/unmodified-build.log`.
   The upstream 2017 configure layer separately rejects `clang++-22` before
   source compilation because it does not recognize Clang and searches only the
   obsolete `lib/gnu` layout. The external CMake build subsequently exposes one
   Clang 22 source defect: const `scv_bag::peekRandom()` assigns the cached
   random-generator pointer. One all-platform generated patch makes only that
   pointer `mutable` in an isolated build copy. The pristine tree remains
   unchanged. `PATCHES.txt` pins the patch, input, output and patched-tree
   identities plus rationale, probes and removal criteria at SHA-256
   `15625eaeef9c640b3e446ff4f1350795e1afb103ad4cfcd40d8329a4c9d20b29`;
   the patched-tree SHA-256 is
   `b8ee86c4050b7a77e39fcbba25fc553d477074b323349885a7634844ecd6a112`.
   Changed manifests, patches, inputs, outputs and source trees reject. This
   supersedes the provisional zero-patch conclusion while retaining the
   unmodified GCC build as the required pre-patch baseline.
8. Change 4 is complete in the same dirty worktree. The external adapter closes
   exactly 90 upstream runtime sources into the sole shared `SCV::scv` target,
   installs `libscv` 2.0.1 with ABI soname 2 against the governed shared SystemC
   runtime, and scopes two legacy-C warning suppressions to SCV C sources. The
   generated configuration header is content-stable, so CMake regeneration
   causes no SCV rebuild. Supported headers, license/notice, relocatable CMake
   metadata with exact SystemCLanguage 3.0.2.20251031 identity and relative
   pkg-config metadata install without source/build-prefix leakage. The focused
   consumer relocates that install to a non-ASCII path, uses
   `find_package(SCV 2.0.1 EXACT)`, builds with eight workers, resolves exactly
   one relocated SCV and one relocated SystemC shared runtime and executes the
   const-bag probe. The final focused Debug inventory, provenance, patch,
   shared-runtime and installed-consumer slice passes 5/5 in 5.15 seconds. The
   ledger preserves Changes 2-4 with fifteen active rows at normalized SHA-256
   `6300b075542373e11d08846af3c4a6a71f4e6f992b1a441aa368b33a58778868`.
   No Release test, sanitizer, hosted-CI action, commit, push or header
   formatting ran. Preserve Changes 1-4 and proceed only to Change 5's
   compatibility, ABI and cache identities.
9. Change 5 is complete in the same dirty worktree. One canonical sixteen-field
   producer identity freezes the SCV release/legacy-header, archive, governed
   patch/tree, SystemC runtime/source/bridge, TLM, compiler, standard library,
   adapter ABI, plug-in ABI, artifact schema and cache schema. The new public
   `fsim/systemc/scv.hpp` adapter exports exact acceptance and field-specific
   `FSIM-SCV-C001` diagnostics; all sixteen single-field mutations reject.
   SystemC's append-only host ABI advances to 4 and carries this identity, so
   direct plug-in loading rejects it before opening an image and macro plug-ins
   reject it before registration. Incremental object/plug-in formats advance to
   2, serialize the identity, include it in cache digests and reject a
   self-consistent stale identity before payload load. Source plug-in cache keys
   and installed CMake/pkg-config metadata carry the same versioned boundary;
   the relocated installed consumer calls the exported identity API. The final
   focused Debug compatibility, ABI, plug-in, compiler, incremental, install,
   provenance, inventory and retained portability/source-audit slice passes
   14/14 in 30.72 seconds. The focused rebuild is a no-op and
   `git diff --check` passes. The ledger preserves Changes 2-5 with fourteen
   active rows at normalized SHA-256
...
10. Change 6 is complete in the same intentionally dirty worktree. The governed
    source-plug-in plan and separately compiled incremental linker now carry
    the exact SCV include root and shared runtime library once beside SystemC.
    A macro-export SCV plug-in exercises the const-bag patch, a dependency
    header and the canonical compatibility API across source cold/warm/edited
    and incremental object/link cold/warm paths. Both loaded factories return
    the host canonical identity pointer, proving one loaded SCV runtime, while
    metadata and cache keys retain the exact producer identity. The final
    focused Debug inventory, SCV producer, plug-in, compiler and incremental
    slice passes 5/5 in 37.61 seconds. The ledger preserves Changes 2-6 with
    thirteen active rows at normalized SHA-256
    ...
    commit, push or formatting-only header churn ran. Preserve Changes 1-6 and
    proceed only to Change 7's mapped-artifact and relocation boundary.
11. Change 7 is complete in the same intentionally dirty worktree. Mapped-
    library format 5 and standalone-design format 11 explicitly retain the
    canonical SCV identity beside the existing incremental object and plug-in
    identities. `FSIM-SCV-A001` rejects missing, oversized, stale and
    incompatible artifact identities before native payload access. Exact
    mapped images reuse directly, cross-compiler images retain the safe source
    fallback, and a stale SCV compiler identity rejects without publishing a
    cache file. Non-ASCII relocation, source-hidden non-project design reuse,
    corrupt payload, future-schema and stale embedded-design cases pass through
    the existing application regressions rather than a duplicate test. The
    final focused Debug artifact, library, application, producer, incremental,
    inventory and portability slice passes 10/10 in 57.22 seconds. The focused
    rebuild is a no-op and `git diff --check` passes. The ledger preserves
    Changes 2-7 with twelve active rows at normalized SHA-256
    `b8f1a4ab9eee8a5b26c16d3beb7dca0e8a4f5abe076be7929441b5e93457228d`.
    Existing artifact headers changed only for semantic schema fields and were
    not reformatted. No Release-configuration build/test, sanitizer, hosted-CI
    action, reset, commit or push ran. Preserve Changes 1-7 and proceed only to
    Change 8's pointer-free backend protocol identities.
12. Change 8 is complete in the same intentionally dirty worktree. The public
    schema-1 SCV backend protocol derives stable pointer-free island, hierarchy,
    object, stream, generator and transaction identities and serializes ten
    bounded operations in one canonical fixed-header format. Exact request,
    receipt and event shapes retain sequence/correlation plus time, delta and
    region order. `FSIM-SCV-B001` through `FSIM-SCV-B003` reject malformed
    identities, unknown schema/operation values, illegal direction/status/
    correlation combinations, reserved bytes, framing errors and resource
    excess. Unit tests round-trip every operation and corrupt the wire format;
    the application test proves deterministic phase ordering, while the
    portability contract rejects SCV/SystemC types and pointers at the public
    boundary. The final focused Debug inventory, portability, protocol,
    application, retained kernel protocol and artifact slice passes 7/7 in
    0.06 seconds. The focused rebuild is a no-op and `git diff --check` passes.
    The ledger preserves Changes 2-8 with eleven active rows at normalized
    SHA-256
    `8e4add4d122acabbdb70249e29ec2a4f8b83c90f4a2b6c4d5ce55012313d0066`.
    Formatting was confined to the new implementation/tests; the new public
...
13. Change 9 is complete in the same intentionally dirty worktree. One
    pointer-free SCV random service derives root, island, object and canonical
    thread seeds through SHA-256 domain separation without a global call-order
    dependency. Frozen SplitMix64 seed/output vectors, unbiased bounded draws,
    explicit ceilings and canonical snapshots provide host-independent replay;
    foreign seeds, corrupt state and excess draws reject through
    `FSIM-SCV-R001` through `FSIM-SCV-R003`. Sorted unique weighted domains
    preserve exclusions for replacement distributions and explicit-reset
    without-replacement bags. Bag snapshots transactionally retain cycle,
    remaining weighted values and stream position. The application test proves
    that reordered island/object execution and an unrelated stream's extra
    draws do not perturb local results. The final focused Debug inventory,
    portability, randomization, application, backend protocol and artifact
    slice passes 6/6 in 0.06 seconds. The focused rebuild is a no-op, all four
    new files are formatter-clean and `git diff --check` passes. The ledger
    preserves Changes 2-9 with ten active rows at normalized SHA-256
    ...
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-9 and proceed only to Change 10's native `scv_smart_ptr` ownership
    boundary.
14. Change 10 is complete in the same intentionally dirty worktree. A bounded
    native registry owns real Accellera `scv_smart_ptr` scalar and aggregate
    payloads behind generational handles associated with stable Change 8 object
    IDs. Copy and matching-kind assignment retain upstream alias semantics;
    failed cross-kind assignment publishes nothing. Opaque extension paths
    cover scalar fields, nested records and fixed arrays with typed checked
    values and native randomization enable/disable/next control. The public
    header exposes no upstream SCV/SystemC type or raw pointer. Live-payload
    accounting proves alias release, destination replacement, stale-handle,
    registry destruction, invalid-limit and capacity-failure paths return to
    ...
    slice passes 7/7 in 0.06 seconds. The focused rebuild is a no-op, the four
    owning files are formatter-clean and `git diff --check` passes. The ledger
    preserves Changes 2-10 with nine active rows at normalized SHA-256
    `7719821746d0e1fcb34d6e654bbe21213573f3737b228e771ca5456379fb5c39`.
    No existing header was reformatted. No Release-configuration build/test,
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-10 and proceed only to Change 11's constraints/distributions boundary.
15. Change 11 is complete in the same intentionally dirty worktree. Stable
    native handles and extension paths feed signed 64-bit domains, weighted
    distributions, solve-before edges and hard/soft linear clauses into fsim's
    retained bounded solver. Scalar predicates, composite sums and referenced
    variables are covered; endpoint preflight rejects overflow, duplicate
    domains, alias targets, malformed paths/weights and invalid/cyclic order.
    All targets and original values validate before solving, the complete
    deterministic result stages before native assignment, and the retained
    rollback path restores prior writes on unexpected publication failure.
    Contradictory and search/elapsed-work-exhausted requests publish nothing
    through `FSIM-SCV-Q001` through `FSIM-SCV-Q003`; identical selection replays
    identical assignments. Application coverage constrains a real aliased
    native `scv_smart_ptr` extension. The final focused Debug inventory,
    portability, constraints, smart-pointer, randomization, application,
    backend protocol and artifact slice passes 8/8 in 0.07 seconds. The focused
    rebuild is a no-op, owning files are formatter-clean and `git diff --check`
    passes. The ledger preserves Changes 2-11 with eight active rows at
    normalized SHA-256
    `ff3826aa3671470b66b14846c5c2e9d068f306a87edd2d34ef676eed551cf14a`.
    No existing header was reformatted. No Release-configuration build/test,
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-11 and proceed only to Change 12's extension-introspection boundary.
16. Change 12 is complete in the same intentionally dirty worktree. An
    iterative pointer-free flat snapshot over real native `scv_extensions`
    retains stable parent IDs, field/element relation and ordinal, names,
    nominal types, kinds, signedness and packed `[width-1:0]` ranges. It covers
    Boolean/signed/unsigned scalar, enum value/name, string, record/class and
    fixed-array nodes plus a 257-bit SystemC value and exact canonical aval/bval
    planes for a 193-bit four-state value containing X and Z above 64 bits. The
    fixture initializes through official extension writes with randomization
    disabled, proving extension readback rather than native layout. Explicit
    node, depth, value-bit, plane-word, per-string and total-string limits reject
    stale, malformed or excessive trees through `FSIM-SCV-X001` through
    `FSIM-SCV-X003` without a partial snapshot. The final focused Debug
    inventory, portability, extensions, application, smart-pointer,
    constraints, backend protocol and artifact slice passes 8/8 in 0.07
    seconds. The focused rebuild is a no-op, owning files are formatter-clean
    and `git diff --check` passes. The ledger preserves Changes 2-12 with seven
    active rows at normalized SHA-256
    ...
    Changes 1-12 and proceed only to Change 13's common transaction-record
    model.
17. Change 13 is complete in the same intentionally dirty worktree. Schema-1
    `TransactionRecord` is a language-neutral pointer-free model carrying
    stable stream/generator/transaction IDs, exact begin/end time-delta-region,
    ordered attributes/relations and ordered correlated SystemC/TLM-1/TLM-2
    object IDs. Boolean, signed, unsigned, enum, string, bit-vector and
    logic-vector typed values retain nominal type, width, signedness, text and
    canonical aval/bval words. Strict validation owns time consistency, unique
    ordering, reserved/high bits and message, collection, string, width and
    plane budgets. Canonical little-endian bytes reject bad/future magic/schema,
    corrupt reserved fields, malformed planes, truncation and trailing data via
    `FSIM-SCV-T001` through `FSIM-SCV-T003`. The application test orders and
    ...
    rebuild is a no-op, four owning files are formatter-clean and
    ...
    No existing header was reformatted. No Release-configuration build/test,
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-13 and proceed only to Change 14's native SCV recording API.
18. Change 14 is complete in the same intentionally dirty worktree. One bounded
    adapter owns real native SCV databases, streams, signed begin/end
    generators and handles behind stable Change 8 identities. SCV's actual
    synchronous begin/end, Boolean/signed/unsigned/string attribute and
    relation callbacks alone populate Change 13 records; every callback checks
    its native value and identity against the active adapter operation before
    canonical sorting and record validation. Recording suspension publishes no
    partial record, explicit release retains native reference-counted lifetime,
    and `FSIM-SCV-N001` through `FSIM-SCV-N003` cover invalid state, callback
    mismatch and bounded resources. Application coverage serializes and
    round-trips a callback-produced record without fabricating signal activity
    or advancing the kernel. The final focused Debug inventory, portability,
    native-recording, application, transaction-record, backend-protocol and
    artifact slice passes 7/7 in 0.06 seconds. The focused rebuild is a no-op,
    owning files are formatter-clean and `git diff --check` passes. The ledger
    preserves Changes 2-14 with five active rows at normalized SHA-256
    `a5a415e665a580cfa1166662e14c8bd12204932dd9ff5658fe97ab09fe11bc30`.
    No existing header was reformatted. No Release-configuration build/test,
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-14 and proceed only to Change 15's trace/debug correlation boundary.
19. Change 15 is complete in the same intentionally dirty worktree. A bounded
    application service correlates common SCV records with stable SystemC
    signal/port and typed native TLM-1/TLM-2 transaction identities plus exact
    VCD/FST signal/time/delta/region associations. Per-generator barriers make
    late enablement deterministic; debugger inspection, contained observers,
    flush/close and retryable bounded backpressure preserve unique strict order
    without a dropped or duplicated accepted record. `FSIM-SCV-L001` through
    `FSIM-SCV-L003` own lifecycle, correlation/order and resource failures. The
    final focused Debug inventory, portability, SCV trace, native recording,
    retained trace observation/control/SystemC trace, SystemC/TLM,
    backend-protocol and artifact slice passes 10/10 in 2.97 seconds. A stale
    merged application host was relinked locally before its retained TLM case
    passed; the final focused rebuild is a no-op, owning files are formatter-
    clean and `git diff --check` passes. The ledger preserves Changes 2-15 with
    four active rows at normalized SHA-256
    `7afa66c38315615ba920133528f9022800c20eb64c8f7cecc33fe529dc9732ec`.
    No existing header was reformatted. No Release-configuration build/test,
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-15 and proceed only to Change 16's backend serialization/merge boundary.
20. Change 16 is complete in the same intentionally dirty worktree. A schema-1
    fixed 64-byte envelope carries the exact Change 13 bytes plus stable island
    and sequence identity. Direct and worker-loopback paths both serialize and
    deserialize through it and return byte-identical receipts; foreign-island
    loopback, duplicate/regressing sequence, malformed replay, disconnected or
    crashed endpoints and bounded record/byte pressure reject without changing
    queue or sequence state. Fresh replay is deterministic and merge orders by
    time, delta, region, island and sequence, while actual worker partitioning
    and scheduling remain post-v2. `FSIM-SCV-W001` through `FSIM-SCV-W003` own
    wire, ownership/order and resource failures. The final focused Debug
    ...
    seconds. The focused rebuild is a no-op, owning files are formatter-clean
    and `git diff --check` passes. The ledger preserves Changes 2-16 with three
    active rows at normalized SHA-256
    `30eed0ddf21251d9c4373e3ef2bf1fc3b1e2adfe3b656bcf91d62100b1423f22`.
    No existing header was reformatted. No Release-configuration build/test,
    sanitizer, hosted-CI action, reset, commit or push ran. Preserve Changes
    1-16 and proceed only to Change 17's corpus/platform/teardown boundary.
21. Change 17 is complete in the same intentionally dirty worktree. Four
    unmodified examples from the pinned official SCV source tree exercise
    hello/runtime linkage, introspection, randomization and native transaction
    recording. Target-local warning compatibility covers only legacy unused
    parameters/signed indices and SystemC's explicit deprecated-API opt-in;
    upstream source and project-wide warnings are unchanged. The fsim corpus
    composes deterministic seeds, constraints, extension capture, recording,
    direct/loopback replay and teardown back to the native payload/handle
    baseline. The registered retained matrix also covers provenance/patch,
    installed non-ASCII relocation, shared runtime, compiler/cache, artifacts,
    trace and compatibility. Current Clang Debug execution passes 26/26 in
    15.22 seconds; per the user-directed boundary, Windows and Release execution
    remain final Batch 177 work. The focused build is a no-op, the corpus file
    is formatter-clean and `git diff --check` passes. The ledger preserves
    Changes 2-17 with two active rows at normalized SHA-256
    `88ffd8869d6427ea734bd7c2bd5c6cb400524253f22332fc405dcf8e935badbe`.
    No existing header or upstream source was reformatted. No Release
    build/test, sanitizer, hosted-CI action, reset, commit or push ran. Preserve
    Changes 1-17 and proceed only to Change 18's performance/resource boundary.
22. Change 18 is complete in the same intentionally dirty worktree. A public
    pointer-free resource probe measures enabled/disabled recording elapsed
    work, transaction/callback volume, serialized bytes, queue peaks,
    approximate queue memory and solver exhaustion under explicit transaction,
    attribute, record, byte and search ceilings. Record-count and byte pressure
    return retryable backpressure and recover after drain. Injected invalid
    producer and disconnected consumer operations leave no handle or queue
    mutation, then accept the same deterministic transaction normally. Two
    64-transaction enabled runs each publish 32,448 canonical bytes at SHA-256
    ...
    while disabled recording publishes zero records/bytes. The explicit
    one-step contradictory constraint probe exhausts without changing native
    state. `FSIM-SCV-E001` through `FSIM-SCV-E003` own invalid probes,
    containment/recovery failure and resource exhaustion. The final focused
    Debug diagnostics, inventory, portability, artifact, protocol, constraint,
    transaction, recording, transport, resource and corpus slice passes 12/12
    in 0.30 seconds. The ledger preserves Changes 2-18 with one active row at
    normalized SHA-256
    `0f8950033a1d15b1f49ac4da59bf1675578818c192d12aa0576954ded98d4e2e`.
    No existing header was reformatted. No Release build/test, sanitizer,
    hosted-CI action, reset, commit or push ran. Preserve Changes 1-18 and
    proceed only to Change 19's documentation/closure handoff.
23. Change 19 is complete in the same intentionally dirty worktree.
    Architecture, language support, README, diagnostics, the twelve-row public
    feature/evidence matrix and the Batch 173 SCV release audit now agree on the
    governed source/patch, one shared runtime, producer/artifact identities,
    pointer-free backend, native features, recording/correlation, transport and
    resource boundary. The audit freezes the exact patch removal rule, 32-code
    diagnostic family, deterministic resource baseline and Batch 177 deferral.
    Direct closure execution retains separate governance, behavior, official-
    example and co-located SystemC/TLM logs plus a result ledger. Its registered
    form uses fixtures so ordinary regression executes each witness only once.
    The direct focused Debug closure passes 33/33 across the four retained
    stages in 10.08 seconds. The fixture-driven registered form passes 31/31 in
    14.97 seconds, with its nonrecursive driver taking 0.01 seconds. All
    eighteen inventory rows are preserved with zero active obligations at
    normalized SHA-256
    `78fda3ccb3264ad3833ddb7334f1b49cdd3c63970fb7aaacd32b8c8b6ad997b0`.
    Preserve Changes 1-19 and perform only Change 20's fresh full exact-LLVM
    Debug qualification, final Debug gates and single commit/push. Do not run a
    Release build/test, sanitizer or hosted-CI inspection; Batch 177 owns them.

24. Change 20 is complete. A clean-first exact-LLVM 22.1.8 Debug build finishes
    warning-free at 2,383/2,383 steps with eight workers. The first complete
    regression identifies only stale static baselines introduced by the SCV
    closure: ten fixture-backed recursive drivers, 2,548 diagnostics, 1,143
    bounded sources, 431 authored test/control files, 1,361 SPDX-owned files,
    the new feature-matrix digest and the refreshed VHDL/PSL closure-ledger
    digest. The same audit finds and repairs the sole missing SPDX notice in
    `cmake/scv.pc.in`. The affected audit slice then passes 15/15, its dependent
    documentation check passes, and the final full Debug regression passes
    276/276 in 169.55 seconds. The retained direct SCV closure transcript and
    result ledger remain in
    `build/llvm22-ninja-debug/tests/scv-closure-evidence`. The accumulated
    Batch 173 worktree is committed and pushed once at this boundary. No
...
Batches 150, 160, 170, and 172 retain their recorded CI-monitoring boundaries.
Batch 173 is no longer a monitoring boundary and runs no Release build/test or
sanitizer; it performs only focused and final full Debug qualification before
its single commit and push. Final release Batch 177 owns the deferred Batch 173
Release, sanitizer, and complete hosted Linux/Windows CI monitoring and repair.
Hosted CI excludes sanitizer instrumentation.
