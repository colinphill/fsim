...
The current matrix contains exactly 1,080 required rows. Every row is classified
...
| Mixed-language behavior | `ML` | 16 | Task 4 |
...
`9fe14444af5348949c1a196ab583b482af8b4e2199e80bfec27afd40df35345a`.
...
- 423 authored C/C++ sources remain within the 2,000-line limit;
...
| `B130-T7-INVENTORIES` | 7 | **Closed.** Diagnostics, source size, licenses, conformance, and provenance | The composed gate freezes 1,624 diagnostics, 423 bounded sources, 499 SPDX-owned artifacts, the 31-file/26-VHDL IEEE snapshot, 105 conformance expectations in 28 fixtures owned by 27 CTests, and 10 reviewed plus 6 excluded provenance identities |
| `B130-T8-RESOURCES` | 8 | **Closed.** Linux/Windows Debug/Release build and test bounds | The composed static audit freezes 12 hosted configurations, six four-worker CI build steps, an eight-link local pool, compact Debug objects, 8 MiB stacks, bounded job/test timeouts, phase traces, 16 platform files, and 20 portability rows; all 11 focused owners pass locally |
| `B130-T9-RECLASSIFICATION` | 9 | **Closed.** Final feature matrix and release corpus | All 1,080 required rows are `execute`; all 4,320 P+/P-/E/R cells link to checked-in owners; the 320-path evidence identity (157 test, 148 production, 15 release/build paths), 94 runtime files, and 36 corpus CTests are exact and digest-pinned |
