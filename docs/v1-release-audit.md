...
The current matrix contains exactly 1,250 required rows. Every row is classified
...
|---|---|---:|---:|
| SystemVerilog language | `SV` | 810 | Task 2 plus v2 Batches 144-157 and 159-160 |
...
V2 Batches 148-149 add `SV-708` through `SV-722` for source-executable classes,
constraint solving, and randomization. V2 Batch 150 adds `SV-723` through
`SV-732` for exact real/time scalars, Unicode strings, opaque chandles,
recursive values, file I/O, artifacts, and transactional differential closure.
V2 Batches 151-152 add `SV-733` through `SV-740` for multidimensional and
recursive unpacked data, mixed/file boundaries, runtime procedural controls,
callable/process lifetime, synchronization and shuffle, scheduler ownership,
and portable differential closure.
V2 Batch 153 adds `SV-741` through `SV-750` for program/reactive scheduling,
clocking blocks and cycle controls, virtual-interface identity and forwarding,
restricted modport views, engine differentials, and portable artifact closure.
V2 Batch 154 adds `SV-751` through `SV-760` for typed sequence/property/checker
and concurrent-directive ownership, semantic observer HIR, a bounded executable
scalar assertion slice, controls, per-instance coverage and events, exact
rejection boundaries, multiple roots, and portable artifact closure.
V2 Batch 155 adds `SV-761` through `SV-770` for covergroup ownership and
...
relocation.
V2 Batch 159 adds `SV-791` through `SV-800` for governed unmodified UVM 1.2
and UVM 2020-3.1 sources, bounded object/component/registry/factory/resource/
configuration/command-line/reporting services, simulation-owned isolation,
portable artifacts, engine/cache/trace parity, resource ceilings, and exact
rejection behavior.
V2 Batch 160 adds `SV-801` through `SV-810` for simulation-owned phase/domain
graphs and execution, objections/drains/quiescence, typed TLM1 and TLM2,
activity/debug and foreign snapshots, portable checkpoints, deterministic
race/deadlock handling, all 33 cataloged diagnostic families, and exact
unmodified UVM 1.2/2020.3.1 engine/artifact/cache parity.

## Existing release evidence

The composed local gates currently establish:

- 2,023 production diagnostics are cataloged;
- 769 authored C/C++ sources remain within the 2,500-line hard limit; any file
  that exceeds it must be refactored below the 2,000-line target;
- all 1,250 required matrix rows are executable with no explicit evidence gap;
