...
The machine gate composes the final release audit, then parses all 287 rows
...
covers all 287 rows and 53 distinct runtime evidence files. The complete
...
Batch 158 adds `VH-270` through `VH-279` for the governed VHPI ABI and loader,
hierarchy, types, values, drivers, time and callbacks, foreign bindings,
associations and I/O, exact/portable checkpoint restart, invalidation,
independent C/C++ images, engine transcripts, negative behavior, and
relocation. All ten rows are `execute` with complete evidence.
