...
|---|---|---|
| `B130-T7-DIAGNOSTICS` | 1,631 production diagnostic codes | Every emitted `FSIM-*` diagnostic is unique in and exactly matched by `docs/diagnostics.md`; stale catalog entries and undocumented emissions fail the gate |
| `B130-T7-SOURCES` | 432 authored C/C++ source and test files | Every file is at most 2,000 lines with an empty exception allowlist |
| `B130-T7-LICENSES` | 516 authored repository files | Every owned build, workflow, documentation, example, header, source, and test artifact carries an Apache-2.0 SPDX identifier; the root license is Apache-2.0 |
