...
provenance, updated through v2 Batch 172 Change 20. The inventories are release
...
|---|---|---|
| `B130-T7-DIAGNOSTICS` | 2,516 production diagnostic codes | Every emitted `FSIM-*` diagnostic is unique in and exactly matched by `docs/diagnostics.md`; stale catalog entries and undocumented emissions fail the gate |
| `B130-T7-SOURCES` | 1,100 authored C/C++ source and test files | Every file is at most 2,500 lines with a 2,000-line refactor target and an empty hard-limit exception list |
| `B130-T7-LICENSES` | 1,304 authored repository files | Every owned build, workflow, documentation, example, header, source, and test artifact, including `.clang-format`, carries an Apache-2.0 SPDX identifier; the root license is Apache-2.0 |
...
| `B130-T7-CONFORMANCE` | 100 expectations in 28 fixtures owned by 27 CTests | IDs, source identities, expected outcomes, fixture/test ownership, all required evidence modes, and the exact sorted digest remain frozen |
...
v2 Batch 162 through Change 19 adds object-policy/copy/packer, synchronization,
command-line, test-runner, source-smoke, platform-limit, and executable
conformance/closure/documentation audit ownership as 27 SPDX-owned artifacts,
including 17 bounded C/C++ headers/sources and eight additional authored test/control
owners. Seventeen production diagnostic codes advance the reviewed inventory
to 2,071 diagnostics, 826 bounded C/C++ sources, 937 authored artifacts, and
313 authored test/control files. The exact upstream UVM trees remain governed
external build products; no upstream source is vendored, patched, or counted.

v2 Batch 163 Change 1 adds the clause-indexed VHDL-2008/embedded-PSL gap
inventory and its registered machine contract as two SPDX-owned release files.
The reviewed authored total advances to 939. Diagnostics, bounded C/C++
sources, authored test/control owners, conformance expectations, reviewed IEEE
third-party bytes, and external provenance identities remain unchanged.
...
v2 Batch 168 through Change 19 advances the synchronized live inventory to
2,389 diagnostics, 974 bounded C/C++ sources, 1,139 SPDX-owned files, and 363
