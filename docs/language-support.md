...
|---|---|---|---|
| VHDL units | `library`/`use`/context-reference clauses retained on their following unit; entities, architectures, scalar integer/Boolean/bit generics, parameterized packed ranges, ports, signals, bounded packed process variables, direct-entity and component-style instances with positional-then-named `generic map` actuals, labeled `if`/`else`, integer-range `for`, and constant-choice `case` generate regions, and unguarded labeled block statements containing local packed signals, concurrent assignments, processes, instances, and nested regions | Recursively elaborated per-occurrence generic specializations with named/positional whole-signal `port map` associations, specialization-selected or always-selected scoped behavior with scope-qualified locals, and persistent process-variable registers | Context declarations and visibility resolution are not implemented; no packages, general generic types/subprograms/packages, configurations, guarded blocks, range choices in case generate, generated constants/subprograms or other declarative items, expression/`open` port actuals, or process declarative items beyond bounded variables |
...
| Verilog/SV units | Modules, ANSI and basic non-ANSI ports, nets/variables, packed constant or parameterized ranges, integral value parameters/localparams, module instances with named or positional parameter overrides, explicit or implicit conditional/canonical inline-`genvar`/constant-choice generates, and direct or named static contents in explicit generate regions containing local packed signals, continuous assignments, processes, instances, and nested regions | Recursive hierarchy with per-instance integral constant specialization, specialization-selected or always-selected generated behavior with scope-qualified locals, loop-variable substitution, and named or positional whole-signal connections | Parameter types are bounded to implicit integral, `int`, `integer`, `bit`, `logic`, and `reg`; no type/string parameters, interfaces, packages, external-`genvar`/increment-operator loops, loop-variable shadowing, generated parameters/functions/tasks, expression port actuals, or unpacked arrays |
| Verilog/SV statements | `assign`, event-controlled `always`/`always_ff`, inferred `always @*`/`always_comb`/`always_latch`, `initial`, blocks, leading packed procedural variables, `if`/`else`, exact `case` with comma-separated choices and `default`, blocking/NBA assignments, integer delays, any-change/`posedge`/`negedge`/wildcard procedural controls, `$finish`, and bounded immediate assertions with an optional literal `$error` message | Whole and constant selected packed signal/local assignments, nested `if`/`else` with packed four-state truth conversion, deterministic simple-expression wildcard dependencies, time-zero `always_comb`/`always_latch`, ordered exact four-state case matching, dynamic any-change and scalar edge-filtered suspension, packed-condition immediate assertions; integer `#` delays inherit and scale by the active `` `timescale`` | Wildcard inference does not inspect function/task bodies; no fractional delays, loops, `casez`/`casex`/`case inside`, `unique`/`unique0`/`priority` case qualifiers, dynamic or indexed part-select assignment targets, nested selected targets/scopes, automatic variables, tasks/functions, fork, general expression controls, named events, files, dynamic data, assertion action blocks beyond the bounded `$error` form, or procedural force |
| Verilog/SV expressions | Identifiers, sized literals, strings, unary and common binary syntax, conditional (`?:`), index/part-select/concatenation syntax, call syntax | Identifiers/literals, unary plus/minus, bitwise complement (`~`), vector-aware logical negation (`!`), mixed-width logical and/or, unary and/or/xor reductions, bitwise and/or/xor, logical left/right shifts, equal-width signed/unsigned add/subtract/multiply/divide/remainder and relational comparisons, equality/inequality, equal-width conditional alternatives under a scalar condition with four-state bit merging, constant in-range bit/part selects with declared-range mapping, and packed concatenations of statically sized operands | Arithmetic shifts, exponentiation, dynamic/out-of-range selects, indexed part-selects, replication concatenation, and call forms are not lowered; conditional vector truth conversion, expression side effects/short-circuit observation, and full self-determined sizing remain pending |
