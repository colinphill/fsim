...
| Verilog/SV expressions | Identifiers, sized literals, strings, unary and common binary syntax, conditional (`?:`), index/part-select/concatenation syntax, call syntax | Identifiers/literals, bitwise complement (`~`), bitwise and/or/xor, unsigned add, equality, and equal-width conditional alternatives under a scalar condition with four-state bit merging | Logical not (`!`) and most arithmetic, logical, relational, index/part-select, concatenation, and call forms are not lowered; conditional vector truth conversion and full self-determined sizing remain pending |
