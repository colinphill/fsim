...
| Verilog/SV statements | `assign`, event-controlled `always`/`always_ff`, `initial`, blocks, leading packed procedural variables, `if`/`else`, blocking/NBA assignments, integer delays, any-change/`posedge`/`negedge` procedural `@(signal-list)` controls, `$finish`, and bounded immediate assertions with an optional literal `$error` message | Whole-signal forms, undelayed blocking local-variable assignment, dynamic any-change and scalar edge-filtered suspension, scalar immediate assertions; integer `#` delays inherit and scale by the active `` `timescale`` | No fractional delays, loops, case, nested local scopes, automatic variables, tasks/functions, fork, wildcard/expression procedural controls, named events, files, dynamic data, assertion action blocks beyond the bounded `$error` form, or procedural force |
