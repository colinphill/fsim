...
| Verilog/SV statements | `assign`, event-controlled `always`/`always_ff`, `initial`, blocks, leading packed procedural variables, `if`/`else`, blocking/NBA assignments, integer delays, any-change procedural `@(signal-list)` controls, `$finish`, and bounded immediate assertions with an optional literal `$error` message | Whole-signal forms, undelayed blocking local-variable assignment, dynamic any-change suspension, scalar immediate assertions; integer `#` delays inherit and scale by the active `` `timescale`` | No fractional delays, loops, case, nested local scopes, automatic variables, tasks/functions, fork, edge-qualified/wildcard procedural controls, named events, files, dynamic data, assertion action blocks beyond the bounded `$error` form, or procedural force |
