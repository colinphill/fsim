...
| Verilog/SV expressions | Identifiers, sized literals, strings, unary and common binary syntax, conditional (`?:`), index/part-select/concatenation syntax, call syntax | Identifiers/literals, bitwise complement (`~`), vector-aware logical negation (`!`), bitwise and/or/xor, unsigned add, equality/inequality, unsigned relational comparisons, and equal-width conditional alternatives under a scalar condition with four-state bit merging | Logical and/or, signed comparison, and most arithmetic, shift, index/part-select, concatenation, and call forms are not lowered; conditional vector truth conversion and full self-determined sizing remain pending |
