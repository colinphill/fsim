...
|---|---|---|---|
| VHDL units | `library`/`use`/context-reference clauses retained on their following unit; entities, architectures, scalar integer/Boolean/bit generics, parameterized packed ranges, ports, signals, bounded packed process variables, direct-entity and component-style instances with positional-then-named `generic map` actuals, labeled `if`/`else generate`, and integer-range `for generate` instance regions | Recursively elaborated per-occurrence generic specializations with named/positional whole-signal `port map` associations, specialization-selected conditional/iterative generate hierarchy, and persistent process-variable registers | Context declarations and visibility resolution are not implemented; no packages, general generic types/subprograms/packages, configurations, case generate, non-instance generate bodies, expression/`open` port actuals, or process declarative items beyond bounded variables |
...
`label[index]` path components. This does not yet establish
