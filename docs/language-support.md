...
| Preprocessing/directives | Quoted and angle includes, manifest/CLI definitions, object/function macros with default arguments, multiline replacement, argument substitution, token concatenation/stringification, `__FILE__`/`__LINE__`, `undef`, nested conditional compilation, legal `` `timescale`` context, recognized `` `default_nettype``, and ordered `file`/`source-set`/`combined` compilation-unit policies | Included units and macro-selected executable source enter the normal frontend; source-set/combined roots share macro, conditional, and parser directive state while retaining library ownership; integer delays are scaled and `auto` selects the finest attached precision; ordered compilation-unit snapshots participate in analysis/native cache identity | Remaining standard compiler-directive semantics, fractional delays, `timeunit`/`timeprecision`, and implicit-net/default-nettype semantics remain incomplete; unsupported directives receive targeted errors |
