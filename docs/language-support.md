...
|---|---|---|---|
| VHDL units | `library`/`use`/context-reference clauses retained on their following unit; entities, architectures, scalar integer/Boolean/bit generics, parameterized packed ranges, ports, signals, bounded packed process variables, direct-entity and component-style instances with positional-then-named `generic map` actuals | Recursively elaborated per-occurrence generic specializations with named/positional whole-signal `port map` associations and persistent process-variable registers | Context declarations and visibility resolution are not implemented; no packages, general generic types/subprograms/packages, configurations, generates, expression/`open` port actuals, or process declarative items beyond bounded variables |
...
| Verilog/SV units | Modules, ANSI and basic non-ANSI ports, nets/variables, packed constant or parameterized ranges, integral value parameters/localparams, and module instances with named or positional parameter overrides | Recursive hierarchy with per-instance integral constant specialization and named or positional whole-signal connections | Parameter types are bounded to implicit integral, `int`, `integer`, `bit`, `logic`, and `reg`; no type/string parameters, interfaces, packages, generates, expression port actuals, or unpacked arrays |
