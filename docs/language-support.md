...
|---|---|---|---|
| VHDL units | `library`/`use`/context-reference clauses retained on their following unit; entities, architectures, scalar integer/Boolean/bit generics, parameterized packed ranges, ports, signals, bounded packed process variables, direct-entity and component-style instances with positional-then-named `generic map` actuals, and labeled `if`/`else`, integer-range `for`, and constant-choice `case` generate regions containing local packed signals, concurrent assignments, processes, instances, and nested regions | Recursively elaborated per-occurrence generic specializations with named/positional whole-signal `port map` associations, specialization-selected generated behavior with scope-qualified locals, and persistent process-variable registers | Context declarations and visibility resolution are not implemented; no packages, general generic types/subprograms/packages, configurations, range choices in case generate, generated constants/subprograms or other declarative items, expression/`open` port actuals, or process declarative items beyond bounded variables |
