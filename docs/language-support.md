...
and constant-function plus interpreter/LLVM O0/O2 selection agree. `case matches`,
tagged patterns, and variable-array/open/type/class sets remain unsupported.

SystemVerilog case-qualifier status update: `unique`, `unique0`, and `priority`
may qualify exact, `casez`, `casex`, and bounded `case inside` statements.
