...
also be locally constant direction-preserving `[left:right]` slices. A child
input sees a read-only formal-typed ordinal view; output and inout writes merge
one complete formal value into one copied parent value and commit only the
selected range. Equal-count ranges may use different indices and directions,
while width, signedness, state domain, and X/Z bits remain exact. Recursive
aliasing carries the view through nested/generated hierarchy, and disjoint
selected writers coexist while overlaps retain deterministic rejection.
Schema 43 and container semantic revision 19 cover the formal profile,
object operation/identity, specialization, and source provenance; selected
parent bounds remain validated runtime alias metadata.
Variable or unknown bounds, direction reversal, indexed `+:`/`-:` unpacked
selections, indirect slice receivers, general expression port actuals, slice
...
wait. Compatible direct static-array slice actuals use formal-typed ordinal
...
types, fork/join and general event expressions, indexed/dynamic module-port
actuals, DPI, generated tasks, and cross-language calls remain deferred. Nonblocking
