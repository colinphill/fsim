...
Capture execute(
  ...
  simulation.set_signal_change_hook(
      [&](const fsim::runtime::simir::SignalId signal,
        ...
        vcd.set_time(time);
        if (signal == *native) vcd.change(vcd_native, value);
        if (signal == *mixed) vcd.change(vcd_mixed, value);
      });
  ...
  capture.forced_result =
      simulation.read_signal(*mixed).to_msb_string();
  simulation.release_signal(*mixed);
  assert(!simulation.signal_is_forced(*mixed));
  capture.released_result =
      simulation.read_signal(*mixed).to_msb_string();
  capture.run = simulation.run();
  capture.native_result =
      simulation.read_signal(*native).to_msb_string();
  capture.mixed_result =
      simulation.read_signal(*mixed).to_msb_string();
  vcd.flush();
...
Capture run_once(
  ...
  if (!project) fsim::diagnostic::print_text(std::cerr, diagnostics);
  assert(project);
  assert(project->design.roots()
         == std::vector<std::string>({"native", "mixed"}));
  assert(project->design.verilog_specify_paths().size() == 6);
  ...
  assert(restored->verilog_specify_paths().size() == 6);
...
Capture run_relocated_library(
  ...
  if (!exported) fsim::diagnostic::print_text(std::cerr, diagnostics);
...
begin
  ...
  for (const auto optimization : {
    ...
    const auto mapped = run_relocated_library(
        ...
        directory.path
            / (optimization == fsim::project::Optimization::o0
                   ? "specify-o0.fsimlib" : "specify-o2.fsimlib"));
