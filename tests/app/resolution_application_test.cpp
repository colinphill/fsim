    ...
    const fsim::app::SimulationEngine engine)
{
    ...
    assert(project);
    const auto shared = project->design.find_signal("resolved_top.shared");
    assert(shared);
    assert(
    ...
    const auto vcd_signal = vcd.declare_signal("resolved_top.shared", 1);
    ...
    capture.final_value = simulation.read_signal(*shared).to_msb_string();
...
void verify_mode(
    ...
    const fsim::project::Optimization optimization)
{
    const auto config = make_config(directory, sv_source, vhdl_source, optimization);
  ...
  initial begin
    ...
    const auto execute = [&](const fsim::app::SimulationEngine engine) {
        ...
        assert(simulation.read_driver(
                             directional_process, directional_target)
                   .width()
        ...
        capture.values.back() = simulation.read_signal(chain_signal).to_msb_string();
        capture.values[capture.values.size() - 2] = simulation.read_signal(conditional_signal).to_msb_string();
        vcd.flush();
        capture.vcd = vcd_output.str();
        return capture;
    };
    ...
    if (reference.values != expected_strengths) {
        for (const auto& value : reference.values)
            std::cerr << value << ' ';
        std::cerr << '\n';
    }
...
begin
  ...
  begin
    ...
    const auto execute = [&](const fsim::app::SimulationEngine engine) {
        ...
        capture.boundary_conversions = project->design.boundary_conversions().size();
        ...
        for (const auto& specialization : project->design.specializations()) {
            if (specialization.instance == "mixed_timing_top.mid") {
                capture.construction_identities = specialization.parameter_identity_values;
            }
        }
        ...
        simulation.set_signal_change_hook(
            [&](const fsim::runtime::simir::SignalId signal,
                ...
                capture.changes.push_back({ std::string { names[index] }, value.to_msb_string(), time, delta });
                vcd.set_time(time);
    ...
    assert(reference.vcd.find("$timescale 1ps $end")
        != std::string::npos);
    assert((reference.outputs == std::vector<std::tuple<std::string, fsim::runtime::SimulationTick, std::uint64_t>> { { "region=11", 0, 0 } }));
