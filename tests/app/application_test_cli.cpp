...
void ApplicationTestFixture::test_preprocessing_debug_and_cli()
{
    ...
    const auto incompatible_metadata_path = incompatible_library / fsim::library::kMetadataFilename;
    std::filesystem::permissions(
    ...
    unused_mapping_config.library_mappings.front().path = directory / "does-not-exist.fsimlib";
...
initial begin value = 1'b0; #1 $finish; end
    ...
    const auto vhdl_mapped_signal = vhdl_mapped_simulation.find_signal("mapped_vhdl_consumer.result");
    ...
    mapped_configuration_config.project.top = "vhdl:vendor.mapped_gate_configuration";
    mapped_configuration_config.build.cache_path = directory / "mapped-vhdl-configuration-cache";
    ...
    const auto dependent_metadata_path = dependent_vendor / fsim::library::kMetadataFilename;
    std::filesystem::permissions(
    ...
    const auto relocation_native_keys = relocation_first->specialization_cache_keys;
    assert(std::ranges::any_of(
...
mapped_parameter #(.WIDTH(8)) child(.value(value), .result(result));
    ...
    const auto preprocessor_include = directory / "preprocessor-include";
    std::filesystem::create_directories(preprocessor_include);
    const auto preprocessor_header = preprocessor_include / "values.svh";
    const auto preprocessor_source = directory / "preprocessor.sv";
...
`endif
    ...
    preprocessor_config.project.top = "sv:work.preprocessor_app";
    preprocessor_config.build.cache_path = directory / "preprocessor-cache";
    preprocessor_config.source_sets.clear();
    fsim::project::SourceSet preprocessor_sources;
    preprocessor_sources.language = fsim::project::Language::system_verilog;
    ...
    const auto preprocessor_checked = fsim::app::check_project(
        preprocessor_config,
    ...
    auto [preprocessor_key_a, preprocessor_miss_a,
        preprocessor_reference_a] = run_preprocessed(fsim::app::SimulationEngine::interpreter);
    auto [preprocessor_key_a_warm, preprocessor_hit_a,
        preprocessor_hybrid_a] = run_preprocessed(fsim::app::SimulationEngine::compiled);
    ...
    auto [preprocessor_key_b, preprocessor_miss_b,
        preprocessor_reference_b] = run_preprocessed(fsim::app::SimulationEngine::interpreter);
    auto [preprocessor_key_b_warm, preprocessor_hit_b,
        preprocessor_hybrid_b] = run_preprocessed(fsim::app::SimulationEngine::compiled);
    ...
    const auto shared_macro_source = directory / "shared-macros.sv";
    const auto shared_module_source = directory / "shared-module.sv";
...
`resetall
    ...
    shared_preprocessor_config.project.name = "shared-preprocessor-test";
    shared_preprocessor_config.project.top = "sv:work.shared_preprocessor_app";
    shared_preprocessor_config.build.cache_path = directory / "shared-preprocessor-cache";
    shared_preprocessor_config.source_sets.clear();
    fsim::project::SourceSet shared_preprocessor_sources;
    shared_preprocessor_sources.language = fsim::project::Language::system_verilog;
    ...
    const auto semantic_source_count = shared_checked->semantics.source_spans().size();
    ...
    auto [shared_key_a, shared_reference_a] = run_shared_preprocessor(
        shared_preprocessor_config,
        fsim::app::SimulationEngine::interpreter);
    auto [shared_key_a_warm, shared_hybrid_a] = run_shared_preprocessor(
        shared_preprocessor_config,
        fsim::app::SimulationEngine::compiled);
    ...
    auto [shared_key_b, shared_reference_b] = run_shared_preprocessor(
        shared_preprocessor_config,
        fsim::app::SimulationEngine::interpreter);
    auto [shared_key_b_warm, shared_hybrid_b] = run_shared_preprocessor(
        shared_preprocessor_config,
        fsim::app::SimulationEngine::compiled);
    ...
    independent_file_config.source_sets.front().compilation_unit = "file";
    ...
    auto combined_preprocessor_config = shared_preprocessor_config;
    combined_preprocessor_config.project.name = "combined-preprocessor-test";
    combined_preprocessor_config.build.cache_path = directory / "combined-preprocessor-cache";
    ...
    auto [combined_key, combined_reference] = run_shared_preprocessor(
        combined_preprocessor_config,
        fsim::app::SimulationEngine::interpreter);
    auto [combined_key_warm, combined_hybrid] = run_shared_preprocessor(
        combined_preprocessor_config,
        fsim::app::SimulationEngine::compiled);
    ...
    const auto mixed_manifest = std::filesystem::path { FSIM_TEST_SOURCE_DIR }
        / "examples/vertical_slice/fsim.toml";
    auto mixed_config = fsim::project::load(mixed_manifest, mixed_diagnostics);
    assert(mixed_config);
    mixed_config->build.cache_path = directory / "mixed-cache";
    mixed_config->run.trace_file.reset();
    auto mixed_reference_project = fsim::app::build_project(*mixed_config, mixed_diagnostics);
    auto mixed_hybrid_project = fsim::app::build_project(*mixed_config, mixed_diagnostics);
    ...
    const auto three_language_manifest = std::filesystem::path { FSIM_TEST_SOURCE_DIR }
    ...
    three_language_config->build.cache_path = directory / "three-language-cache";
    three_language_config->run.trace_file.reset();
    auto three_language_reference_project = fsim::app::build_project(
        *three_language_config, three_language_diagnostics);
    auto three_language_hybrid_project = fsim::app::build_project(
        *three_language_config, three_language_diagnostics);
    ...
    const auto logic9 = fsim::app::parse_value("uWlH-", 5, error);
    ...
    assert(
        ...
        != std::string::npos);
    const auto source_breakpoint_hit = transcript.find("hit breakpoint 3: ");
    assert(source_breakpoint_hit != std::string::npos);
    const auto source_breakpoint_end = transcript.find('\n', source_breakpoint_hit);
    assert(
        transcript.find(
    ...
    assert(
        ...
        != std::string::npos);
    const auto first_finished = transcript.find("simulation has finished");
    ...
    const auto debug_native_cache = compiled_debug_simulation.native_cache_statistics();
    ...
    assert(
        ...
        != std::string::npos);
    const auto unavailable = poisoned_output.str().find(
...
trace_filters = ["__none__"]
    ...
    const auto unicode_directory = directory / fsim::support::path_from_utf8("tool-path-\xC3\xA9");
    std::filesystem::create_directories(unicode_directory);
    const auto unicode_source = unicode_directory / fsim::support::path_from_utf8("source-\xCE\xBB.sv");
    {
...
files = ["different_filename.sv"]
    ...
    test_cli_trace_and_timescale(services);
}
