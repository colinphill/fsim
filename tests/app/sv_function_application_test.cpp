    ...
    const fsim::app::SimulationEngine engine)
{
    ...
    const auto recursive = simulation.find_signal(
        "function_top.recursive_witness");
    assert(recursive);
    capture.recursive = simulation.read_signal(*recursive).to_msb_string();
    ...
    const auto qualified_returned = simulation.design().find_container(
    ...
    const auto associative_returned = simulation.design().find_container(
        "function_top.associative_returned");
    const auto partial_queue_first = simulation.design().find_container(
        "function_top.partial_queue_first");
    const auto partial_queue_second = simulation.design().find_container(
        "function_top.partial_queue_second");
    const auto conditional_returned = simulation.design().find_container(
        "function_top.conditional_returned");
    const auto located_returned = simulation.design().find_container(
    ...
    capture.qualified_returned = simulation.read_container_object(*qualified_returned);
    capture.selected = simulation.read_container_object(*selected);
    capture.dynamic_returned = simulation.read_container_object(*dynamic_returned);
    capture.queue_returned = simulation.read_container_object(*queue_returned);
    capture.associative_returned = simulation.read_container_object(*associative_returned);
    capture.partial_queue_first = simulation.read_container_object(*partial_queue_first);
    capture.partial_queue_second = simulation.read_container_object(*partial_queue_second);
    capture.conditional_returned = simulation.read_container_object(*conditional_returned);
    capture.located_returned = simulation.read_container_object(*located_returned);
    return capture;
}
    ...
    const std::array<std::string, 23>& expected)
{
    ...
    std::string expected_recursive(137, '0');
    expected_recursive[0] = '1';
    expected_recursive[64] = '1';
    expected_recursive[133] = '1';
    expected_recursive[136] = '1';
    assert(capture.recursive == expected_recursive);
    ...
    assert(capture.call_operations == 43);
    ...
    for (std::size_t ordinal = 0; ordinal < 4; ++ordinal) {
        ...
        assert(
            capture.selected.elements[ordinal + 1]
                ...
                .aval
            == shifted_expected);
        const auto qualified_expected = UINT64_C(2) + ordinal
            + (expected[0] == "00101011"
                    ? UINT64_C(1)
                    : UINT64_C(0));
...
int main()
{
    ...
    const auto package_source = directory.path / "function_package.sv";
...
module function_top #(
  ...
  logic [136:0] recursive_witness;

  function automatic logic [136:0] recursive_mix(
      input int depth,
      input logic [136:0] retained);
    logic [136:0] first;
    if (depth == 0)
      return retained;
    first = recursive_mix(depth - 1, retained);
    return first ^ recursive_mix(depth - 1, '0);
  endfunction
  ...
  initial begin
    recursive_witness = '0;
    recursive_witness[136] = 1'b1;
    recursive_witness[72] = 1'b1;
    recursive_witness[3] = 1'b1;
    recursive_witness[0] = 1'b1;
    recursive_witness = recursive_mix(4, recursive_witness);
