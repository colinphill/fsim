...
void ApplicationTestFixture::test_non_project_cli() {
  ...
  const auto conflict_trace_text =
      fsim::support::path_to_utf8(conflict_trace);
  ...
  const auto compile =
      cli::parse_arguments(static_cast<int>(compile_arguments.size()),
  ...
  assert(compile->trace_compression ==
  ...
  const auto elaborate =
      cli::parse_arguments(static_cast<int>(elaborate_arguments.size()),
  ...
  assert(elaborate->trace_compression ==
         project::TraceCompression::deterministic);
  ...
  const auto simulate =
      cli::parse_arguments(static_cast<int>(simulate_arguments.size()),
  ...
  assert(simulate->trace_compression ==
         project::TraceCompression::deterministic);
  ...
  assert(cli::run(static_cast<int>(help_arguments.size()),
                  help_arguments.data(), services, output, error) == 0);
  ...
  assert(cli::run(static_cast<int>(compile_arguments.size()),
                  compile_arguments.data(), services, output, error) == 0);
  ...
  assert(cli::run(static_cast<int>(elaborate_arguments.size()),
                  elaborate_arguments.data(), services, output, error) == 0);
...
SC_FSIM_EXPORT_AS(IncrementalTop, "first");
  ...
  output.str({});
  error.str({});
  assert(cli::run(static_cast<int>(systemc_compile_arguments.size()),
                  systemc_compile_arguments.data(), production_services, output,
                  error) == 0);
  assert(error.str().empty());
  output.str({});
  error.str({});
  ...
  const auto systemc_plugin_metadata =
      systemc_plugin / systemc::kIncrementalPluginMetadataFilename;
  const auto hidden_systemc_plugin_metadata =
      systemc_plugin /
  ...
  const auto metadata =
      artifact::load_object_metadata(object, object_diagnostics);
  ...
  const auto provenance_signature = [](const auto& provenance) {
    ...
    for (const auto& item : provenance) {
      result.push_back(item.path + "|" + item.semantic_unit + "|" +
                       std::to_string(item.unit.value()) + "|" +
                       std::to_string(item.source.value()) + "|" +
                       item.source_path + "|" + item.standard + "|" +
  ...
  const auto runtime_state =
      app::serialize_runtime_state(object_build->design, state_diagnostics);
  const auto semantic_state =
      app::serialize_semantic_state(object_build->semantics, state_diagnostics);
  ...
  const auto published_design_metadata =
      artifact::load_design_metadata(design, design_metadata_diagnostics);
  ...
  assert(published_design_metadata->roots.front().selected_identity ==
         "sv:work.tb");
  ...
  auto sdf_build =
      app::build_objects(object_config, object_inputs, sdf_build_diagnostics);
  ...
  auto parsed_sdf =
      frontend::parse_sdf({support::path_to_utf8(sdf_source), sdf_text});
  ...
  assert(sdf_scope.ok());
  const auto sdf_cells =
      app::resolve_sdf_cells(sdf_scope.scope, sdf_build->design);
  assert(sdf_cells.ok());
  const auto sdf_endpoints =
      app::resolve_sdf_endpoints(sdf_cells.resolution, sdf_build->design);
  if (!sdf_endpoints.ok()) {
  ...
  const auto sdf_mapping =
      app::validate_sdf_mapping(sdf_endpoints.resolution, sdf_build->design);
  ...
  const auto sdf_snapshot =
      app::decode_sdf_schema(sdf_schema.bytes, "fsim-compiler-compat-v1",
  ...
  const auto sdf_metadata =
      artifact::load_design_metadata(sdf_design, sdf_metadata_diagnostics);
  ...
  const auto run_sdf_design = [&](const std::filesystem::path& path,
    ...
    if (!loaded_sdf) diagnostic::print_text(std::cerr, diagnostics);
    ...
    assert(app::sdf_phase_artifacts(*loaded_sdf)
               .front()
               ->snapshot()
               .annotation() == sdf_metadata->sdf_annotations.front());
  ...
  const auto sdf_cold =
      run_sdf_design(sdf_design, app::SimulationEngine::compiled);
  const auto sdf_warm =
      run_sdf_design(sdf_design, app::SimulationEngine::compiled);
  ...
  const auto sdf_relocated =
      run_sdf_design(relocated_sdf_design, app::SimulationEngine::compiled);
  ...
  auto loaded_design =
      app::load_design_artifact(design, design_load_diagnostics);
  if (!loaded_design) {
  ...
  app::Simulation standalone_simulation{std::move(*loaded_design), 1000,
                                        app::SimulationEngine::interpreter};
  assert(
      provenance_signature(standalone_simulation.verilog_scope_provenance()) ==
      object_provenance_signature);
  ...
  auto debug_design =
      app::load_design_artifact(design, debug_design_diagnostics);
      ...
      artifact::compute_design_digest(alternate_metadata);
  {
    ...
    alternate_output.write(alternate_bytes.data(), static_cast<std::streamsize>(
                                                       alternate_bytes.size()));
    assert(alternate_output);
  }
  diagnostic::Engine alternate_design_diagnostics;
  auto alternate_loaded =
      app::load_design_artifact(alternate_design, alternate_design_diagnostics);
  ...
  corrupt_trace_metadata.design_digest =
      artifact::compute_design_digest(corrupt_trace_metadata);
  {
...
begin
  ...
  const auto vhdl_metadata =
      artifact::load_object_metadata(vhdl_object, vhdl_metadata_diagnostics);
  ...
  const auto ordered_vhdl =
      app::load_objects(ordered_vhdl_inputs, ordered_vhdl_diagnostics);
  ...
  assert(std::ranges::all_of(ordered_vhdl->objects, [](const auto& provenance) {
    return provenance.language == "vhdl" && provenance.standard == "2008" &&
           provenance.compatibility_profile == "fsim-synopsys-ieee-compat-v2";
  }));
  ...
  const auto standalone_vhdl =
      app::load_objects(ordered_vhdl_inputs, standalone_vhdl_diagnostics);
...
`endif
  ...
  const auto isolated_metadata =
      artifact::load_object_metadata(isolated_object, isolated_diagnostics);
