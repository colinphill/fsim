...
void ApplicationTestFixture::test_class_simulation_integration() {
  ...
  for (const auto engine : {fsim::app::SimulationEngine::interpreter,
    ...
    assert(
        ...
        uvm_param_item != built->systemverilog_class_specializations.end() &&
        uvm_factory_item != built->systemverilog_class_specializations.end() &&
        test_uvm_component != built->systemverilog_class_specializations.end());
    ...
    const auto test_uvm_component_identity =
        test_uvm_component->declaration_identity;
    ...
    if (engine == fsim::app::SimulationEngine::interpreter) {
      ...
      const auto restored_derived =
          std::ranges::find(*restored, derived_specialization,
      ...
      const auto restored_wide = std::ranges::find(
          ...
          &fsim::frontend::SystemVerilogClassPropertyLayout::name);
      assert(restored_wide != restored_derived->properties.end() &&
      ...
      assert(
          ...
          restored_interface->type.systemverilog_interface_type ==
              "class_view_if" &&
          restored_interface->type.systemverilog_interface_modport == "view" &&
          restored_interface->type.systemverilog_class_parameter_actuals
                  .size() == 1 &&
          restored_interface->type.systemverilog_class_parameter_actuals.front()
                  .name == std::optional<std::string>{"WIDTH"} &&
          restored_interface->type.systemverilog_class_parameter_actuals.front()
                  .value.text == "4");
      auto malformed_classes = *restored;
      auto malformed_derived =
          std::ranges::find(malformed_classes, derived_specialization,
                            &fsim::frontend::SystemVerilogClassSpecialization::
                                specialization_identity);
      ...
      assert(restored_constraint_hir->classes().size() ==
      ...
      assert(restored_constraint_class !=
    ...
    if (engine == fsim::app::SimulationEngine::interpreter) {
      ...
      auto stale_debug_project =
          fsim::app::build_project(config, stale_debug_diagnostics);
      ...
      const auto stale_debug_root =
          stale_debug_simulation.create_uvm_root("stale-debug");
      const auto stale_debug_component =
          stale_debug_simulation.allocate_uvm_component(
    ...
    const auto api_uvm_activity_token =
        simulation.add_uvm_activity_hook([&](const auto &event) {
          api_uvm_activity_callbacks.push_back(event);
        });
    ForeignActivityCapture foreign_activity;
    std::uint64_t foreign_activity_token{};
    const auto foreign_host =
        fsim::runtime::make_systemverilog_uvm_foreign_host(
            simulation.uvm_foreign());
    assert(foreign_host.abi_version == FSIM_UVM_FOREIGN_ABI_VERSION &&
           foreign_host.struct_size == sizeof(fsim_uvm_foreign_host_v1) &&
           foreign_host.simulation_identity ==
               simulation.uvm_foreign().simulation_identity() &&
           foreign_host.add_callback(
               foreign_host.context, capture_foreign_activity,
               &foreign_activity,
               &foreign_activity_token) == FSIM_UVM_FOREIGN_OK);
    std::cerr << "application classes: engine " << static_cast<unsigned>(engine)
              << " execute\n";
    const auto handle =
        simulation.allocate_class(derived_specialization, base_identity);
    ...
    for (std::size_t bit = 0; bit < 32; ++bit) {
      if (api_tlm_payload &&
          api_tlm_payload->value.get(bit) == fsim::runtime::Logic4::one) {
        api_tlm_low |= std::uint64_t{1} << bit;
      }
    }
    fsim::runtime::PackedLogic4 retained_tlm_value(96,
                                                   fsim::runtime::Logic4::one);
    assert(api_tlm1.try_put(api_tlm_port, {"work::engine_payload",
                                           std::move(retained_tlm_value),
                                           0,
                                           api_component_root,
                                           {},
                                           0}));
    ...
    api_tlm2.set_blocking_handler(
        api_tlm2_target, [](auto &transaction, auto &delay) {
          ...
          transaction.response_status =
              fsim::runtime::SystemVerilogUvmTlm2ResponseStatus::Ok;
          delay += 4;
        });
    ...
    api_tlm2_payload.command =
        fsim::runtime::SystemVerilogUvmTlm2Command::Write;
    ...
    const auto api_build_phase =
        api_schedule.phase(fsim::runtime::SystemVerilogUvmPhaseKind::Build);
    assert((
        simulation.uvm_components().full_name(api_component_child) ==
            "api_top.api_child" &&
        api_tlm_payload &&
        api_tlm_payload->nominal_type == "work::engine_payload" &&
        api_tlm_payload->value.width() == 96 && api_tlm_low == 0x160a'cce5U &&
        api_tlm_payload->owner_endpoint == api_tlm_port &&
        api_analysis_result.success() &&
        api_analysis_result.deliveries.size() == 1 && api_analysis_calls == 1 &&
        api_analysis_width == 73 && api_tlm2_result.target == api_tlm2_target &&
        api_tlm2_result.payload.data.size() == 37 &&
        api_tlm2_result.payload.data.front() == 0xd2 &&
        api_tlm2_result.payload.extensions.size() == 1 &&
        api_tlm2_result.delay == 7 &&
        api_tlm2_result.payload.response_status ==
            fsim::runtime::SystemVerilogUvmTlm2ResponseStatus::Ok &&
        api_tlm2_nb_result.transaction &&
        api_tlm2_nb_result.sync ==
            fsim::runtime::SystemVerilogUvmTlm2Sync::Accepted &&
        simulation.uvm_components().parent(api_component_child) ==
            api_component_top &&
        simulation.uvm_components().lookup_root(
            api_component_root, "api_top.api_child") == api_component_child &&
        simulation.uvm_components().lookup_root(
            api_component_second_root, "api_top") == api_component_isolated &&
    ...
    assert((uvm_debug.domains.size() == 2 && uvm_debug.phases.size() == 21 &&
            uvm_debug.tlm1_endpoints.size() == 4 &&
            uvm_debug.tlm1_fifos.size() == 1 &&
            uvm_debug.tlm2_sockets.size() == 2 &&
            uvm_debug_text.find("api-first:api_top.engine_port") !=
                std::string::npos &&
            uvm_debug_text.find("api-first:api_top.initiator_socket") !=
                std::string::npos));
    assert((
        std::ranges::any_of(
            ...
            [](const auto &event) {
              return event.kind ==
                     ...
                     event.action ==
                         fsim::runtime::SystemVerilogUvmActivityAction::Created;
            }) &&
        std::ranges::any_of(api_uvm_activity_callbacks, [](const auto &event) {
          return event.kind ==
                     fsim::runtime::SystemVerilogUvmActivityKind::Transaction &&
                 event.action ==
                     fsim::runtime::SystemVerilogUvmActivityAction::Completed &&
                 event.detail == "tlm2-blocking";
        })));
    ...
    const auto foreign_capture_status =
        foreign_host.capture(foreign_host.context, &foreign_snapshot);
    ...
    assert(foreign_capture_status == FSIM_UVM_FOREIGN_OK &&
           foreign_snapshot.abi_version == FSIM_UVM_FOREIGN_ABI_VERSION &&
           foreign_snapshot.simulation_identity ==
               simulation.uvm_foreign().simulation_identity() &&
           foreign_snapshot.record_count >= 29);
    bool foreign_phase{};
    bool foreign_tlm1{};
    ...
    for (std::size_t index = 0; index < foreign_snapshot.record_count;
      ...
      const auto sized = foreign_host.copy_record(
          ...
          nullptr, 0, nullptr, 0);
      assert(sized == FSIM_UVM_FOREIGN_BUFFER_TOO_SMALL ||
             sized == FSIM_UVM_FOREIGN_OK);
      ...
      assert(foreign_host.copy_record(
                 ...
                 payload.data(), payload.size()) == FSIM_UVM_FOREIGN_OK);
      const std::string identity_text{identity.begin(), identity.end()};
      foreign_phase = foreign_phase || (record.kind == FSIM_UVM_FOREIGN_PHASE &&
                                        identity_text == "common.build");
      foreign_tlm1 =
          foreign_tlm1 || (record.kind == FSIM_UVM_FOREIGN_TLM1_FIFO &&
                           record.value == 1 && record.auxiliary == 2);
      foreign_tlm2_payload =
          foreign_tlm2_payload ||
          (record.kind == FSIM_UVM_FOREIGN_TLM2_TRANSACTION &&
           payload.size() == 37 && payload.front() == 0xd2);
    }
    assert(foreign_phase && foreign_tlm1 && foreign_tlm2_payload);
    const auto portable_uvm = simulation.capture_uvm_checkpoint();
    assert(portable_uvm && portable_uvm.artifact.time == simulation.now() &&
           portable_uvm.artifact.delta == simulation.delta() &&
           portable_uvm.artifact.external_state.callbacks == 1 &&
           portable_uvm.artifact.external_state.phase_processes == 0 &&
           std::ranges::none_of(portable_uvm.artifact.records,
                                [](const auto &record) {
                                  return record.kind ==
                                         FSIM_UVM_FOREIGN_PHASE_PROCESS;
                                }) &&
           std::ranges::any_of(portable_uvm.artifact.records,
                               [](const auto &record) {
                                 return record.kind == FSIM_UVM_FOREIGN_PHASE &&
                                        record.identity == "common.build" &&
                                        record.detail.find("successor=") !=
                                            std::string::npos;
                               }) &&
           std::ranges::any_of(portable_uvm.artifact.records,
                               [](const auto &record) {
                                 return record.kind ==
                                            FSIM_UVM_FOREIGN_TLM1_ENDPOINT &&
                                        record.detail.find("outbound=") !=
                                            std::string::npos;
                               }) &&
           std::ranges::any_of(portable_uvm.artifact.records,
                               [](const auto &record) {
                                 return record.kind ==
                                            FSIM_UVM_FOREIGN_TLM1_FIFO &&
                                        record.value == 1 &&
                                        record.payload.size() >
                                            sizeof(std::uint64_t);
                               }) &&
           std::ranges::any_of(
               portable_uvm.artifact.records, [](const auto &record) {
                 return record.kind == FSIM_UVM_FOREIGN_TLM2_TRANSACTION &&
                        record.payload.size() == 37 &&
                        record.detail.find("target=") != std::string::npos;
               }));
    ...
    ForeignActivityCapture teardown_activity;
    {
      ...
      assert(teardown_foreign.add_callback(
                 ...
                 teardown_token) == FSIM_UVM_FOREIGN_OK);
    }
    ...
    assert(bounded_foreign.capture(bounded_foreign_snapshot) ==
               FSIM_UVM_FOREIGN_OK &&
           bounded_foreign.capture(rejected_foreign_snapshot) ==
               FSIM_UVM_FOREIGN_RESOURCE_LIMIT &&
           bounded_foreign.diagnostic_code() == "FSIM-UVM-FOREIGN-002" &&
           bounded_foreign.release(bounded_foreign_snapshot) ==
               FSIM_UVM_FOREIGN_OK);
    api_tlm2.cancel_transaction(api_tlm2_nb_result.transaction);
    fsim_uvm_foreign_snapshot_v1 cancelled_foreign_snapshot{};
    assert(simulation.uvm_foreign().capture(cancelled_foreign_snapshot) ==
           FSIM_UVM_FOREIGN_OK);
    ...
    for (std::size_t index = 0; index < cancelled_foreign_snapshot.record_count;
      ...
      assert(status == FSIM_UVM_FOREIGN_BUFFER_TOO_SMALL ||
             status == FSIM_UVM_FOREIGN_OK);
      foreign_cancelled =
          foreign_cancelled ||
          (record.kind == FSIM_UVM_FOREIGN_TLM2_TRANSACTION &&
           record.state ==
               static_cast<std::uint32_t>(
                   fsim::runtime::SystemVerilogUvmTlm2TransactionState::
                       Cancelled));
    }
    assert(foreign_cancelled &&
           simulation.uvm_foreign().release(cancelled_foreign_snapshot) ==
               FSIM_UVM_FOREIGN_OK);
    ...
    const auto callback_order = [](const auto &phase_result) {
      ...
      for (const auto &event : phase_result.events) {
        if (event.callback ==
            fsim::runtime::SystemVerilogUvmPhaseCallbackKind::Execute) {
    ...
    const auto build_result =
        simulation.execute_uvm_function_phase(api_build_phase);
    ...
    for (const auto kind : remaining_function_phases) {
      const auto phase_result =
          simulation.execute_uvm_function_phase(api_schedule.phase(kind));
      assert((phase_result.success() &&
              phase_result.final_state ==
                  fsim::runtime::SystemVerilogUvmPhaseState::Done &&
              callback_order(phase_result) ==
                  std::vector<fsim::runtime::SystemVerilogClassHandle>{
                      api_component_child, api_component_top,
                      api_component_isolated}));
    }
    for (const auto component :
      ...
      assert(
          simulation
                  .read_class_property(component, test_uvm_component_identity +
                                                      "::phase_callbacks")
                  .packed.low_word()
                  .aval == 32 &&
          simulation
    ...
    assert(std::ranges::any_of(
        simulation.uvm_activity().failures(), [](const auto &failure) {
          return failure.message.find("foreign activity callback failed") !=
                 std::string::npos;
        }));
    ...
    assert(duplicate_component_rejected &&
           simulation.class_heap().live_objects() == live_before_duplicate);
    const auto &uvm_registry = simulation.uvm_registry();
    const auto item_wrapper =
        uvm_registry.wrapper_by_specialization(uvm_item_specialization);
    const auto component_wrapper = uvm_registry.wrapper_by_specialization(
        test_uvm_component_specialization);
    const auto param_item_wrapper =
        uvm_registry.wrapper_by_specialization(uvm_param_item_specialization);
    const auto factory_item_wrapper =
        uvm_registry.wrapper_by_specialization(uvm_factory_item_specialization);
    assert(uvm_registry.size() == 4 && item_wrapper != 0 &&
           component_wrapper != 0 && param_item_wrapper != 0 &&
           factory_item_wrapper != 0 && item_wrapper != component_wrapper &&
           ...
           uvm_registry.wrapper_by_name("UvmComponent") == component_wrapper &&
    ...
    const auto registered_object =
        simulation.uvm_registry().create_object_by_type(item_wrapper,
                                                        "registered_object");
    const auto registered_component =
        simulation.uvm_registry().create_component_by_name(
            "UvmComponent", "registered_child", api_component_top,
            api_component_root);
    const auto registered_param_object =
        simulation.uvm_registry().create_object_by_name(
    ...
    assert((
        ...
        std::get<fsim::runtime::PackedLogic4>(
            uvm_resources.read(timeout_resource, "api-second.api_top.child"))
                .low_word()
                .aval == 0x2020 &&
        ...
        shared_resource_callbacks ==
            std::vector{
                ...
                fsim::runtime::SystemVerilogUvmResourceCallbackEvent::
                    PostRead}));
    ...
    assert(config_wake_order == std::vector<unsigned>({1, 2}) &&
           uvm_config_db.exists(config_child, "child", "limit",
                                config_type.identity) &&
           std::get<fsim::runtime::PackedLogic4>(
               *uvm_config_db.get(config_child, "child", "limit",
                                  config_type.identity))
    ...
    const auto command_config =
        uvm_config_db.get(config_root, "api_top.child", "command_mode",
                          fsim::runtime::kSystemVerilogUvmStringConfigType);
    assert(uvm_factory.debug_resolve_by_name("UvmFactoryItem").resolved ==
               param_item_wrapper &&
           command_config &&
    ...
    assert(routed_reports.size() == 3 && routed_reports[0].sequence == 1 &&
    ...
    const auto isolated_report_policy = uvm_reports.policy(
        api_component_isolated,
        fsim::runtime::SystemVerilogUvmReportSeverity::Warning, "APP_HANDLER");
    assert(child_report_policy.verbosity == 130 &&
    ...
    const auto expected_server_record =
        std::string{"UVM_WARNING(UVM_NONE) classes.sv(615) @ 17: "
                    "api_top.api_child@@api-server [APP_SERVER] server action "
                    "-UVM_WARNING\n"};
    assert((routed_reports.size() == 4 && routed_reports.back().sequence == 4 &&
            ...
            report_record_count == 1 && report_control_events.size() == 1 &&
    ...
    const auto application_instance_catcher = uvm_reports.add_catcher(
        api_component_child, "application-instance", [&](auto &context) {
          application_catcher_order.push_back("instance:" +
                                              context.message().id);
          assert(context.message().severity ==
                     fsim::runtime::SystemVerilogUvmReportSeverity::Info &&
    ...
    assert((!uvm_reports.report(caught_application_report) &&
            application_catcher_order ==
                std::vector<std::string>{"global:APP_CATCH",
                                         "instance:APP_CATCH"} &&
            routed_reports.size() == 4 && uvm_reports.routed_count() == 4 &&
    ...
    if (engine == fsim::app::SimulationEngine::interpreter) {
      ...
      const auto assert_fresh_context = [&](const auto &context) {
        assert(context.uvm_registry().size() == 4 &&
               ...
               context.uvm_phases().domains().size() == 2 &&
               context.uvm_phases()
                   .snapshot(
                       context.uvm_phases().standard_schedule()->common_domain)
                   .roots.empty() &&
               context.uvm_phases()
                   .snapshot(
                       context.uvm_phases().standard_schedule()->runtime_domain)
                   .roots.empty());
      };
      std::size_t peer_callback_count{};
      {
        ...
        peer_report.severity =
            fsim::runtime::SystemVerilogUvmReportSeverity::Warning;
        assert(peer.uvm_reports().report(peer_report));
        assert(peer.uvm_components().lookup_root(peer_root, "api_top") ==
                   peer_top &&
               peer.uvm_phases().contains(peer_phase_domain) &&
        ...
        assert(peer.uvm_factory().set_type_override_by_type(peer_item,
                                                            peer_factory_item));
        assert(peer.uvm_factory().debug_resolve_by_name("UvmItem").resolved ==
                   peer_factory_item &&
               simulation.uvm_factory()
                       .debug_resolve_by_name("UvmItem")
                       .resolved == param_item_wrapper);
        ...
        assert(
            peer_callback_count == 2 && shared_resource_callbacks.size() == 4 &&
            std::get<fsim::runtime::PackedLogic4>(
                peer.uvm_resources().read(peer_timeout, "peer.api_top.child"))
                    .low_word()
                    .aval == 0xcafe &&
            std::get<fsim::runtime::PackedLogic4>(
                *peer.uvm_config_db().get(config_child, "child", "limit",
                                          config_type.identity))
                    .low_word()
                    .aval == 0xbeef &&
            std::get<fsim::runtime::PackedLogic4>(
        ...
        assert(shared_resource_callbacks.size() == 6 &&
      ...
      assert(uvm_factory.debug_resolve_by_name("UvmItem").resolved ==
                 param_item_wrapper &&
             std::get<fsim::runtime::PackedLogic4>(
                 *uvm_config_db.get(config_child, "child", "limit",
                                    config_type.identity))
    ...
    assert(simulation.uvm_objects().name(uvm_clone) == "api_root" &&
    ...
    assert(simulation.class_heap()
               ...
               .kind == fsim::runtime::SystemVerilogClassRandomKind::Rand);
    ...
    simulation.set_class_property_change_hook(
        [&](const auto changed_handle, const std::string_view property,
          ...
          if (property.ends_with("::generated_value")) {
            const auto &random =
                simulation.class_heap().random_state(changed_handle, property);
    ...
    auto wide_service_value =
        fsim::runtime::PackedLogic4{137, fsim::runtime::Logic4::zero};
    ...
    const auto safe_point =
        simulation.add_safe_point_hook([&](const auto &, const auto) {
          ++safe_points;
    ...
    if (engine == fsim::app::SimulationEngine::debug) {
      ...
      assert(early_debug_output.str().find("source_object = ") !=
             std::string::npos);
      assert(early_debug_output.str().find(" declared " + derived_identity) !=
             std::string::npos);
      simulation.clear_stop();
    }
    const auto result = simulation.run();
    assert(source_uvm_prints == 1 && source_uvm_records == 1);
    const auto source_uvm_ready =
        simulation.find_signal("class_top.source_uvm_constructed");
    const auto source_uvm_object =
        simulation.find_signal("class_top.source_uvm_object");
    const auto source_uvm_clone =
        simulation.find_signal("class_top.source_uvm_clone");
    const auto source_uvm_compared =
        simulation.find_signal("class_top.source_uvm_clone_compared");
    const auto source_uvm_copy =
        simulation.find_signal("class_top.source_uvm_copy");
    const auto source_uvm_copy_compared =
        simulation.find_signal("class_top.source_uvm_copy_compared");
    const auto source_uvm_top =
        simulation.find_signal("class_top.source_uvm_top");
    const auto source_uvm_child =
        simulation.find_signal("class_top.source_uvm_child");
    const auto source_uvm_parent_ok =
        simulation.find_signal("class_top.source_uvm_parent_ok");
    const auto source_uvm_child_count =
        simulation.find_signal("class_top.source_uvm_child_count");
    const auto source_uvm_component_clone_null =
        simulation.find_signal("class_top.source_uvm_component_clone_null");
    const auto source_uvm_item_type =
        simulation.find_signal("class_top.source_uvm_item_type");
    const auto source_uvm_component_type =
        simulation.find_signal("class_top.source_uvm_component_type");
    const auto source_uvm_item_wrapper_equal =
        simulation.find_signal("class_top.source_uvm_item_wrapper_equal");
    const auto source_uvm_component_wrapper_equal =
        simulation.find_signal("class_top.source_uvm_component_wrapper_equal");
    const auto source_uvm_wrapper_kinds_distinct =
        simulation.find_signal("class_top.source_uvm_wrapper_kinds_distinct");
    const auto source_uvm_param_type =
        simulation.find_signal("class_top.source_uvm_param_type");
    const auto source_uvm_param_wrapper_equal =
        simulation.find_signal("class_top.source_uvm_param_wrapper_equal");
    const auto source_uvm_factory_selected_override = simulation.find_signal(
        "class_top.source_uvm_factory_selected_override");
    const auto source_uvm_factory_created_override =
        simulation.find_signal("class_top.source_uvm_factory_created_override");
    assert(source_uvm_ready && source_uvm_object && source_uvm_clone &&
           source_uvm_compared && source_uvm_copy && source_uvm_copy_compared &&
           source_uvm_top && source_uvm_child && source_uvm_parent_ok &&
           source_uvm_child_count && source_uvm_component_clone_null &&
           source_uvm_item_type && source_uvm_component_type &&
           source_uvm_item_wrapper_equal &&
           source_uvm_component_wrapper_equal &&
           source_uvm_wrapper_kinds_distinct && source_uvm_param_type &&
           source_uvm_param_wrapper_equal &&
           source_uvm_factory_selected_override &&
           source_uvm_factory_created_override);
    const auto source_uvm_handle =
        simulation.read_signal(*source_uvm_object).low_word().aval;
    ...
    assert(simulation.read_signal(*source_uvm_copy_compared).low_word().aval ==
           1);
    const auto source_uvm_top_handle =
        simulation.read_signal(*source_uvm_top).low_word().aval;
    const auto source_uvm_child_handle =
        simulation.read_signal(*source_uvm_child).low_word().aval;
    assert(
        ...
        simulation.uvm_components().lookup(
            source_uvm_top_handle, "source_child") == source_uvm_child_handle);
    ...
    assert(simulation
               ...
               .packed.low_word()
               .aval == 0x1595);
    const auto source_uvm_clone_handle =
        simulation.read_signal(*source_uvm_clone).low_word().aval;
    assert(source_uvm_clone_handle != source_uvm_handle &&
           simulation.uvm_objects().type_name(source_uvm_clone_handle) ==
               "UvmItem" &&
           simulation
                   .read_class_property(source_uvm_clone_handle,
                                        uvm_item_identity + "::value")
                   .packed.low_word()
                   .aval == 0x1595);
    const auto source_uvm_copy_handle =
        simulation.read_signal(*source_uvm_copy).low_word().aval;
    assert(source_uvm_copy_handle != source_uvm_handle &&
           source_uvm_copy_handle != source_uvm_clone_handle &&
           simulation
                   .read_class_property(source_uvm_copy_handle,
                                        uvm_item_identity + "::value")
                   .packed.low_word()
                   .aval == 0x1595);
    ...
    const auto source_object =
        simulation.find_signal("class_top.source_object");
    assert(source_object);
    const auto source_handle =
        simulation.read_signal(*source_object).low_word().aval;
    ...
    assert(std::ranges::any_of(static_changes, [&](const auto &change) {
      return std::get<0>(change) == base->specialization_identity &&
             ...
             std::get<2>(change) ==
                 fsim::runtime::PackedLogic4::from_aval_bval(64, 10, 0)
                     .to_msb_string();
    }));
    ...
    const auto selected_randomized =
        simulation.find_signal("class_top.source_selected_randomize_generated");
    const auto fully_randomized =
        simulation.find_signal("class_top.source_randomize_generated");
    const auto third_randomized =
        simulation.find_signal("class_top.source_randc_third");
    ...
    const auto randomized_object_signal =
        simulation.find_signal("class_top.randomized_object");
    assert(randomized_object_signal);
    const auto randomized_handle =
        simulation.read_signal(*randomized_object_signal).low_word().aval;
    assert(simulation.class_heap()
    ...
    const auto generated_trace =
        std::ranges::find_if(randomization_trace, [&](const auto &state) {
    ...
    assert(generated_trace != randomization_trace.end() &&
           generated_trace->enabled && generated_trace->revision == 3 &&
           generated_trace->stream_seed != 0 &&
           generated_trace->domain_signature != 0 &&
           generated_trace->cycle == 0 && generated_trace->used_values == 3);
    assert(std::ranges::any_of(randomization_trace, [&](const auto &state) {
      return state.object == randomized_handle &&
             state.kind == fsim::app::ClassRandomizationTraceKind::constraint &&
             state.enabled &&
    ...
    if (engine == fsim::app::SimulationEngine::debug) {
      ...
      assert(random_debug_output.str().find("randc enabled 1 revision 3") !=
                 std::string::npos &&
             random_debug_output.str().find("cycle 0 used 3") !=
                 std::string::npos &&
             random_debug_output.str().find("constraint " + derived_identity +
    ...
    for (const auto name :
      ...
      const auto failed_handle =
          simulation.read_signal(*failed_object).low_word().aval;
      assert(simulation.class_heap()
    ...
    assert(
        simulation.read_class_property(source_handle, base_identity + "::peer")
            .handle == simulation.read_signal(*source_other).low_word().aval);
    ...
    assert(std::ranges::any_of(trace_values, [&](const auto &trace) {
      return !trace.object &&
             trace.path.ends_with(base->specialization_identity + ".shared") &&
    ...
    assert(debug_output.str().find(suspended_method_identity) !=
           std::string::npos);
    ...
    assert(foreign_host.remove_callback(foreign_host.context,
               ...
               FSIM_UVM_FOREIGN_OK &&
           !foreign_activity.kinds.empty() &&
           foreign_host.release(foreign_host.context, &foreign_snapshot) ==
               FSIM_UVM_FOREIGN_OK &&
           foreign_host.release(foreign_host.context, &foreign_snapshot) ==
               FSIM_UVM_FOREIGN_STALE_HANDLE &&
           simulation.uvm_foreign().diagnostic_code() ==
               "FSIM-UVM-FOREIGN-001");
  ...
  assert(portable_uvm_snapshots.size() == 3 &&
         std::ranges::all_of(portable_uvm_snapshots | std::views::drop(1),
                             [&](const auto &snapshot) {
                               return snapshot ==
                                      portable_uvm_snapshots.front();
                             }));
  ...
  const auto edited_delay =
      edited_source.find("#1 $finish; // class-cache-delay");
  ...
  auto standalone =
      fsim::app::load_design_artifact(relocated_design, standalone_diagnostics);
  assert(standalone);
  ...
  assert(std::ranges::any_of(
      standalone->design.processes(), [](const auto &process) {
        const auto has_class_call =
            std::ranges::any_of(process.operations, [](const auto &op) {
              return fsim::runtime::simir::operation_holds<
                  fsim::runtime::simir::ClassMethodCall>(op);
            });
        const auto has_continuation =
            std::ranges::any_of(process.operations, [](const auto &op) {
              return fsim::runtime::simir::operation_holds<
  ...
  assert(standalone_derived !=
         standalone_simulation.class_specializations().end());
  const auto standalone_transfer =
      std::ranges::find(standalone_derived->methods, std::string{"transfer"},
                        &fsim::frontend::SystemVerilogClassMethodProfile::name);
  ...
  assert(standalone_simulation.read_class_property(standalone_handle, "value")
             .packed.to_msb_string() == "XXXXXXXX");
  const auto standalone_base = std::ranges::find_if(
      standalone_simulation.class_specializations(),
      [](const auto &specialization) {
        return specialization.declaration_identity.ends_with("::AppBase");
      });
  assert(standalone_base !=
         standalone_simulation.class_specializations().end());
  ...
  assert(standalone_result.status == fsim::runtime::RunStatus::stopped &&
         standalone_result.time == 9);
  const auto standalone_property =
      standalone_simulation.find_signal("class_top.source_property");
  assert(
      standalone_property &&
      standalone_simulation.read_signal(*standalone_property).low_word().aval ==
          18);
  assert(standalone_simulation.class_static_store()
             .property(standalone_base->declaration_identity, "shared")
             .packed.low_word()
  ...
  assert(mapped_result.status == fsim::runtime::RunStatus::stopped &&
         mapped_result.time == 9);
  const auto mapped_property =
      mapped_simulation.find_signal("class_top.source_property");
  assert(mapped_property &&
  ...
  for (const auto engine : {fsim::app::SimulationEngine::interpreter,
    ...
    auto multiple_project =
        fsim::app::build_project(multiple, multiple_diagnostics);
    if (!multiple_project) {
