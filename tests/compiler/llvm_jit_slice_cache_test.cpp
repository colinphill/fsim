...
namespace {
        ...
        const std::uint32_t source_line)
    {
        ...
        if (consumer == SliceConsumer::size) {
            ...
            const auto constant = graph_constant.value_or(std::uint8_t { 0 });
        ...
        const std::uint32_t source_line)
    {
        ...
        Process process;
        process.id = 43;
        process.name = "cached_static_slice_port." + std::string { specialization };
        ...
        if (mode == SlicePortMode::output
            || mode == SlicePortMode::inout) {
            process.operations.push_back(
                WriteContainerObject { object, 0, std::nullopt });
        }
        process.operations.push_back(Halt { });
        return process;
    }
    ...
    {
        assert(width > 0);
        const auto distance = static_cast<std::int64_t>(width) - 1;
        const auto lower = plus
            ? static_cast<std::int64_t>(base)
            : static_cast<std::int64_t>(base) - distance;
        const auto upper = plus
            ? static_cast<std::int64_t>(base) + distance
            : static_cast<std::int64_t>(base);
        ...
        selected.fixed = true;
        selected.index_left = static_cast<std::int32_t>(
            declared_descending ? upper : lower);
        selected.index_right = static_cast<std::int32_t>(
        ...
        const std::uint32_t function_line)
    {
        ...
        const auto count = result_left >= result_right
            ? result_left - result_right
            : result_right - result_left;
        source_type.index_right = source_left
...
{
    constexpr std::string_view symbol = "cached_static_slice_consumer";
...
void test_static_slice_call_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_static_slice_call";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
    ...
    const auto make =
        [&](const std::int32_t actual_left = 3,
            ...
            const SliceCallMode mode = SliceCallMode::function_input,
...
void test_static_slice_ordering_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_static_slice_ordering";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
    ...
    const auto make =
        [&](const std::int32_t left = 4,
            const std::uint32_t width = 8,
            const ContainerOrderingOperator ordering = ContainerOrderingOperator::reverse,
...
void test_static_slice_port_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_static_slice_port";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
...
void test_static_indexed_slice_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_static_indexed_slice";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
...
void test_fixed_array_function_return_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_fixed_array_function_return";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
    ...
    const auto make =
        [&](const std::int32_t result_left = 3,
            ...
            const std::string_view source = "fixed-array-function-return.sv",
...
void test_nonstatic_function_return_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_nonstatic_function_return";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
    ...
    const auto make =
        [&](const ContainerType& type,
            ...
            const std::string_view source = "nonstatic-function-return.sv",
            const std::uint32_t line = 18) {
...
void test_expression_selection_cache_identity(
    const std::filesystem::path& cache_directory)
{
    constexpr std::string_view symbol = "cached_expression_selection";
    const std::array<std::uint32_t, 0> no_signals { };
    const auto options = LlvmJitOptions {
        JitOptimizationLevel::o2, cache_directory
    };
    const auto make =
        [](const std::uint32_t selection_width = 4,
            ...
            const bool two_state = false,
            const std::string_view source_path = "expression-selection.sv",
            ...
            const ExpressionSizingKind sizing = ExpressionSizingKind::context_determined,
            const ExpressionValueDomain domain = ExpressionValueDomain::four_state,
