...
fsim::runtime::SystemVerilogConstraintVariable variable(
  ...
  for (const auto item : domain) result.domain.push_back(value(width, item));
  return result;
}
...
void test_systemverilog_constraint_solver() {
  ...
  SystemVerilogConstraintSolver elapsed_limited({
      10, 10, 10, 10, std::chrono::steady_clock::duration::zero()});
  ...
  const auto corrupted_variable = corrupted_solver.add_variable(variable(
      ...
      1, "bit", {0}));
  corrupted_solver.add_clause({
      "corrupted-clause", {corrupted_variable},
  ...
  const auto soft_value = soft_solver.add_variable(variable(
      ...
      1, "bit", {0, 1}));
  soft_solver.add_clause({
      "earlier-soft", {soft_value}, equals(soft_value, 0), true});
  soft_solver.add_clause({
      "later-soft", {soft_value}, equals(soft_value, 1), true});
  ...
  const auto distributed = distribution_solver.add_variable(variable(
      ...
      2, "logic[1:0]", {0, 1, 2, 3}));
  distribution_solver.add_distribution({
      "weighted-domain",
      distributed,
      {
  ...
  const auto normalized =
      distribution_solver.distribution_weights("weighted-domain");
  ...
  const auto conflict_value = conflicting_distributions.add_variable(variable(
      ...
      1, "bit", {0, 1}));
  conflicting_distributions.add_distribution({
      "only-zero", conflict_value,
      {{value(1, 0), value(1, 0), 1}}});
  conflicting_distributions.add_distribution({
      "only-one", conflict_value,
  ...
  try {
    overflowing_distribution.add_distribution({
        "overflowing", overflow_value,
        {
            {value(1, 0), value(1, 0),
  ...
  try {
    ...
    solve_cycle = std::string_view{error.what()}.find(
        "cycle reaches 'alpha'") != std::string_view::npos;
  }
  require(
  ...
  domain_limited_request.call_identity =
      "work::Packet::randomize@domain-limit";
  ...
  search_limited_request.call_identity =
      "work::Packet::randomize@search-limit";
  ...
  require(
      cycle_heap.random_state(cycle_handle, "value").randc_cycle == 1
          && cycle_heap.random_state(
                 cycle_handle, "value").randc_used_values.size() == 4
  ...
  const auto constrained_cycle_handle =
      constrained_cycle_heap.allocate(randc_descriptor);
  ...
  constrained_cycle_request.inline_constraints = [](
    ...
    configured.add_clause({
        "cycle-domain", {variable},
        [=](const SystemVerilogConstraintAssignment& assignment) {
          if (!assignment.assigned(variable)) {
            return SystemVerilogConstraintClauseState::Undetermined;
          }
  ...
  for (std::size_t call = 0; call < 4; ++call) {
    ...
    constrained_values.push_back(constrained_cycle_heap.property(
        constrained_cycle_handle, "value").packed.low_word().aval);
  }
  require(
  ...
  require(
      randomize_systemverilog_class_object(
          domain_cycle_heap, domain_cycle_handle, first_domain).language_result()
          == 1,
      "the first exact randc domain must solve");
  const auto first_signature = domain_cycle_heap.random_state(
      domain_cycle_handle, "value").randc_domain_signature;
  auto changed_domain = cycle_request;
  changed_domain.property_domains["value"] = {value(2, 2), value(2, 3)};
  require(
      randomize_systemverilog_class_object(
          domain_cycle_heap, domain_cycle_handle, changed_domain).language_result()
              == 1
          && domain_cycle_heap.property(
                 domain_cycle_handle, "value").packed.low_word().aval >= 2
          && domain_cycle_heap.random_state(
                 domain_cycle_handle, "value").randc_domain_signature
              != first_signature
          && domain_cycle_heap.random_state(
                 domain_cycle_handle, "value").randc_cycle == 0
          && domain_cycle_heap.random_state(
                 domain_cycle_handle, "value").randc_used_values.size() == 1,
  ...
  require(
      randomize_systemverilog_class_object(
          cycle_heap, cycle_handle, cycle_request).language_result() == 1
          && cycle_heap.property(cycle_handle, "value").packed
              == reseeded_value,
  ...
  require(
      randomize_systemverilog_class_object(
          cycle_heap, cycle_handle, failed_cycle_request).language_result() == 0
          && cycle_heap.random_state(
                 cycle_handle, "value").randc_domain_signature
          ...
          && cycle_heap.random_state(
                 cycle_handle, "value").randc_used_values
  ...
  const auto exact_cycle_object_bytes =
      storage_probe.object(storage_probe_handle).accounted_bytes;
  SystemVerilogClassHeap cycle_storage_limited{
      {1, exact_cycle_object_bytes}, 271};
  const auto cycle_storage_handle =
      cycle_storage_limited.allocate(randc_descriptor);
  const auto storage_before = cycle_storage_limited.property(
      cycle_storage_handle, "value").packed;
  ...
  require(
      cycle_storage_failed
          && cycle_storage_limited.property(
                 cycle_storage_handle, "value").packed == storage_before
          && cycle_storage_limited.random_state(
                 cycle_storage_handle, "value").revision == 0
          && cycle_storage_limited.random_state(
                 cycle_storage_handle, "value").randc_used_values.empty()
  ...
  scope_request.inline_constraints = {};
