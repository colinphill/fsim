...
void test_simir_wide_extract_and_concatenate()
{
    ...
    const auto source = "1" + std::string(62, '0') + "XZ";
...
void test_simir_dynamic_packed_indices()
{
    ...
    const auto waveform = interpreter.add_signal(
        { "top.dynamic.waveform",
            ...
            ResolutionKind::none,
            ValueKind::logic9 });
    const auto wide_inertial = interpreter.add_signal(
        { "top.dynamic.wide_inertial", PackedLogic4(257, Logic4::zero) });
    const auto wide_source = "1" + std::string(127, '0') + "X";

    Process process;
    process.id = 0;
    process.name = "dynamic_packed_indices";
    process.register_count = 17;
    ...
    process.operations = {
        ...
        WriteProjectedWaveformDynamicSlice {
            ...
            ProjectedDelayMode::transport },
        LoadConstant {
            15, PackedLogic4::from_msb_string(wide_source) },
        LoadConstant { 16, integer(64) },
        WriteInertialDynamicPartSlice {
            wide_inertial,
            15,
            DynamicPartIndex { 16, 256, 0, 0, 129, true, true },
            TransitionDelays { 3, 3, 3 } },
        Halt { },
    };
    (void)interpreter.add_process(std::move(process));
    ...
    require(
        interpreter.signal_value(wide_inertial).to_msb_string()
            == std::string(64, '0') + wide_source
                + std::string(64, '0'),
...
void test_simir_alternate_executor_context_and_boundaries()
{
    ...
    auto executor = std::make_unique<SignalExecutor>(
    ...
    const auto alternate_stop_process = add_wait_yield_stop(alternate_stop);
...
void test_simir_alternate_executor_zero_delay_and_frame()
{
    ...
    using Change = std::pair<SignalId, std::pair<SchedulerPhase, std::string>>;
    ...
    private:
        ...
        PackedLogic4 frame_register_ = PackedLogic4::from_msb_string("X");
    };
...
void test_simir_alternate_executor_cpp_exception_containment()
{
    ...
    public:
        explicit ContainingExecutor(const SignalId signal)
            : signal_(signal)
        {
        }
