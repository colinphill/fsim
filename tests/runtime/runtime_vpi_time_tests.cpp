...
void test_systemverilog_vpi_time_service() {
  ...
  require_vpi_time(
      service.valid() && !invalid_precision.valid()
          ...
          && invalid_precision.query(
                 SystemVerilogVpiTimeFormat::IntegerTicks).error
              == SystemVerilogVpiTimeError::InvalidProfile,
      "VPI time profiles require precision at or below units within femtoseconds");

  const auto initial_integer =
      service.query(SystemVerilogVpiTimeFormat::IntegerTicks);
  const auto initial_real =
      service.query(SystemVerilogVpiTimeFormat::ScaledReal);
  require_vpi_time(
  ...
  const auto integer =
      service.convert_delay(tick_delay(0x100000002ULL));
  ...
  invalid_format.format =
      static_cast<SystemVerilogVpiTimeFormat>(99);
  require_vpi_time(
      ...
      [&](Scheduler& running) {
        running.schedule_next_delta(
            ...
            [&](Scheduler&) {
              next_delta =
                  service.query(
                      SystemVerilogVpiTimeFormat::IntegerTicks).delta;
            });
      });
  (void)scheduler.run();
  require_vpi_time(
  ...
  require_vpi_time(
      service.schedule(
          tick_delay(1),
          16,
          {}).error == SystemVerilogVpiTimeError::InvalidHandle
