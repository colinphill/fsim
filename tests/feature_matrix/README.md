...
| [app/expression_application_test.cpp](../app/expression_application_test.cpp) | Fast source-to-runtime interpreter/compiled differential for expression checkpoints, currently covering SystemVerilog wildcard equality/inequality and logical-equality unknown dominance plus VHDL `sla`/`rol`/`ror`, negative-count reversal, oversized arithmetic fill, modulo-width rotation, and LLVM O0/O2 execution |
