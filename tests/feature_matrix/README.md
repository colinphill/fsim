...
| [app/display_application_test.cpp](../app/display_application_test.cpp) | Verilog/SystemVerilog literal, empty, and constant-unsigned-numeric `$display`/`$write`/`$strobe` ordering, literal-only `$monitor` initial publication, and VHDL literal note reports; decoded language escapes, newline/no-newline and immediate/postponed policy, process/time/delta output metadata, CLI stream routing, and interpreter versus LLVM O0/O2 equivalence |
