...
| [feature_matrix/systemverilog_2023_inventory.tsv](systemverilog_2023_inventory.tsv) | Independently worded 56-row IEEE 1800-2023 delta inventory assigning frontend, verification, hierarchy, timing, foreign API, annex, and closure obligations one-to-one to Batch 185 Changes 2-19 and Batch 186-187 Changes 1-19; through Batch 187 Change 19, all 56 rows are preserved with zero active obligations, including frontend, runtime, verification, foreign API, artifact, engine, and platform ownership without private-reference text |
...
| [app/random_application_test.cpp](../app/random_application_test.cpp) | Default, numeric, entropy-selected, and native-session seeds; stable independent per-process streams; bare/parenthesized `$random` and `$urandom`; one-/two-bound and reversed `$urandom_range`; all seven seeded `$dist_*` families using the low 23 updated-seed bits; exact results and seed updates; equal/reversed-bound, X/Z, invalid-domain, negative-deviation, zero-mean Erlang, and bounded-draw behavior; source-located warning transactions; signed formatting; and interpreter versus LLVM O0/O2 equivalence |
