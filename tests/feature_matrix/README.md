...
|---|---|
| [frontend/frontend_tests.cpp](../frontend/frontend_tests.cpp) | Minimal VHDL entity/architecture/process syntax including bounded process variables and timed/any-change waits, SystemVerilog module/procedural-variable/event-control/wildcard/always_comb/always_latch syntax, non-ANSI Verilog ports, bounded `` `timescale`` context/scaling, and targeted directive/duplicate/initializer/type/wait/implicit-process diagnostics |
| [elaboration/elaborator_test.cpp](../elaboration/elaborator_test.cpp) | Independently selected VHDL/SystemVerilog counters, executable SV-parent and VHDL-parent mixed hierarchies, persistent named VHDL/SV local-register lowering and reads, source-level timed/dynamic wait lowering and wakeups, deterministic static/dynamic wildcard sensitivity inference and latch retention, dense instance-specific specialization ownership with source/language/library metadata, cross-library VHDL selection, and targeted unsupported-semantics/driver diagnostics |
