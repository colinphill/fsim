...
| [app/vhdl_enumeration_application_test.cpp](../app/vhdl_enumeration_application_test.cpp) | VHDL package enumeration types and subtypes, identifier/character literals, typed constants/generics, all declaration-range scalar attributes through imported/subtype type marks, static package/generic folding, checked dynamic successor failure, same-nominal hierarchy aliases, assignments, case choices, equality/ordinal comparison, debugger literal names, interpreter versus LLVM O0/O2 equivalence, normalized VCD, cold/warm cache reuse, and package-edit invalidation |
