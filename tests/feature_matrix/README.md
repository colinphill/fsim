...
| [app/display_application_test.cpp](../app/display_application_test.cpp) | Verilog/SystemVerilog literal and empty `$display`/`$write`/`$strobe` ordering, newline/no-newline and immediate/postponed policy, process/time/delta output metadata, CLI stream routing, and interpreter versus LLVM O0/O2 equivalence |
