...
| [app/display_application_test.cpp](../app/display_application_test.cpp) | Verilog/SystemVerilog literal and empty `$display`/`$write` ordering, newline/no-newline policy, process/time/delta output metadata, CLI stream routing, and interpreter versus LLVM O0/O2 equivalence |
