...
namespace {
    ...
    make_test_directory(std::string_view name)
    {
        ...
        const auto directory = std::filesystem::temp_directory_path() / ("fsim-" + std::string { name } + "-" + std::to_string(suffix));
...
`endif
    ...
    require(parsed.design.units.size() == 1 && parsed.design.units.front().name == "preprocessed",
    ...
    require(unit.ports.size() == 2 && unit.ports.front().type.width() == std::optional<std::uint64_t> { 4 },
        "object macro expands into a packed range");
    require(unit.processes.size() == 1 && unit.processes.front().statements.size() == 6 && unit.processes.front().statements[0].value.text == "4'b1010" && unit.processes.front().statements[1].value.text == "4'b0101",
    ...
    require(unit.processes.front().statements[3].value.kind == ExpressionKind::IntegerLiteral,
        "`__LINE__ expands to an integer literal");
    require(unit.processes.front().statements[4].else_statements.size() == 1 && unit.processes.front().statements[4].else_statements.front().output_text == "preprocessed",
...
`endif
    ...
    require(shared_parsed.ok() && shared_parsed.design.units.size() == 1 && shared_parsed.design.units.front().name == "shared_compilation_unit" && shared_parsed.design.units.front().time_unit == "10ns" && shared_parsed.design.units.front().processes.front().statements.front().value.text == "4'b1100",
...
module bad;
  ...
  initial begin
    ...
    auto bad_preprocessed = preprocess_verilog_file(bad_root, Language::SystemVerilog2017);
    ...
    require(std::any_of(bad.diagnostics.begin(), bad.diagnostics.end(),
                [](const Diagnostic& diagnostic) {
                    return !diagnostic.expansion_stack.empty() && diagnostic.expansion_stack.front().find("macro `BAD_ASSIGN'") != std::string::npos;
                }),
    ...
    const auto include_error = parse_verilog(std::move(include_error_preprocessed.lexed), true);
    require(!include_error.ok() && std::any_of(include_error.diagnostics.begin(), include_error.diagnostics.end(), [](const Diagnostic& diagnostic) {
        return !diagnostic.expansion_stack.empty() && diagnostic.expansion_stack.front().find("included '") != std::string::npos;
    }),
    ...
    require(!missing.ok() && std::any_of(missing.lexed.diagnostics.begin(), missing.lexed.diagnostics.end(), [](const Diagnostic& diagnostic) {
...
`endif
    ...
    const auto legacy_include = directory / "legacy-width.vh";
    const auto legacy_root = directory / "legacy-root.v";
    const auto wide_literal = std::string("257'h1") + std::string(64, '0');
    write_text(
        legacy_include,
        "wire [256:0] \\escaped.name ;\ninitial \\escaped.name = "
            + wide_literal + ";\n");
    write_text(
        legacy_root,
        "module legacy;\n`include \"legacy-width.vh\"\nendmodule\n");
    PreprocessorOptions legacy_options;
    legacy_options.include_directories = { directory };
    legacy_options.standard_revision = StandardRevision::Verilog1995;
    auto legacy = preprocess_verilog_file(
        legacy_root, Language::Verilog2005, legacy_options);
    const auto wide_token = std::ranges::find(
        legacy.lexed.tokens, wide_literal, &Token::text);
    const auto escaped_token = std::ranges::find(
        legacy.lexed.tokens, "\\escaped.name", &Token::text);
    require(
        legacy.ok() && wide_token != legacy.lexed.tokens.end()
            && escaped_token != legacy.lexed.tokens.end()
            && legacy.dependencies.size() == 2
            && legacy.dependencies.front().standard_revision
                == StandardRevision::Verilog1995
            && legacy.dependencies.back().standard_revision
                == StandardRevision::Verilog1995,
        "Verilog-1995 preserves exact wide literals, escaped identifiers, and include revision provenance");

    const auto keyword_2005 = parse_verilog(
        SourceText { "keyword-2005.sv", "module checker; endmodule\n" },
        StandardRevision::SystemVerilog2005);
    const auto keyword_2009 = parse_verilog(
        SourceText { "keyword-2009.sv", "module checker; endmodule\n" },
        StandardRevision::SystemVerilog2009);
    const auto keyword_region = parse_verilog(
        SourceText {
            "keyword-region.sv",
            "`begin_keywords \"1800-2005\"\nmodule checker; endmodule\n`end_keywords\n" },
        StandardRevision::SystemVerilog2017);
    const auto future_keyword_region = parse_verilog(
        SourceText {
...
`SELECTED_MODULE
    ...
    const auto later_preprocessing = preprocess_verilog(
        SourceText {
            ...
            R"(`define DEFAULT(value=1) value
`define PASTE(left,right) left``right
`__FILE__
`undefineall
module later_literals;
  initial #12ns value = '1;
  initial $display("\x41");
endmodule
)" },
        Language::Verilog2005,
        verilog_2005_options);
    const auto later_diagnostic = [&](const std::size_t line) {
        return std::ranges::any_of(
            later_preprocessing.lexed.diagnostics,
            [&](const Diagnostic& diagnostic) {
                return diagnostic.code == "FSIM-SV-PP-052"
    ...
    require(
        !later_preprocessing.ok() && later_diagnostic(1)
            && later_diagnostic(2) && later_diagnostic(3)
            && later_diagnostic(4) && later_diagnostic(6)
            && later_diagnostic(7),
        "later-only macro, directive, numeric, time, and string forms receive exact-line standard diagnostics");

    const auto verilog_timescale = preprocess_verilog(
        SourceText {
            "verilog-timescale.v",
            "`timescale 1ns/1ps\nmodule timed; initial #1 value = 1'b1; endmodule\n" },
        Language::Verilog2005,
        verilog_2005_options);
    require(
        verilog_timescale.ok(),
        "Verilog-2005 timescale directive units are not SystemVerilog time literals");

    PreprocessorOptions compatibility_options;
    compatibility_options.standard_revision
        = StandardRevision::SystemVerilog2009;
    auto compatibility_preprocessed = preprocess_verilog(
        SourceText { "compatibility-preprocessing.sv",
            "module checker; logic [256:0] value; endmodule\n" },
        Language::SystemVerilog2017,
        compatibility_options);
    auto compatibility_parsed = parse_verilog(
        std::move(compatibility_preprocessed.lexed),
        StandardRevision::SystemVerilog2009,
        "keyword-profile");
    require(
        compatibility_parsed.ok()
            && compatibility_parsed.design.units.front().name == "checker"
            && compatibility_parsed.design.units.front()
                    .verilog_compatibility_profile
                == "keyword-profile"
            && compatibility_parsed.design.units.front().signals.front().type.width()
                == std::optional<std::uint64_t> { 257 },
        "preprocessed tokens retain an explicit compatibility profile without width loss");
...
`BAD_STRING(value)
...
#if defined(FSIM_TEST_UVM_1_2_SOURCE_DIR) && defined(FSIM_TEST_UVM_2020_3_1_SOURCE_DIR)
    ...
    for (std::size_t release = 0; release < upstream_roots.size(); ++release) {
        const auto upstream_probe = directory / ("upstream-" + std::to_string(release) + ".sv");
...
function void fsim_uvm_legacy_macro_calls;
  ...
  `uvm_fatal("LEGACY_FATAL", "message")
        ...
        const auto identifier_count = [&](const std::string_view spelling) {
            return std::ranges::count_if(
                upstream.lexed.tokens, [&](const Token& token) {
                    return token.kind == TokenKind::Identifier && token.text == spelling;
                });
        };
        ...
        const auto field_helper = release == 0 ? std::string_view { "__m_uvm_field_automation" }
                                               : std::string_view { "__m_uvm_execute_field_op" };
        const auto required_present = std::ranges::all_of(required_identifiers, [&](const auto spelling) {
            return identifier_count(spelling) > 0;
        });
        const auto macro_free = std::ranges::none_of(upstream.lexed.tokens, [](const Token& token) {
            return token.text.find('`') != std::string::npos;
        });
        std::string observed {
            " ok=" + std::to_string(upstream.ok()) + " required=" + std::to_string(required_present) + " macro_free=" + std::to_string(macro_free) + " type_id=" + std::to_string(identifier_count("type_id")) + " get_type=" + std::to_string(identifier_count("get_type")) + " get_object_type=" + std::to_string(identifier_count("get_object_type")) + " create_item=" + std::to_string(identifier_count("create_item")) + " start_item=" + std::to_string(identifier_count("start_item")) + " finish_item=" + std::to_string(identifier_count("finish_item"))
        };
        ...
        require(
            upstream.ok() && required_present && identifier_count(field_helper) > 0 && identifier_count("type_id") >= 4 && identifier_count("get_type") >= 4 && identifier_count("get_object_type") >= 4 && identifier_count("create_item") >= 3 && identifier_count("start_item") >= 4 && identifier_count("finish_item") >= 4 && macro_free,
            ...
            "modification;"
                + observed);
        const auto upstream_package = preprocess_verilog_file(upstream_roots[release] / "uvm_pkg.sv",
        ...
        const auto version_probe = directory / ("upstream-version-" + std::to_string(release) + ".sv");
        if (release == 0) {
...
`endif
        ...
        require(
            version.ok() && (release == 0 ? version_has_token("1") && version_has_token("2") && version_has_token("\"UVM-1.2\"") && version_has_token("uvm_version_1_2_enabled") : version_has_token("2020") && version_has_token("3.0") && version_has_token("uvm_pkg") && version_has_token("UVM_VERSION_STRING") && version_has_token("uvm_version_post_2020_2_0_enabled") && version_has_token("uvm_post_version_1_2_enabled")),
...
endclass
            ...
            require(deprecated.ok() && std::ranges::all_of(deprecated_generated_identifiers, [&](const auto spelling) {
                return deprecated_identifier_count(
                           spelling)
                    > 0;
            }) && std::ranges::none_of(deprecated.lexed.tokens, [](const Token& token) {
                return token.text.find('`') != std::string::npos;
            }),
            ...
            require(!disabled_deprecated.ok() && std::ranges::any_of(disabled_deprecated.lexed.diagnostics, [](const Diagnostic& diagnostic) {
            ...
            require(!invalid_arity.ok() && std::ranges::any_of(invalid_arity.lexed.diagnostics, [](const Diagnostic& diagnostic) {
            ...
            require(!shallow.ok() && std::ranges::any_of(shallow.lexed.diagnostics, [](const Diagnostic& diagnostic) {
            ...
            const auto removed_macro_probe = directory / "upstream-removed-deprecated-1.sv";
            ...
            require(!removed_macros.ok() && std::ranges::count_if(removed_macros.lexed.diagnostics, [](const Diagnostic& diagnostic) {
                return diagnostic.code == "FSIM-SV-PP-028";
            }) == 2,
...
{
    const auto remapped = preprocess_verilog(SourceText { "physical.sv",
...
`__LINE__
    ...
    const auto token_named =
        ...
        const auto found = std::ranges::find(remapped.lexed.tokens, spelling, &Token::text);
    ...
    require(file_token != nullptr && file_token->span.source_name == R"(C:\rtl\logical"name.sv)" && file_token->span.begin.line == 201,
        "`__FILE__ is safely re-escaped from the logical source name");
    require(line_token != nullptr && line_token->span.source_name == R"(C:\rtl\logical"name.sv)" && line_token->span.begin.line == 202,
...
physical_token
        ...
        Language::SystemVerilog2017);
    const auto inactive_token = std::ranges::find(inactive.lexed.tokens, "physical_token", &Token::text);
    require(inactive.ok() && inactive_token != inactive.lexed.tokens.end() && inactive_token->span.source_name.find("inactive-line.sv") != std::string::npos && inactive_token->span.begin.line == 4,
...
parent_after
    ...
    const auto included = preprocess_verilog_file(parent, Language::SystemVerilog2017);
    const auto included_token =
        [&](const std::string_view spelling) -> const Token* {
        const auto found = std::ranges::find(included.lexed.tokens, spelling, &Token::text);
    ...
    require(included.ok() && before != nullptr && child_value != nullptr && after != nullptr && before->span.source_name == "logical-parent.sv" && before->span.begin.line == 50 && child_value->span.source_name == "logical-child.svh" && child_value->span.begin.line == 700 && after->span.source_name == "logical-parent.sv" && after->span.begin.line == 52,
    ...
    const auto second_token = std::ranges::find(roots.lexed.tokens, "second_root_token", &Token::text);
    require(roots.ok() && second_token != roots.lexed.tokens.end() && second_token->span.source_name == std::filesystem::weakly_canonical(second).generic_string() && second_token->span.begin.line == 1,
...
`line 1 "bad-level.sv" 3
    ...
    require(!malformed.ok() && has_code("FSIM-SV-PP-039") && has_code("FSIM-SV-PP-040") && has_code("FSIM-SV-PP-041") && has_code("FSIM-SV-PP-042") && has_code("FSIM-SV-PP-043"),
    ...
    require(!parsed_error.ok() && std::ranges::any_of(parsed_error.diagnostics, [](const Diagnostic& diagnostic) {
        return diagnostic.span.source_name == "parser-logical.sv" && diagnostic.span.begin.line == 900;
    }),
    ...
    const auto lexical_error = preprocess_verilog(
        ...
        Language::SystemVerilog2017);
    require(!lexical_error.ok() && std::ranges::any_of(lexical_error.lexed.diagnostics, [](const Diagnostic& diagnostic) {
        return diagnostic.span.source_name == "lexer-logical.sv" && diagnostic.span.begin.line == 300;
    }),
...
begin
  ...
  begin
    ...
    const auto& process = architecture->processes.front();
    require(process.sensitivities.size() == 1 && process.sensitivities.front().signal == "clk" && process.sensitivities.front().edge == EdgeKind::Negative,
...
begin
  ...
  qualified_actual: child
    ...
    require(architecture->instances[0].parameter_overrides.size() == 1 && architecture->instances[0].parameter_overrides[0].name == std::optional<std::string> { "width" } && architecture->instances[0].parameter_overrides[0].value.text == "2",
        "named generic maps must remain in the instance HIR");
    require(architecture->instances[1].connections.size() == 2 && architecture->instances[1].connections[0].value.kind == ExpressionKind::Call && architecture->instances[1].connections[1].kind == PortActualKind::Open,
        "indexed actual and explicit open state retained");
    require(architecture->instances[2].connections.size() == 2 && architecture->instances[2].connections[0].value.kind == ExpressionKind::Call && architecture->instances[2].connections[0].value.text == "@vhdl-qualified:std_logic_vector" && architecture->instances[2].connections[0].value.operands.size() == 1 && architecture->instances[2].connections[0].value.operands.front().text == "data",
...
begin
  child: entity work.invalid_port_leaf(rtl)
    ...
    require(!invalid.ok() && has_code("FSIM-VHDL-SEM-078") && has_code("FSIM-VHDL-SEM-079"),
...
begin
  ...
  open_default_child: entity work.generic_child(rtl)
    ...
    require(
        child->parameters[0].name == "element_t" && child->parameters[0].kind == ParameterKind::Type && child->parameters[0].default_value.kind == ExpressionKind::Invalid && child->parameters[1].name == "width" && child->parameters[1].object_class == InterfaceObjectClass::Constant && child->parameters[1].direction == PortDirection::Input && child->parameters[1].type.spelling == "positive" && child->parameters[1].default_value.text == "8" && child->parameters[2].name == "depth" && child->parameters[3].type.domain == ValueDomain::Boolean && child->parameters[3].default_value.kind == ExpressionKind::BooleanLiteral && child->parameters[4].default_value.kind == ExpressionKind::Invalid,
        "type formals, typed defaults, and required value generics");
    require(child->ports.size() == 1 && !child->ports[0].type.packed_range && child->ports[0].type.packed_range_expression && child->ports[0].type.packed_range_expression->descending == std::optional<bool> { true },
        "VHDL symbolic ranges retain explicit direction");
    const auto top_architecture = std::find_if(result.design.units.begin(), result.design.units.end(),
        [](const DesignUnit& unit) {
            return unit.kind == UnitKind::VhdlArchitecture && unit.primary_name == "generic_top";
        });
    require(top_architecture != result.design.units.end() && top_architecture->instances.size() == 5 && top_architecture->instances[0].parameter_overrides.size() == 3 && top_architecture->instances[0].parameter_overrides[0].name == std::optional<std::string> { "element_t" } && !top_architecture->instances[1].parameter_overrides[0].name && top_architecture->instances[1].parameter_overrides[2].name == std::optional<std::string> { "enabled" },
        "named and positional-then-named generic maps are represented");
    require(
        top_architecture->instances[4].parameter_overrides.size() == 4 && top_architecture->instances[4].parameter_overrides[1].default_box && top_architecture->instances[4].parameter_overrides[2].default_box,
        "open value-generic actuals retain default selection in HIR");
    const auto& ranged_actual = top_architecture->instances[2].parameter_overrides[0];
    require(ranged_actual.type_value && ranged_actual.type_value->spelling == "integer" && ranged_actual.type_value->integer_range_expression && !ranged_actual.type_value->integer_range_expression->descending && ranged_actual.type_value->integer_range_expression->span.source_name == "generics.vhd" && ranged_actual.value.kind == ExpressionKind::Invalid,
        "unambiguous VHDL range subtype actuals retain typed HIR");
    const auto& constrained_actual = top_architecture->instances[3].parameter_overrides[0];
    require(!constrained_actual.type_value && constrained_actual.value.kind == ExpressionKind::Slice && constrained_actual.value.text == "downto" && constrained_actual.value.operands.size() == 3 && constrained_actual.value.operands[0].text == "bit_vector",
...
begin
  bad: entity work.invalid_generic(rtl)
    ...
    require(!invalid.ok() && has_code("FSIM-VHDL-SEM-013") && has_code("FSIM-VHDL-SEM-014") && has_code("FSIM-VHDL-SEM-015") && has_code("FSIM-VHDL-SEM-016") && has_code("FSIM-VHDL-SEM-036") && has_code("FSIM-VHDL-SEM-076") && has_code("FSIM-VHDL-SEM-077") && has_code("FSIM-VHDL-UNSUPPORTED-018") && has_code("FSIM-VHDL-UNSUPPORTED-028"),
...
begin
  ...
  begin
    ...
    require(alias.name == "packet_t" && alias.type.packed_aggregate == PackedAggregateKind::Struct && alias.type.domain == ValueDomain::Logic9 && alias.type.width() == 18 && alias.type.packed_members.size() == 4,
        "record type retains flattened width, domain, and elements");
    require(alias.type.packed_members[0].name == "data" && alias.type.packed_members[0].lsb_offset == 10 && alias.type.packed_members[1].name == "shadow" && alias.type.packed_members[1].lsb_offset == 2 && alias.type.packed_members[2].name == "valid" && alias.type.packed_members[2].lsb_offset == 1 && alias.type.packed_members[3].name == "parity" && alias.type.packed_members[3].lsb_offset == 0,
        "record elements use declaration-order flattened offsets");
    require(
        architecture.signals.size() == 2 && architecture.signals[0].type.named_type == "packet_t" && architecture.processes.size() == 1 && architecture.processes.front().variables.size() == 1 && architecture.processes.front().variables.front().type.named_type == "packet_t",
    ...
    require(statements.size() == 5 && statements[0].target.text == "local_value.data" && statements[0].value.text == "source.data" && statements[1].target.kind == ExpressionKind::Slice && statements[1].target.operands.front().text == "local_value.shadow" && statements[1].value.kind == ExpressionKind::Slice && statements[1].value.operands.front().text == "source.shadow" && statements[2].target.text == "result.data" && statements[3].target.text == "result.valid",
...
begin
  ...
  begin
    ...
    require(packaged.design.units.size() == 3 && packaged.design.units[0].kind == UnitKind::VhdlPackage && packaged.design.units[0].type_aliases.size() == 1 && packaged.design.units[0].type_aliases.front().name == "packet_t" && packaged.design.units[0].type_aliases.front().type.width() == 5,
    ...
    require(
        packaged_entity.ports.size() == 2 && packaged_entity.ports[0].type.named_type == "packet_t" && packaged_entity.ports[1].type.named_type == "work.packet_types.packet_t" && packaged_architecture.signals.size() == 1 && packaged_architecture.signals[0].type.named_type == "packet_types.packet_t" && packaged_architecture.processes[0].variables[0].type.named_type == "packet_t",
...
begin
  ...
  begin
    ...
    const auto& aggregate_process = aggregates.design.units.back().processes.front();
    require(aggregate_process.variables.size() == 2 && aggregate_process.variables[0].initializer && aggregate_process.variables[1].initializer,
    ...
    const auto& positional_initializer = *aggregate_process.variables[1].initializer;
    require(named_initializer.kind == ExpressionKind::Aggregate && named_initializer.operands.size() == 2 && named_initializer.aggregate_choices == std::vector<std::string>({ "valid", "data" }) && named_initializer.operands[0].kind == ExpressionKind::BooleanLiteral && named_initializer.operands[1].kind == ExpressionKind::StringLiteral,
        "named record aggregate associations preserve canonical choices and "
        "source order");
    require(positional_initializer.kind == ExpressionKind::Aggregate && positional_initializer.operands.size() == 2 && positional_initializer.aggregate_choices == std::vector<std::string>({ "", "" }),
    ...
    require(
        aggregate_statements.size() == 5 && aggregate_statements[0].value.kind == ExpressionKind::Aggregate && aggregate_statements[0].value.aggregate_choices == std::vector<std::string>({ "data", "others" }) && aggregate_statements[1].value.kind == ExpressionKind::Aggregate && aggregate_statements[2].value.kind == ExpressionKind::Aggregate && aggregate_statements[3].value.kind == ExpressionKind::BooleanLiteral,
...
begin
  ...
  begin
    ...
    require(!invalid_aggregates.ok() && has_aggregate_code("FSIM-VHDL-PARSE-132") && has_aggregate_code("FSIM-VHDL-PARSE-133") && has_aggregate_code("FSIM-VHDL-SEM-038") && has_aggregate_code("FSIM-VHDL-SEM-039"),
...
begin
    ...
    require(has_code("FSIM-VHDL-PARSE-130") && has_code("FSIM-VHDL-SEM-035") && has_code("FSIM-VHDL-SEM-036") && has_code("FSIM-VHDL-SEM-037"),
...
begin
  ...
  begin
    ...
    const auto* entity = result.design.find(UnitKind::VhdlEntity, "select_concat");
    require(entity != nullptr && entity->ports[0].type.packed_range && entity->ports[0].type.packed_range->descending && entity->ports[1].type.packed_range && !entity->ports[1].type.packed_range->descending,
        "VHDL port range directions");
    const auto* architecture = result.design.find(UnitKind::VhdlArchitecture, "rtl");
    require(architecture != nullptr && architecture->processes.size() == 1 && architecture->processes.front().statements.size() == 5,
        "VHDL select/concatenation process");
    const auto& statements = architecture->processes.front().statements;
    require(
        statements[0].value.kind == ExpressionKind::Call && statements[0].value.text == "descending" && statements[0].value.operands.size() == 1,
        "ambiguous VHDL indexed-name syntax retained for semantic resolution");
    require(statements[1].value.kind == ExpressionKind::Slice && statements[1].value.text == "to" && statements[1].value.operands.size() == 3,
        "VHDL ascending slice expression");
    require(statements[2].value.kind == ExpressionKind::Binary && statements[2].value.text == "&" && statements[2].value.operands[0].kind == ExpressionKind::Binary,
        "left-associated VHDL concatenation expression");
    require(statements[3].target.kind == ExpressionKind::Index && statements[3].target.operands.size() == 2 && statements[3].target.operands[1].text == "4" && statements[4].target.kind == ExpressionKind::Slice && statements[4].target.text == "to",
...
begin
  ...
  absolute <= abs lhs;
    ...
    require(entity != nullptr && entity->ports.size() == 12 && std::ranges::all_of(entity->ports, [](const SignalDeclaration& port) {
        return port.type.is_signed;
    }),
        "VHDL signed subtype metadata");
    require(
        architecture != nullptr && architecture->concurrent_statements.size() == 10 && architecture->concurrent_statements[0].value.text == "/" && architecture->concurrent_statements[1].value.text == "rem" && architecture->concurrent_statements[2].value.text == "mod" && architecture->concurrent_statements[3].value.text == "sll" && architecture->concurrent_statements[4].value.text == "srl" && architecture->concurrent_statements[5].value.text == "sra" && architecture->concurrent_statements[6].value.text == "sla" && architecture->concurrent_statements[7].value.text == "rol" && architecture->concurrent_statements[8].value.text == "ror" && architecture->concurrent_statements[9].value.kind == ExpressionKind::Unary && architecture->concurrent_statements[9].value.text == "abs",
...
module signed_ops;
  ...
  always_comb begin
    ...
    require(unit.signals.size() == 21 && unit.signals[0].type.is_signed && unit.signals[1].type.is_signed && !unit.signals[2].type.is_signed,
        "SystemVerilog explicit signedness metadata");
    require(
        unit.processes.size() == 1 && unit.processes.front().statements.size() == 18 && unit.processes.front().statements[0].value.text == "/" && unit.processes.front().statements[1].value.text == "<" && unit.processes.front().statements[2].value.text == "<" && unit.processes.front().statements[2].value.operands.front().kind == ExpressionKind::Call && unit.processes.front().statements[2].value.operands.front().text == "$signed" && unit.processes.front().statements[3].value.text == ">>>" && unit.processes.front().statements[3].value.operands.front().text == "$unsigned" && unit.processes.front().statements[4].value.kind == ExpressionKind::Call && unit.processes.front().statements[4].value.text == "$isunknown" && unit.processes.front().statements[5].value.kind == ExpressionKind::Call && unit.processes.front().statements[5].value.text == "$bits" && unit.processes.front().statements[5].value.operands.front().kind == ExpressionKind::Concatenation && unit.processes.front().statements[6].value.text == "$left" && unit.processes.front().statements[7].value.text == "$right" && unit.processes.front().statements[8].value.text == "$low" && unit.processes.front().statements[9].value.text == "$high" && unit.processes.front().statements[10].value.text == "$size" && unit.processes.front().statements[10].value.operands.size() == 2 && unit.processes.front().statements[11].value.text == "$increment" && unit.processes.front().statements[12].value.text == "$dimensions" && unit.processes.front().statements[13].value.text == "$unpacked_dimensions" && unit.processes.front().statements[14].value.text == "$onehot" && unit.processes.front().statements[15].value.text == "$onehot0" && unit.processes.front().statements[16].value.text == "$countones" && unit.processes.front().statements[17].value.text == "$countbits" && unit.processes.front().statements[17].value.operands.size() == 3,
