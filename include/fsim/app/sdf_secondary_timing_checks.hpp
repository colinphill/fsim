...
struct SdfAppliedSecondaryTimingCheck {
    std::uint32_t check_id { };
    runtime::simir::ModuleTimingCheckKind kind {
        runtime::simir::ModuleTimingCheckKind::skew
    };
    ...
    friend bool operator==(const SdfAppliedSecondaryTimingCheck&,
        const SdfAppliedSecondaryTimingCheck&) = default;
};

class SdfSecondaryTimingCheckApplication final {
public:
    static constexpr std::uint32_t schema_version = 2U;

    [[nodiscard]] const std::shared_ptr<const SdfAnnotationPlan>& plan() const
        noexcept;
    [[nodiscard]] std::span<const SdfAppliedSecondaryTimingCheck> checks() const
        noexcept;
    [[nodiscard]] const SdfAppliedSecondaryTimingCheck* find_check(
        std::uint32_t id) const noexcept;
    [[nodiscard]] std::string_view semantic_identity() const noexcept;

    SdfSecondaryTimingCheckApplication(
...
private:
    ...
    std::vector<SdfAppliedSecondaryTimingCheck> checks_;
    std::string semantic_identity_;
};

struct SdfSecondaryTimingCheckLimits {
    std::size_t max_checks { 1'000'000U };
    std::size_t max_limits_per_check { 2U };
    std::size_t max_identity_bytes { 1U << 20U };
};

struct SdfSecondaryTimingCheckResult {
    std::shared_ptr<const SdfSecondaryTimingCheckApplication> application;
    std::vector<frontend::Diagnostic> diagnostics;
    [[nodiscard]] bool ok() const noexcept;
};

[[nodiscard]] SdfSecondaryTimingCheckResult apply_sdf_secondary_timing_checks(
    std::shared_ptr<const SdfAnnotationPlan> plan,
    const elaboration::ElaboratedDesign& elaborated,
    SdfSecondaryTimingCheckLimits limits = { });
