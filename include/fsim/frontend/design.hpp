...
/// spellings.
enum class SystemVerilogCovergroupOwnerKind { DesignUnit,
    Class };
...
enum class SystemVerilogCovergroupSamplingKind { Event,
    WithFunctionSample };
...
enum class SystemVerilogCovergroupOptionScope { Instance,
    Type };
...
enum class SystemVerilogCoverageDeclarationKind { Coverpoint,
    Cross };
...
struct SystemVerilogCoverageBin {
    SystemVerilogCoverageBinKind kind { SystemVerilogCoverageBinKind::Regular };
...
struct GenerateBody {
    ...
    std::vector<VhdlAttributeDeclaration> vhdl_attributes;
    std::vector<VhdlGroupDeclaration> vhdl_groups;
    std::vector<VhdlDisconnectionSpecification> vhdl_disconnections;
...
struct SystemVerilogModportMember {
    ...
    PortDirection direction { PortDirection::Unknown };
    SourceSpan span;
    SystemVerilogModportMemberKind kind { SystemVerilogModportMemberKind::Signal };
};
...
struct SystemVerilogSequenceExpression {
    ...
    std::vector<SystemVerilogSequenceIntersectionOperand> intersection_operands;
...
struct SystemVerilogPropertyUntilOperation {
    SystemVerilogPropertyUntilKind kind { SystemVerilogPropertyUntilKind::Until };
    std::vector<Token> left_tokens;
    std::vector<Token> right_tokens;
    SourceSpan span;
};
...
inline constexpr std::size_t maximum_udp_table_storage_bytes = 256U * 1024U * 1024U;
...
[[nodiscard]] bool
verilog_udp_level_matches(VerilogUdpLevelSymbol pattern,
    VerilogUdpLevelSymbol actual) noexcept;

[[nodiscard]] bool
verilog_udp_input_matches(const VerilogUdpInputPattern& pattern,
...
struct SystemVerilogDpiDeclaration {
    ...
    SystemVerilogDpiQualifier qualifier { SystemVerilogDpiQualifier::None };
...
enum class VhdlPslVerificationUnitKind {
    Unit,
    Property,
    Mode,
};

enum class VhdlPslDeclarationKind {
    DefaultClock,
    Boolean,
    Sequence,
    Property,
...
enum class VhdlPslDirectiveKind {
    Assert,
    Assume,
    Restrict,
    Cover,
...
enum class VhdlPslExpressionClass {
    Invalid,
    Boolean,
    Sequence,
    Property,
...
enum class VhdlPslTemporalOperatorKind {
    SequenceConcatenation,
    SequenceFusion,
    ConsecutiveRepetition,
    NonconsecutiveRepetition,
    GotoRepetition,
    OverlappedSuffixImplication,
    NonoverlappedSuffixImplication,
    Next,
    Previous,
    Eventually,
    Always,
...
enum class VhdlPslUnknownPolicy {
    False,
};

struct VhdlPslStaticRange {
...
struct VhdlPslTemporalOperator {
    VhdlPslTemporalOperatorKind kind {
...
struct VhdlPslReference {
    std::string name;
    VhdlPslExpressionClass expression_class { VhdlPslExpressionClass::Invalid };
    SourceSpan span;
};

struct VhdlPslAnalyzedExpression {
    VhdlPslExpressionClass expression_class { VhdlPslExpressionClass::Invalid };
    std::vector<Token> expression_tokens;
    std::vector<VhdlPslTemporalOperator> temporal_operators;
    std::vector<VhdlPslReference> references;
    std::vector<std::string> sampled_names;
    std::optional<VhdlPslClock> clock;
    VhdlPslUnknownPolicy unknown_policy { VhdlPslUnknownPolicy::False };
    SourceSpan span;
};

struct VhdlPslFormal {
    std::string name;
    Token name_token;
    std::vector<Token> profile_tokens;
    VhdlPslExpressionClass expression_class { VhdlPslExpressionClass::Invalid };
...
// Change 9 retains PSL declarations and directives as source-owned token HIR.
// Change 10 is responsible for typing the body token streams and assigning
// clocks; no parser storage or source-text reparsing is part of that contract.
struct VhdlPslDeclaration {
    VhdlPslDeclarationKind kind { VhdlPslDeclarationKind::Boolean };
    std::string name;
    std::optional<Token> name_token;
    std::vector<VhdlPslFormal> formals;
    std::vector<Token> body_tokens;
    std::optional<VhdlPslAnalyzedExpression> analyzed_expression;
    bool comment_embedded { };
    SourceSpan span;
};

struct VhdlPslDirective {
    VhdlPslDirectiveKind kind { VhdlPslDirectiveKind::Assert };
    std::string label;
    std::optional<Token> label_token;
    std::vector<Token> property_tokens;
    std::optional<VhdlPslAnalyzedExpression> analyzed_property;
    bool comment_embedded { };
    SourceSpan span;
};

struct VhdlPslVerificationUnit {
    VhdlPslVerificationUnitKind kind { VhdlPslVerificationUnitKind::Unit };
...
struct DesignUnit {
    ...
    std::optional<VhdlPslVerificationUnit> vhdl_psl_verification_unit;
    std::vector<VhdlPslDeclaration> vhdl_psl_declarations;
    std::vector<VhdlPslDirective> vhdl_psl_directives;
    ...
    std::vector<SystemVerilogDpiDeclaration> systemverilog_dpi_declarations;
    ...
    std::vector<SystemVerilogClockingBlock> systemverilog_clocking_blocks;
    std::optional<std::string> systemverilog_default_clocking_block;
    ...
    std::vector<VhdlComponentConfiguration> vhdl_configuration_specifications;
