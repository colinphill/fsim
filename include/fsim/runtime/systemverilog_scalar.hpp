...
[[nodiscard]] SystemVerilogScalarResult convert_systemverilog_scalar(
    ...
    SystemVerilogScalarRounding rounding = SystemVerilogScalarRounding::NearestAwayFromZero) noexcept;
...
[[nodiscard]] SystemVerilogPackedScalarResult systemverilog_scalar_to_packed(
    ...
    std::size_t width,
    bool is_signed,
    SystemVerilogScalarRounding rounding = SystemVerilogScalarRounding::NearestAwayFromZero);
...
/// Execute one IEEE 1800 scalar math or bit-preserving conversion function.
/// Packed integral operands remain arbitrary-width until the requested real
/// conversion; raw real/shortreal conversions preserve every payload bit.
[[nodiscard]] SystemVerilogPackedScalarResult
systemverilog_math_payload(
    SystemVerilogMathFunction function,
    const PackedLogic4& first,
    SystemVerilogScalarKind first_kind,
    bool first_signed,
    const PackedLogic4* second,
    SystemVerilogScalarKind second_kind,
    bool second_signed) noexcept;
...
struct SystemVerilogScalarFormatOptions {
    static constexpr std::uint32_t automatic_precision = std::numeric_limits<std::uint32_t>::max();
