...
public:
  ...
  friend bool operator==(const SystemVerilogUvmTlm1EndpointHandle &,
                         const SystemVerilogUvmTlm1EndpointHandle &) = default;

private:
  friend class SystemVerilogUvmTlm1Service;
  SystemVerilogUvmTlm1EndpointHandle(std::shared_ptr<const void> owner,
                                     std::uint64_t slot,
...
struct SystemVerilogUvmTlm1Profile {
  ...
  friend bool operator==(const SystemVerilogUvmTlm1Profile &,
                         const SystemVerilogUvmTlm1Profile &) = default;
};

struct SystemVerilogUvmTlm1EndpointDescriptor {
  SystemVerilogUvmTlm1EndpointKind kind{SystemVerilogUvmTlm1EndpointKind::Port};
...
public:
  ...
  explicit operator bool() const noexcept { return valid(); }
  friend bool operator==(const SystemVerilogUvmTlm1OperationHandle &,
                         const SystemVerilogUvmTlm1OperationHandle &) = default;

private:
  friend class SystemVerilogUvmTlm1Service;
  SystemVerilogUvmTlm1OperationHandle(std::shared_ptr<const void> owner,
                                      std::uint64_t slot,
                                      std::uint64_t generation)
      : owner_(std::move(owner)), slot_(slot), generation_(generation) {}
  std::shared_ptr<const void> owner_;
...
struct SystemVerilogUvmTlm1Payload {
  ...
  friend bool operator==(const SystemVerilogUvmTlm1Payload &,
...
public:
  using CompletionCallback =
      std::function<void(const SystemVerilogUvmTlm1OperationSnapshot &)>;
  using TransportHandler =
      std::function<std::optional<SystemVerilogUvmTlm1Payload>(
          const SystemVerilogUvmTlm1Payload &)>;
  using AnalysisSubscriber = std::function<void(SystemVerilogUvmTlm1Payload)>;

  explicit SystemVerilogUvmTlm1Service(
      SystemVerilogUvmComponentService &components,
      SystemVerilogUvmTlm1Limits limits = {});
  SystemVerilogUvmTlm1Service(SystemVerilogClassHeap &heap,
                              SystemVerilogUvmComponentService &components,
                              SystemVerilogUvmTlm1Limits limits = {});
  ~SystemVerilogUvmTlm1Service();
  SystemVerilogUvmTlm1Service(const SystemVerilogUvmTlm1Service &) = delete;
  SystemVerilogUvmTlm1Service &
  operator=(const SystemVerilogUvmTlm1Service &) = delete;

  [[nodiscard]] SystemVerilogUvmTlm1EndpointHandle
  register_endpoint(SystemVerilogUvmTlm1EndpointDescriptor descriptor);
  void release_endpoint(SystemVerilogUvmTlm1EndpointHandle endpoint);
  void connect(SystemVerilogUvmTlm1EndpointHandle source,
  ...
  void configure_fifo(SystemVerilogUvmTlm1EndpointHandle implementation,
                      std::size_t capacity);
  void set_transport_handler(SystemVerilogUvmTlm1EndpointHandle implementation,
                             TransportHandler handler);
  [[nodiscard]] SystemVerilogUvmTlm1FifoSnapshot
  fifo_snapshot(SystemVerilogUvmTlm1EndpointHandle implementation) const;
  [[nodiscard]] std::vector<SystemVerilogUvmTlm1FifoSnapshot>
  fifo_snapshots() const;
  [[nodiscard]] std::vector<SystemVerilogUvmTlm1Payload>
  fifo_values(SystemVerilogUvmTlm1EndpointHandle implementation) const;
  [[nodiscard]] bool implementation_can_write(
      SystemVerilogUvmTlm1EndpointHandle implementation) const;
  [[nodiscard]] bool
  implementation_try_write(SystemVerilogUvmTlm1EndpointHandle implementation,
                           SystemVerilogUvmTlm1Payload payload,
                           bool response = false);
  [[nodiscard]] std::optional<SystemVerilogUvmTlm1Payload>
  implementation_try_read(SystemVerilogUvmTlm1EndpointHandle implementation,
                          bool peek = false);

  [[nodiscard]] bool can_put(SystemVerilogUvmTlm1EndpointHandle source,
                             std::size_t binding_index = 0) const;
  [[nodiscard]] bool try_put(SystemVerilogUvmTlm1EndpointHandle source,
                             SystemVerilogUvmTlm1Payload payload,
                             std::size_t binding_index = 0);
  [[nodiscard]] bool can_get(SystemVerilogUvmTlm1EndpointHandle source,
                             std::size_t binding_index = 0) const;
  [[nodiscard]] std::optional<SystemVerilogUvmTlm1Payload>
  try_get(SystemVerilogUvmTlm1EndpointHandle source,
          std::size_t binding_index = 0);
  [[nodiscard]] bool can_peek(SystemVerilogUvmTlm1EndpointHandle source,
                              std::size_t binding_index = 0) const;
  [[nodiscard]] std::optional<SystemVerilogUvmTlm1Payload>
  try_peek(SystemVerilogUvmTlm1EndpointHandle source,
           std::size_t binding_index = 0);
  [[nodiscard]] bool can_transport(SystemVerilogUvmTlm1EndpointHandle source,
                                   std::size_t binding_index = 0) const;
  [[nodiscard]] std::optional<SystemVerilogUvmTlm1Payload>
  try_transport(SystemVerilogUvmTlm1EndpointHandle source,
                SystemVerilogUvmTlm1Payload request,
                std::size_t binding_index = 0);

  [[nodiscard]] SystemVerilogUvmTlm1OperationHandle
  put(SystemVerilogUvmTlm1EndpointHandle source,
      SystemVerilogUvmTlm1Payload payload,
      SystemVerilogUvmPhaseHandle phase = {},
      CompletionCallback completion = {}, std::size_t binding_index = 0);
  [[nodiscard]] SystemVerilogUvmTlm1OperationHandle
  get(SystemVerilogUvmTlm1EndpointHandle source,
      SystemVerilogUvmPhaseHandle phase = {},
      CompletionCallback completion = {}, std::size_t binding_index = 0);
  [[nodiscard]] SystemVerilogUvmTlm1OperationHandle
  peek(SystemVerilogUvmTlm1EndpointHandle source,
       SystemVerilogUvmPhaseHandle phase = {},
       CompletionCallback completion = {}, std::size_t binding_index = 0);
  [[nodiscard]] SystemVerilogUvmTlm1OperationHandle
  transport(SystemVerilogUvmTlm1EndpointHandle source,
            SystemVerilogUvmTlm1Payload request,
            SystemVerilogUvmPhaseHandle phase = {},
            CompletionCallback completion = {}, std::size_t binding_index = 0);
  void complete_transport(SystemVerilogUvmTlm1OperationHandle operation,
                          SystemVerilogUvmTlm1Payload response);
  [[nodiscard]] SystemVerilogUvmTlm1OperationSnapshot
  operation_snapshot(SystemVerilogUvmTlm1OperationHandle operation) const;
  [[nodiscard]] std::vector<SystemVerilogUvmTlm1OperationHandle>
  operations() const;
  ...
  void
  set_analysis_subscriber(SystemVerilogUvmTlm1EndpointHandle implementation,
                          AnalysisSubscriber subscriber);
  void
  configure_analysis_fifo(SystemVerilogUvmTlm1EndpointHandle implementation,
                          std::size_t capacity);
  [[nodiscard]] SystemVerilogUvmTlm1EndpointHandle
  register_analysis_implementation(SystemVerilogClassHandle component,
                                   std::string name, std::string nominal_type,
                                   AnalysisSubscriber subscriber);
  [[nodiscard]] SystemVerilogUvmTlm1AnalysisResult
  write_analysis(SystemVerilogUvmTlm1EndpointHandle source,
                 SystemVerilogUvmTlm1Payload payload);
  [[nodiscard]] std::optional<SystemVerilogUvmTlm1Payload>
  analysis_fifo_try_get(SystemVerilogUvmTlm1EndpointHandle implementation);

  [[nodiscard]] bool
  contains(SystemVerilogUvmTlm1EndpointHandle endpoint) const noexcept;
  [[nodiscard]] SystemVerilogUvmTlm1EndpointSnapshot
  snapshot(SystemVerilogUvmTlm1EndpointHandle endpoint) const;
  [[nodiscard]] std::vector<SystemVerilogUvmTlm1EndpointHandle>
  endpoints() const;
  [[nodiscard]] std::span<const SystemVerilogUvmTlm1EndpointHandle>
  resolved(SystemVerilogUvmTlm1EndpointHandle endpoint) const;
...
private:
  ...
  [[nodiscard]] const Endpoint &
  endpoint(SystemVerilogUvmTlm1EndpointHandle handle) const;
  [[nodiscard]] SystemVerilogUvmTlm1EndpointHandle
  handle(std::uint64_t slot) const;
  void require_live(const Endpoint &endpoint) const;
  [[nodiscard]] bool reaches(std::uint64_t from, std::uint64_t wanted,
  ...
  implementation_slot(SystemVerilogUvmTlm1EndpointHandle source,
                      SystemVerilogUvmTlm1OperationKind operation,
                      std::size_t binding_index) const;
  [[nodiscard]] bool supports(SystemVerilogUvmTlm1Interface interface_kind,
                              SystemVerilogUvmTlm1OperationKind operation,
  ...
  void validate_payload(SystemVerilogUvmTlm1Payload &payload,
                        const Endpoint &implementation, bool response);
  void require_payload_type(const SystemVerilogUvmTlm1Payload &payload,
                            const Endpoint &implementation,
                            bool response) const;
  [[nodiscard]] bool
  payload_type_matches(const SystemVerilogUvmTlm1Payload &payload,
                       const Endpoint &implementation,
  ...
  void publish_fifo_activity(std::uint64_t implementation,
                             std::string_view detail);
  [[nodiscard]] SystemVerilogUvmTlm1OperationHandle
  create_operation(SystemVerilogUvmTlm1OperationKind kind,
                   SystemVerilogUvmTlm1EndpointHandle source,
                   ...
                   CompletionCallback completion);
  [[nodiscard]] SystemVerilogUvmTlm1OperationHandle
  operation_handle(std::uint64_t slot) const;
  [[nodiscard]] Operation &
  operation(SystemVerilogUvmTlm1OperationHandle handle);
  [[nodiscard]] const Operation &
  operation(SystemVerilogUvmTlm1OperationHandle handle) const;
